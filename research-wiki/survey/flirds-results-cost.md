...
**참고 — 오염축 정렬 축 그리드의 자체 측정치** `[비-canonical]` ● cifar10 24/24 · ◐ mnist 17/24
...
> median 표본 = cifar10 **24셀**(완주) · mnist **17셀**. 07-26 스냅샷(cifar10 9셀·mnist 8셀) 대비 값이 움직였다 — **동시 8셀 경합의 표본 편차**이지 코드 변화가 아니고, **방법 간 서열은 그대로**다.

| 방법 | mnist (median s) | cifar10 (median s) |
|---|---|---|
| **Flirds-1st** | **0.098** | **0.384** |
| loss-heur | <u>0.171</u> | 0.816 |
| FedIF | 0.189 | <u>0.509</u> |
| **Flirds** | 0.996 | 1.405 |
| ComFedSV | 5.757 | 22.262 |
| FedSV | 6.877 | 27.097 |
| GTG | 20.368 | 94.770 |
| ShapleyFL | 34.205 | 134.641 |
| _(참조) 학습 궤적 자체_ | _133.7_ | _111.3_ |
| _(참조) **(a)oracle 2¹⁰ 재학습**_ | _63,042 (17.5 h)_ | _49,814 (13.8 h)_ |
...
- C1 축 그리드(비-canonical 참고치): `python runs/track_c/c1/make_analysis.py` → `analysis/runtime_median.csv`(방법별 median) · `cells.csv`의 `traj_time`·`t_a`. **41/48셀 기준**(cifar10 24 완주 · mnist 17).
