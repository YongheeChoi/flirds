...
**참고 — 오염축 정렬 축 그리드의 자체 측정치** `[비-canonical]` ● cifar10 24/24 · ◐ mnist 19/24
...
> median 표본 = cifar10 **24셀**(완주) · mnist **19셀**(2026-07-27 dir1 seed0 2셀 추가). 07-26 스냅샷(cifar10 9셀·mnist 8셀) 대비 값이 움직였다 — **동시 8셀 경합의 표본 편차**이지 코드 변화가 아니고, **방법 간 서열은 그대로**다.

| 방법 | mnist (median s) | cifar10 (median s) |
|---|---|---|
| **Flirds-1st** | **0.098** | **0.384** |
| loss-heur | <u>0.171</u> | 0.816 |
| FedIF | 0.190 | <u>0.509</u> |
| **Flirds** | 1.025 | 1.405 |
| ComFedSV | 5.757 | 22.262 |
| FedSV | 6.877 | 27.097 |
| GTG | 20.368 | 94.770 |
| ShapleyFL | 34.310 | 134.641 |
| _(참조) 학습 궤적 자체_ | _133.6_ | _111.3_ |
| _(참조) **(a)oracle 2¹⁰ 재학습**_ | _63,000 (17.5 h)_ | _49,814 (13.8 h)_ |

> **배율 구조는 그대로다**: Flirds vs ShapleyFL = **33×**(mnist 1.025 vs 34.310) / **96×**(cifar10 1.405 vs 134.641) — canonical 표의 98×(1.168 vs 114.5)와 같은 자릿수. (a) 재학습은 Flirds의 **61,500×**(mnist 63,000/1.025) / **35,500×**(cifar10 49,814/1.405) — canonical의 64,730×/28,177×와 자릿수 동일. 절대 초는 하드웨어 탓에 어긋나지만(**GTG·(a)가 특히**: 3090 cifar10 94.8 s vs canonical 87.7 s, (a) 49,814 s vs 32,912 s) **"valuation ≪ 학습 ≪ (a) 재학습"의 3단 구조**는 두 환경에서 같다. **출처**: `runs/track_c/c1/analysis/runtime_median.csv` · `cells.csv`(`traj_time`·`t_a`).
...
- C1 축 그리드(비-canonical 참고치): `python runs/track_c/c1/make_analysis.py` → `analysis/runtime_median.csv`(방법별 median) · `cells.csv`의 `traj_time`·`t_a`. **43/48셀 기준**(cifar10 24 완주 · mnist 19).
