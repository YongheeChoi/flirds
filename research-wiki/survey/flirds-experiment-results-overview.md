...
> **기준일**: 2026-07-23 · **git HEAD**: `373a1d8`(2026-07-23) · **스코프**: flirds 프로젝트의
...
> - φ-부호 감사(§5.2): `runs/track_g/audit/` (전 rundir 파생 재분석) · φ-게이팅 Phase B(§3.2.2): `runs/track_g/rundirs/`(LLM 218; ⚠ `rundirs_llm/` 폴더는 존재하지 않음) + `rundirs_cnn/`(**144 = COMPLETE**; skew 레짐+strmain — **표 미게재**: §3.2.3 경쟁과 중복, §6.2-14) + `rundirs_cnn_restack/`(36) + `rundirs_cnn_v3/`(12) · 정본 `runs/track_g/analysis/{llm_summary.csv,cnn_summary.csv}`(3-seed 완주 = canon)
> - 점수원 경쟁(§3.2.3–4·§4.8): `runs/track_h/rundirs_cnn/`(228=경쟁 96+strmain 24+P5 108) · `rundirs_llm/`(**56**=silo5 12+**gsm50k5 44**) · `rundirs_llm_gn20/`(1) · `gnoise_diag/`(진단) · `rundirs_cnn_scale/`(21) · `rundirs_cnn_dyn/`(9) · 정본 `runs/track_h/analysis/*.csv`(R4 = `gsm50k5_tier_a.csv`) + `scale/analysis/` + `dyn/analysis/`
...
>
> 자매 문서(중복 X, 링크만): 선행연구 지형 분류는 [[prior-work-taxonomy/README]]
> (baseline 수치 ↔ 원 논문 대조 문서는 2026-07-23 삭제 = 4399be3, git 이력에서 조회).
...
- AUROC = 탐지 ↑ (1=완벽; 0.5=무작위; <0.5=뒤집힘) · runtime = wall-clock(초) ↓
- **표 헤더의 ↑/↓** = 그 열 지표가 **클수록 좋음(↑) / 작을수록 좋음(↓)**. Spearman·Kendall·Pearson·AUROC·MMLU·ROUGE-L·정확도·rho/Jaccard = ↑ · 거리(cosine_d·euclid_d·max_diff)·val-loss·runtime = ↓ · flirds_keep 등은 방법 우열이 아니라 설정/산출이라 방향 표기 없음. 열이 α·threat·scale·arm 이름인 표는 캡션에 지표 화살표를 단다.
- **위협 스코프(2026-07-22)**: 본 문서(=논문 스코프)는 **noise류(noisy·grad/feature-noise)·free-rider류(zero/random/delta)·label-flip류**만 다룬다 — §6.2 caveat 8.
...
| 8     | CNN · cross-device N=100 (`track_c/c2`)                                                     | §3.3.5(탐지 주석)        | 소형 CNN · N=100 · 10/100                       | client | in-run intervention arms (+recon/sfedavg)                         | **Perf·Detection**                      | – (개입 arm)                  | corrupt 마스크                           | 3            | ● (개입 표 미사용 07-22 — §6.2-14; rundir 존속)                                                                                                                                                                                                                                                                                                                 |
| 9     | Robustness · 1B cross-silo N=5 (`phase2_matrix/1B_silo5_*`)                                 | §3.3.1(·§3.1.4)      | Llama-3.2-1B · N=5 · full                     | client | in-run (+recon/IF/탐지기4)                                           | **Fidelity·Detection**·Cost             | approx vs exact(b)          | (b) 2⁵ in-run                         | 3            | ● (오염 셀=β0.3 재실행판 ce0b454)                                                                                                                                                                                                                                                                                                                              |
| 10    | Robustness · 1B cross-device N=100 α-sweep (`.../1B_device100-a{0,0.01,0.1,5.0}_*`)         | §3.3.2               | Llama-3.2-1B · N=100 · 10/100                 | client | in-run (+IF/ComFedSV/탐지기4)                                        | **Detection**·Fidelity                  | approx vs Flirds-proxy      | **Flirds proxy reference**            | 3            | ● (β0.3 재실행판 = a0.1 3위협 c05a951·49c402a + a0.01 noisy·frrand 2d3f482)                                                                                                                                                                                                                                                                                    |
...
| 12    | Robustness · 3B cross-silo N=5 (`phase2_matrix/3B_silo5_*`)                                 | §3.3.3               | Llama-3.2-3B · N=5 · full                     | client | in-run (+IF/탐지기4)                                                 | Fidelity·Detection                      | approx vs exact(b)          | (b) 2⁵ in-run                         | **1**        | ◐                                                                                                                                                                                                                                                                                                                                                       |
| 13    | Foundational · 1B 첫 clean run (`phase1/...full-lr*`)                                        | §6.1                 | Llama-3.2-1B · N=5 · full                     | client | in-run                                                            | Detection                               | approx                      | 주입 라벨(oracle 없음)                      | 3×2lr        | ● (top-k arm 미사용 07-22 — §6.2-14)                                                                                                                                                                                                                                                                                                                       |
| 14    | Foundational · 1B LR sweep (`phase1/...sweep-lr*`)                                          | §6.3(검증-전용)          | Llama-3.2-1B · N=5 · full                     | client | in-run                                                            | Detection·Perf(selection)               | approx vs exact(b)          | (b) 2⁵ in-run                         | 1×4lr        | ●                                                                                                                                                                                                                                                                                                                                                       |
| 15    | Signal-size probe C1 (`probe_signal/cnn_c1`)                                                | §4.3                 | 소형 CNN · N=10 · 2·5·10/10                     | client | in-run (+recon/IF)                                                | **Fidelity** (폭·참여 sweep)               | approx vs exact(b)          | (b) 2¹⁰/per-round in-run              | 3            | ●                                                                                                                                                                                                                                                                                                                                                       |
...
| 23    | φ-부호 감사 Stage 0 (`track_g/audit`, derived)                                                  | §5.2                 | 기존 전 rundir 309개                              | client | – (재분석)                                                           | **Track G 예측 확정/수정**                    | –                           | rundir φ 부호                           | –            | ●                                                                                                                                                                                                                                                                                                                                                       |
| 24    | φ 부호-게이팅 Phase B (`track_g/rundirs`+`rundirs_cnn(+_v3)`)                                    | §3.2.2(LLM)          | 1B N=5 full (silo5·iid5) + 소형 CNN N=100       | client | in-run 게이트 arms (sign/z/V2w/V3)                                   | **Perf(온라인 배제)·게이트 P/R**                | – (개입 arm)                  | oracle_excl 상한 + corrupt 마스크          | 3 (일부 seed0) | ● CNN 그리드 144/144 완주하되 **표 미게재**(§3.2.3 경쟁과 중복 = 07-22 결정, 07-23 완주 검토서 재확인 — §6.2-14); CNN rundir는 §3.2.3 R1이 재사용                                                                                                                                                                                                                                                                                   |
| 25    | Track H 확증 런 3종 — P5 신뢰-게이트(108런)·Scale 완전참여(21런)·Dyn 재추첨(9런) (`track_h/rundirs_cnn*`)      | §4.8                 | 소형 CNN · N=100 · 10/100 및 100/100             | client | in-run 게이트 arms (P1/P5h/P5s)                                      | **Perf(정책 확증; 사전등록 예측 대조)**             | – (개입 arm)                  | oracle_excl/random_excl + corrupt 마스크 | 3            | ●                                                                                                                                                                                                                                                                                                                                                       |
| —     | **이하 계획 행 (⬚=미실행; 완료된 행은 status ●/◐·§ 참조)**                                                 |                      |                                               |        |                                                                   |                                         |                             |                                       |              |                                                                                                                                                                                                                                                                                                                                                         |
| P1    | LLM N=10 (a)/(b) oracle                                                                     | §3.1.3               | LLM · N=10                                    | client | retrain+in-run                                                    | Fidelity(고-power)                       | exact                       | (a)/(b) 2¹⁰                           | (b)=1        | ◐ (b) seed0 완료=행20; **잔여((a) 2¹⁰·seeds1-2) = 미진행 확정**(Yonghee 07-22 — 시간 제약; seed0 결과는 1-seed caveat로 유지)                                                                                                                                                                                                                                               |
...
| P10   | **Track H 점수원 경쟁** (`track_h/rundirs_cnn`+`rundirs_llm`)                                    | §3.2.3               | CNN N=100 dir1 + 1B silo5 noisy               | client | in-run **전 점수원 8종** × 정책 4(sign±가중/mult/z) × 시점 2(online/retrain) | **Perf(경쟁: 어느 φ 정의가 학습을 잘 만드나)**        | – (개입 arm)                  | oracle_excl 상한 + corrupt 마스크          | 3 (Tier1·2)  | ◐ Tier 1 CNN 96런+Tier 2 LLM 12런 완주(FAIL 0); **Tier 3·R2 std50k5 = 미사용 예정**(Yonghee 07-22 — 논문 비게재; rundir 존속) — LLM 경쟁 본판은 R4                                                                                                                                                                                                                           |
| P11   | **Track H R4 — gsm50k5 accuracy 무대** (`track_h/rundirs_llm/gsm50k5_*`)                      | §3.2.4               | Llama-3.2-1B-Instruct · N=50 · 5/50 · GSM8K   | client | in-run 게이트 arms (P1·T2, 이후 P5-leg)                                | **Perf(LLM 경쟁; exact-match 심판)**        | – (개입 arm)                  | oracle_excl + corrupt 마스크             | Tier A=seed0 | ◐ **Tier A seed0 + P5-soft 전부 완주**(1-seed)(Tier A 4c40e30 · P5s online 3 + t2 12 = 77ba500·49c402a·**4fb02fb**[마지막 frzero×fedif, 07-23]) — §3.2.4(T1/T2 절대 EM 표). **gnoise = γ축 종결**(γ={1,5,20} 전부 무대 미성립 — negative result 77ba500; §3.2.4 미수록, 기록 = §6.3). Tier B(전 8종 점수원, ~300–350 GPU-h) = Yonghee 승인 게이트; Tier C 3-seed = 차기 큐 L1. **LLM selection 무대 확정**(Yonghee 07-22: alpaca/MMLU/ROUGE 심판은 성능 차이 변별력 부족 → GSM8K exact-match) |
| P12   | **R4 (b)-fidelity 셀** (`phase2_matrix` REGIME=gsm50k5 → `1B_gsm50k5_{noisy,clean}_s0`) | §3.1(신설 예정)         | Llama-3.2-1B-Instruct · N=50 · 5/50 · GSM8K   | client | in-run 9종 (+거리·timing; 러너의 Fed-LOO 산출은 비게재)                     | **Fidelity**(R4 무대에 exact oracle 부착)   | approx vs exact(b)-perround | **(b) per-round**                     | seed0 우선     | ⬚ 큐 등재(0c8b9aa — 러너 이식 완료·사전등록 **H-13**; 셀당 ~10–15 GPU-h). **R4 개입 결과의 유일한 fidelity 대조축** — 현 §3.2.4은 (b) 없이 EM만으로 비교(§6.2 caveat 참조) |
| P13   | **CNN C2 fidelity leg** (`track_c/c2fid`, 144셀)                                             | §3.1(신설 예정)         | 소형 CNN · N=100 · 10/100                       | client | in-run 8종(C1 11종 −Banzhaf −Ripple; 러너의 Fed-LOO 산출은 비게재)          | **Fidelity**(부분참여 무대 vs (b))           | approx vs exact(b)-perround | **(b) per-round**(셀당 122,880 eval)   | 3            | ◐ 파일럿 1셀 착지(b31666f; cifar10 dir1 grad-noise seed0): **1.05 GPU-h/셀**(oracle-b 824s + valuation 2669s) → 144셀 본런 ≈ **151 GPU-h** GO-게이트. F-1/F-2/F-4 = 단일 셀이라 계열-Δ 산출 불가(nan), **F-3 MISS**(그 1셀 Spearman vs (b): loss-heur .989 > FedSV .870 > GTG .861 > Flirds .846 > ShapleyFL .784 > FedIF .774 > ComFedSV/Flirds1st .27) — **부분참여 1셀 = 인용 불가, 본런 대기**. downstream 그리드와 **동결 궤적 비트동일 twin** |
| P14   | **CNN downstream 확장** (`track_g/rundirs_cnn` skew·restack + `track_h` strmain)              | §3.2.3 (b1′)          | 소형 CNN · N=100 · 10/100                       | client | in-run 게이트 arms (P1/P5)                                          | **Perf**(skew축 분해·강도응답 ruler)          | – (개입 arm)                  | oracle_excl + corrupt 마스크             | 3            | ● 완주 — track_g `rundirs_cnn` **144/144**(skew 레짐 qskew/shard/fmnist + strmain; 570a93f+restack 373a1d8) **표 미게재**(§3.2.3 경쟁과 중복 = §6.2-14, 07-23 재확인; skew 2×2 분해·V2w DO-NOT-PROMOTE·prereg H-K MISS는 `track_g/analysis`·rundir에 존속) · track_h **strmain 24/24**(bd15fa1) = §3.2.3 경쟁 새 stage → **(b1′)에 반영**(estimator>renorm 재현) |
...
> **⚠ §3.2 재번호(2026-07-23)**: 미사용 3절 삭제(§6.2-14)로 생긴 결번을 연속 번호로 정리했다 — **구 §3.2.3→§3.2.2 · 구 §3.2.6→§3.2.3 · 구 §3.2.7→§3.2.4**(§3.2.1은 불변). 표·본문의 "**구 §3.2.x**" 표기는 언제나 **재번호 전 문서**를 가리키며 현행 번호와 무관하다(예: "구 §3.2.4"=삭제된 CNN 게이트 그리드 ≠ 현 §3.2.4=R4).
...
| `track_c` C2 (구 §3.2.2·§3.3.2) | CNN cross-device 개입 | **미사용(07-22 — §6.2-14; git 이력)**; 탐지 주석만 §3.3.5 |
...
| `removal_dose` A2·A3 (구 §3.7.1–2·§3.7.5) | removal-curve(게임-무관 검증) | §4.4 |
...
| `track_g` Phase B (구 §3.2.3–4) | φ 부호-게이팅 + V3 | **§3.2.2**(LLM); CNN 그리드(구 §3.2.4)는 **미사용(07-22 — §6.2-14)** |
| `track_h` Tier 1·2·R2 (구 §3.2.6) | 점수원 경쟁 본판 | **§3.2.3** |
| `track_h` P5·Scale·Dyn (구 §3.2.6 하위 블록) | 확증 런 3종 | §4.8 |
| `track_h` R4 (`gsm50k5`) | LLM accuracy 경쟁 무대 | §3.2.4 (Tier A seed0 + P5-soft 완주; gnoise 신정의·Tier B는 §2 P11 status) |
| `phase1` full (구 §4.1·§3.2.5) | foundational 탐지 (top-k selection arms는 미사용 — 07-22) | §6.1 (top-k 미사용 = §6.2-14) |
...
| 구 §2.5·§3.2.7 종합판정 (재구조화 전 번호 — 현 §3.2.4와 무관) | 위계별 승·패 분석 | §5.1 |
...
> **진행 중인 확장 2건(2026-07-23 큐 등재; 수치 착지 전 = 절 신설 대기)** — 현 fidelity 표들이 대부분 포화(Spearman +1.000 근방)라 변별이 안 나는 문제에 대한 답:
> - **P12 R4 (b)-fidelity 셀** (`phase2_matrix` REGIME=gsm50k5): §3.2.4의 accuracy 무대(N=50·5/50·오염 40%)에 **exact (b) per-round oracle을 처음 부착** — 개입 결과(EM)와 fidelity를 같은 무대에서 잇는다. 사전등록 H-13 = same-game 계열 ≈1.0 vs renorm/uniform 계열 하락.
> - **P13 CNN c2fid leg** (`track_c/c2fid`, 144셀): C2 부분참여(10/100) 무대의 **동결 궤적 twin** 위에서 9방법 vs (b). 사전등록 F-1~F-4(비등n qskew서 uniform-subset 계열 하락 · free-rider exact-0 vs renorm 유령값 · 참여축 붕괴 서열 재현 · strmain dose 해상도). ⚠ **C1(N=10 전원참여)과 다른 게임**이라 §3.1.2 표와 수치 직접 비교 금지.
...
- **제외**: **(a) retrain oracle** = 비용(2⁵×R 재학습) → 1B anchor5만, 3B/7B ⬚. (전역 비교군 제외 = §6.2 caveat 12·15.)
...
- **제외**: 탐지기(FLDetector/FLTrust/STD-DAGMM/FedDQC) = C1엔 update-level 위협 축이 없음(시나리오는 skew/flip/noise) → *적용규칙: 탐지기는 오염축 있는 실험만* (탐지는 §3.3). (a)/(b) exact 가능 = N=10 ≤ 10이라 *exact 2ᴺ 규칙 충족*.
...
> - **silo5 (N=5, (b) 2⁵; β0.3 재실행판 ce0b454)**: noisy·free-rider(random/zero) 위협서 Flirds/Flirds-1st/loss-heur Spearman **1.000**, FedSV 0.93~1.0, GTG 1.0, FedIF 0.90~0.93, ComFedSV 0.83~0.87.
...
**(a) 세팅**: §3.3.1 silo5와 동일 무대(N=5 full, R=10, 10 steps, batch 16, lr=1e-3, maxlen 768, train200/val20/test40, warmup2, (b)=exact 2⁵, 3 seed), 단 **비IID축과 오염축을 2×2로 분리**: 비IID축 = `iid5`(build_alpaca_iid 균질) ↔ `silo5`(5-domain 비IID); 오염축 = clean ↔ {noisy·free-rider}. 신규 = iid5 셀 + silo5_clean(silo5 오염 셀은 §3.3.1 재사용 — **β0.3 재실행판**). 목적 = "silo5의 신호가 오염 때문인가 도메인 이질성 때문인가"의 분리. 드라이버 = `runs/matrix_cxni/`(rundir는 `phase2_matrix/rundirs/`에 착지).

**(b1) 1차 fidelity — (b)oracle 자기순위 cross-seed ρ ↑** (신호 실재성; 1=완전재현·≈0=추첨노이즈; 3-seed 쌍별 Spearman 평균)

| 무대 \ 오염 | clean | noisy | free-rider(zero) |
|---|---|---|---|
| **IID** (iid5) | 0.13 | 0.60 | 0.70 |
| **non-IID** (silo5) | **0.87** | 0.93 | 0.93 |

> **headline (결정타)**: **non-IID clean ρ 0.87** — 오염이 0인데 **도메인 분리만으로** oracle 자기순위가 재현된다 → silo5의 높은 fidelity가 오염이 아니라 **도메인 이질성** 때문임을 확정(진단 §1.4 caveat 해소). 대비 **IID clean ρ 0.13**(≈0, 신호 거의 없음 = §3.1.2 (b2)·진단 §1.4 재현). 두 축이 각각 독립적으로 신호를 만든다(오염 하나만 있어도 IID 0.60~0.70; 둘 다면 0.93). → **A축(rank·참여·lr·steps §4.2, 폭 §4.3)이 신호를 못 만든 것과 정확히 대비: 신호는 B축(클라 간 실제 차이)이 만든다.** ⚠ silo5 frzero 칸은 β0.3 재실행(ce0b454) 후 0.93(재실행 전 rundir는 1.000 — seed2에서 근소차 순위 1쌍 뒤집힘; tracked `matrix_cxni/figures/crossseed_rho.csv`는 재실행 전 산출이라 1.000으로 남아 있음 — rundir 재계산이 정본).
...
> - STD-DAGMM(model-free)은 전반 약하고 배경 무관 저조(silo5 값은 β0.3 재실행판: noisy 0.25·frzero 0.00).

**(c) 출처·baseline-set 노트**
- **출처**: `runs/phase2_matrix/rundirs/1B_{iid5,silo5}_{clean,noisy,frrand,frzero}/` — cross-seed ρ = `phi.parquet`의 (b)oracle를 seed로 피벗한 쌍별 Spearman, AUROC = `metrics.json`의 per-seed `auroc` 평균. **⚠ `make_analysis.py`(06-19 생성)엔 iid5/silo5_clean 미포함** → 이 표는 rundir 직접 집계(master_metrics.csv에 없음). **정식 재생성 = `runs/matrix_cxni/make_figures.py`**(B축 셀 전담; tracked figures는 β0.3 재실행 전 산출 — 재실행 후 재생성 필요).
- valuation φ(Flirds 등) + 탐지기 4종. silo5 오염 셀 재사용(§3.3.1).
...
| ------------------------------------ | ----------------------------- | ------------------------------------------- | ----------------------------------- | ------------------------------------------- |
| retrain × top-k                      | —                             | ○ 미사용(phase1 K=3/5 — 07-22, §6.2-14)      | —                                   | —                                           |
| retrain × 양수-only (V3)               | ● iid5 (§3.2.2) · **gsm50k5 T2 4점수원** (§3.2.4) | ● silo5 (§3.2.2)                            | ○ 미사용(V3 — 07-22, §6.2-14)          | ● dir1 T2 **점수원 8종**×{plain, 크기가중} (§3.2.3) |
| per-update × soft top-k (softmax 선택) | ● std20 `flirds_sel` (§3.2.1) | —                                           | ○ 미사용(C2 — 07-22, §6.2-14)          | ○ 미사용(C2 — 07-22, §6.2-14)                 |
| per-update × 양수-only (sign-게이트)      | ● iid5 (§3.2.2) · **gsm50k5** (§3.2.4) | ● silo5 (§3.2.2; noisy 셀 **점수원 8종** §3.2.3) | ○ 미사용(게이트 그리드 — 07-22, §6.2-14)     | ● dir1 (§3.2.3 — **점수원 8종**)                |

> 매트릭스 주: **하드 top-k per-update arm은 전 무대 설계상 부재** — per-update 선택은 softmax 샘플링(확률적 soft top-k) 아니면 parameter-free sign-게이트뿐이며, top-k 상한은 `oracle_excl`이 대역(`runs/track_g/README.md` §7). soft **가중** arm(flirds_w 등 — 선택이 아니라 집계 가중)은 §3.2.1(LLM)·§3.2.3 P3(CNN, 점수원 8종)에 수록. CNN 개입의 문서 내 증거는 **Track H 경쟁(§3.2.3)·확증 런(§4.8)으로 일원화**(구 §3.2.2 C2 개입·§3.2.4 게이트 그리드·§3.2.5 top-k = 미사용 삭제, §6.2-14).
>
...
>
> **[07-19 Yonghee 판정 — 위 공백 자체가 채워야 할 결함]**: fidelity는 우리가 정의한 게임((b))의 자기-일치라 타 정의 방법의 심판이 될 수 없음 — "다른 baseline들이 계산한 기여도로 **똑같은 실험**을 했을 때 우리가 더 잘한다"를 보여야 기여도 정의의 실효 우열이 증명됨(다운스트림 = 게임-무관 중립 심판). sign-게이트 경쟁도 **불공정이 아님**: 0의 의미론(zero-semantics)은 기여도 품질의 일부이므로, 0을 다르게 정의한 방법의 오발화는 그 방법의 실측 감점(②·③의 제외 근거를 경쟁 실험에선 승격). → **Track H 점수원 경쟁**(스펙·예측표 = §2 P10, `runs/track_h/README.md`; 점수원 8종 × 정책 4종[sign±크기가중·mult·z] × 시점 2종[online/retrain] × 차이-무대) — **Tier 1(CNN)+Tier 2(LLM) 실행 완료 = §3.2.3**; LLM 경쟁 무대의 본판은 **R4 gsm50k5(accuracy 심판) — Tier A seed0 첫 실측 = §3.2.4**(전 8종 경쟁은 Tier B 승인 게이트). 이로써 온라인 사분면 공백(track_g 점수원 3종뿐)은 8종으로 채워졌고, retrain 사분면도 T2(관찰자→재학습)가 8종 커버 — removal(§4.4, 전 방법·사후 제거)과 상보.
>
> **[07-22 Yonghee 결정 — 이 절의 LLM 무대 방향]**: ① LLM selection→성능 실험의 심판은 **GSM8K(exact-match)로 확정** — alpaca val-loss·MMLU·ROUGE는 성능 차이를 보여주기 어렵다고 판단(§3.2.1의 parity도 이 변별력 한계와 겹침). **첫 실측(Tier A seed0) = §3.2.4**. ② **소규모 cohort(silo5류, 클라 수 N=5급) selection 세팅은 이 축에서 더 진행하지 않음** — (07-22 후속 확인) '소규모'는 **cohort 규모**를 지칭하며 부분참여가 아님: **R4(N=50·5/50)는 해당 없음, 사용 확정**. ③ 그 귀결로 **std50k5(alpaca 부분참여) selection 계열 = 미사용 예정**(Track H Tier 3·R2, track_g std50k5-mixed 파일럿 — 결과 논문 비게재, rundir 존속). std50k5의 *fidelity* 결과(§4.2 부분참여 probe)는 별개 축이라 **유지 확정**(07-22 후속 — 지양은 selection 축 한정).
...
- **selection** `softmax(s/T)`로 k명 **선택** 샘플링(비복원). **S-FedAvg 관행** (`flirds_sel`; cohort가 진부분집합인 std20만 — CNN 쪽 `flirds_select`는 C2 개입 표 미사용에 포함, §6.2-14).
...
- 포함 arm = base/vanilla/flirds_w/shapleyfl_w/fedif_w (+ std20만 flirds_sel). **flirds_sel 제외@anchor5** = 전원 참여라 선택이 무의미(degenerate) ─ *적용규칙: 참여형태*. arm은 valuation 비교가 아니라 *개입 효과* 측정. **개별 결과**: 위 MMLU/ROUGE 표는 (scale × stage) 6 셀로 *개별* 수록(pool 아님).

---

### 3.2.2 LLM φ-게이팅 + V3 재학습 (Track G Phase B; `track_g/rundirs`, LLM 218 rundir)

> 전제가 된 Stage 0 부호 감사 = §5.2. ⚠ **경로 정정**: LLM rundir는 `runs/track_g/rundirs/`(218개)이며 별도 `rundirs_llm/` 폴더는 존재하지 않는다.

**(a) 세팅**: silo5 {clean, noisy(nr1.0), frrand, frzero}×3-seed + iid5 {clean, frzero}×3-seed + silo5 noisy-nr0.75 seed0 (+std50k5-mixed 8런 — **미사용 예정**[Yonghee 07-22, §3.2 서두 결정 ③; rundir 존속, 상세 git 이력]) — 전 셀 arms + per-round `phi_rounds.parquet`(프로젝트 최초 per-round φ 영속). arm = sign-게이트 2종(V1/V2; V2=burn-in·probation 포함; **per-update × 양수-only**), z-게이트(cohort-상대), lossheur/oracleb 게이트 대조, soft 가중 `flirds_w`, `oracle_excl`/`random_excl` 상·하한, **V3**(vanilla 완주 후 게이트 판정 kept로 init부터 재학습 = **retrain × 양수-only**: sign/z/random). recovery = (vanilla−arm)/(vanilla−oracle_excl).

**(b) 결과 — final val-loss ↓, arm×셀, 3-seed mean±std** (llm_summary.csv 재집계; silo5 noisy-nr0.75는 seed0뿐이라 생략, std50k5-mixed는 미사용 예정이라 생략 — §3.2 서두 ③)
...
- **비용**: silo5 게이팅 4셀 = seed0 기준 셀당 1.73–2.29 GPU-h(합 7.77; **3-seed 전체 25.2 GPU-h** + nr0.75 12런 2.3).

**(c) 출처·baseline-set 노트**
- **출처(정본)**: `runs/track_g/analysis/{README.md,llm_summary.csv}`(rundir-only 재생성; 스팟 대조 3셀 rundir 일치 확인) · rundir = `runs/track_g/rundirs/`.
- arm은 valuation 방법 비교가 아니라 **같은 V2 게이트 정책에 점수원을 교체한 통제 실험** — flirds / **loss-heur**(경쟁 점수원) / **(b)oracle**(정책 천장, silo5만) + oracle_excl·random_excl 상·하한 + v3_random(재학습 통제). **제외**: GTG/FedSV/ComFedSV 게이트 — 개입 정책이 원 논문에 없고, GTG/FedSV는 frzero exact-0이 아니어서(coalition-renorm, §5.2 판정 2) sign-게이트 발화가 값-오차 부산물이 됨 ─ *적용규칙: 점수원 통제 설계*. 이 제외는 *게이트 정책 실효성* 질문용이고, *점수원 경쟁* 질문에선 **§3.2.3(Track H)이 renorm 4종 게이트를 포함해 실측**(오발화 = 그 방법의 실측 감점, Yonghee 07-19). arm당 FL 학습 전체 1회 = 비용 선형(§3.2 서두 ①).

### 3.2.3 Track H 점수원 경쟁 — 같은 개입 정책에서 어느 기여도 정의가 학습을 잘 만드나 (`track_h/rundirs_cnn` 96런 + `rundirs_llm` 12런)

> §3.2 서두 [07-19 Yonghee 판정]의 실행 — §3.2.2(Track G)가 *점수원-통제*였다면 여기는 **점수원-경쟁**: 같은 정책·같은 무대·같은 seed에 **점수원만 교체**. 스펙·예측표(H-1~7 사전 등록) = `runs/track_h/README.md`. **Tier 1(CNN)+Tier 2(LLM) 완주(실패 0)**; **R2·Tier 3 std50k5 = 미사용 예정**(§3.2 서두 07-22 결정 ③ — 결과 논문 비게재) — **LLM 경쟁 무대는 R4 gsm50k5(§3.2.4, Tier A seed0 완주)**. 정책 확증 후속 런(P5·Scale·Dyn) = **§4.8**.

**(a) 세팅**: 점수원 8종 = Flirds / Flirds-1st / loss-heur / GTG / FedSV / ComFedSV / ShapleyFL(β0.3 un-normalized raw) / FedIF — 전부 contribution orientation(도움=양수) 통일, per-round in-run 채점(`fl/score_providers.py`; ComFedSV는 per-round 대용치[균등평균 submodel + loss-감소 효용, 논문 Eq.6] — Yonghee 승인 caveat). 정책 4종 = **P1** sign-게이트(cum>0 참여·n-가중, V2) / **P2** sign+크기가중(w∝n·max(cum,0)) / **P3** soft 곱셈가중(EMA min-max — 부호 파괴) / **P4** z-게이트(cohort-상대) × 시점 2종 = **T1** online / **T2** retrain(**관찰자 런** 1회에 8 점수원 동시 부착[vanilla와 비트동일 궤적] → 최종 누적 부호로 kept 결정 → init부터 재학습; kept-set 동일 시 재학습 공유, kept=전원이면 vanilla와 동일 처리). 무대 = **R1** CNN cifar10 dir1 {clean, grad-noise, free-rider, label-flip@0.70}×3-seed(Flirds arm·통제 = `track_g/rundirs_cnn` 재사용 — 그리드 문서 절은 미사용 삭제, §6.2-14) / **R3** silo5 noisy nr1.0×3-seed(renorm 4종만 신규; Flirds·loss-heur·(b) = §3.2.2 재사용) (R2 std50k5는 미사용 예정 — 아래 (b3)). **판정 = 학습 성능만**(탐지 AUROC류 없음): 각 셀에서 vanilla(바닥)~oracle_excl(천장) 사이 절대 성능(CNN acc / LLM val-loss)으로 직접 비교. 분석 정본 = `runs/track_h/analysis/`(CSV엔 정규화 지표[셀 간 합산용]도 있으나 본 절 표는 **절대값만** — Yonghee 2026-07-20 지시).
...
- **clean(오발화)**: vanilla .6389가 천장. flirds1st .638·fedif .639는 발화 0(천장 유지)이나 **flirds .632·loss-heur .626은 소폭 하락**(clean 일부 오배제 — LLM 무발화 §3.2.2과 대비), renorm은 .60~.605로 가장 큰 오발화. P3/P4(부호 파괴·상대 게이트)는 전원 clean .638~.644로 안전하나 GN 회복도 약함(.34~.52).
...
**(b1′) strmain stage — 경계 label-flip(rate~U(.5,1)) 경쟁 확장** (`track_h/rundirs_cnn` strmain 24런; bd15fa1): dir1 strmain에 8 점수원 × 정책 × 시점을 추가 — 강도응답 상단(경계 클라)에서 점수원-회복 서열이 dir1@0.70과 **동형 재현**(score_corrupt_recovery, 3-seed):

| 정책·시점 | 1위 | estimator 4종 | renorm 4종 |
|---|---|---|---|
| P1 online | **flirds +0.72** | flirds1st .64 · fedif .62 · lossheur .52 | gtg .08 · shapleyfl .01 · comfedsv −.09 · fedsv −.32 |
| P1 retrain | **flirds +0.98** | lossheur .98 · flirds1st .92 · fedif .91 | gtg .69 · shapleyfl .64 · comfedsv .62 · fedsv .60 |
| P2 retrain | fedif +0.96 | flirds1st .95 · flirds .91 · lossheur .89 | shapleyfl .74 · gtg .67 · comfedsv .66 · fedsv .64 |

> 읽기: online서 renorm은 경계-flip에도 파국(fedsv **−0.32**)이나 **retrain서 renorm이 0.60~0.74로 회복** = dir1@0.70과 같은 "online 복리붕괴 ↔ retrain 완화" 구조. estimator 계열은 online·retrain 모두 상위 — 더 어려운 경계 클라 무대(strmain)에서도 아래 판정 ①(0-의미론이 게이트 실효성을 가름)이 유지. (regime별 skew 분해는 track_g CNN 그리드에 존재하나 §3.2.3 경쟁과 중복이라 미게재 — §6.2-14.)
...
| flirds·lossheur·oracleb_gate_v2 (§3.2.2 재사용) | 2.3340±.0471 | 발화 0(침묵) |
| flirds_w (soft 가중 참조, §3.2.2) | 2.3294±.0471 | – |
...
**(b3) R2 std50k5 mixed — 미사용 예정** (Yonghee 07-22, §3.2 서두 결정 ③): alpaca 부분참여 selection 무대 자체를 논문에서 쓰지 않기로 — seed0 파일럿 수치(H-3 어긋남 방향 관찰 포함)는 git 이력·rundir 존속. LLM 경쟁 판정은 R4(GSM8K exact-match, §3.2.4)가 담당.
...
| H-3 (R2 ShapleyFL ≤ random) | 판정 없음(무대 미사용) | R2 std50k5 미사용 예정(07-22) — seed0 관찰(어긋남 방향)은 git 이력; LLM 경쟁 판정은 R4로 이관 |
...
**판정(Tier 1+2)**: ① **0-의미론이 게이트 실효성을 가른다** — exact-0 계열(Flirds·Flirds-1st·loss-heur·FedIF)만 free-rider 무대 생존(acc .61~.62 ≈ 천장), renorm 4종은 clean 오배제+free-rider 방치로 붕괴(acc .37~.40 < vanilla .59; CNN 3-seed). ② 그 반대급부 = **위협-의존 트레이드오프**(LLM noisy선 renorm 발화가 이득·절대-0은 침묵) — "0을 어떻게 정의하느냐"가 *어느 위협에서 개입이 작동하는지*를 결정. ③ 계열 내 1위는 정책·시점 의존 — 정확한 클레임은 "**Flirds = 전 정책·전 시점 상위권 + grad-noise를 잡는 유일한 estimator(2차항: flirds GN .567~.607 vs flirds1st/fedif .244~.248)**"이지 단독 1위가 아님(clean·label-flip 등 개별 칸 최고는 FedIF·loss-heur도 차지). ④ clean 오발화 없이 완주는 FedIF·Flirds-1st뿐 — Flirds·loss-heur의 CNN clean 소폭 하락은 cum 0-교차 노이즈가 원인(H-2 MISS; LLM 무발화 §3.2.2과 대비되는 CNN 레짐 특성).

**(c) 출처·baseline-set 노트**
- **출처**: `runs/track_h/rundirs_cnn/`(**228** = 경쟁 96 + strmain 24 + P5 108)·`rundirs_llm/`(12런) + §3.2.2 계열 rundir 재사용(track_g) · `runs/track_h/analysis/{competition_score,cnn_competition,llm_competition,observer_zero_semantics}.csv`(**07-20 집계 정정판** — 수정 내역 3건은 `make_analysis.py` docstring; R2 표의 정본 = `llm_competition.csv`; strmain stage_cell = competition_score.csv, (b1′)). 코드 = `codes/flirds/fl/score_providers.py` + `experiments/track_c2.py`(관찰자·T2)·`track_g.py`(provider 확장) + `tests/test_track_h.py`.
- baseline-set = 점수원 8종 전부(경쟁 설계라 renorm 게이트 포함 — §3.2 서두 판정). 통제 = vanilla / oracle_excl / random_excl.

### 3.2.4 Track H R4 — GSM8K accuracy 무대 · Tier A seed0 + P5-soft (`track_h/rundirs_llm/gsm50k5_*`) — **◐ 1-seed 파일럿**

> §3.2 서두 [07-22 결정 ①]의 실행 — **LLM selection→성능 확정 무대의 첫 실측**(Tier A = 4c40e30 · P5-soft = 77ba500 + 49c402a + **마지막 frzero×fedif = 4fb02fb, 07-23 → 12/12 완주**). exact-match 정확도가 심판이라 §3.2.1(alpaca parity)·§3.2.3 (b2)(val-loss 4번째 자리)의 변별력 한계를 벗어난 무대. 위협 4셀 중 **gnoise는 무대 미성립(γ축 종결)으로 본 절 미수록** — 기록은 §6.3 검증-전용. **Tier B**(전 8종 점수원 경쟁, ~300–350 GPU-h) = Yonghee 승인 게이트, **Tier C** = 3-seed.

**(a) 세팅**: Llama-3.2-1B-**Instruct**, GSM8K(`openai/gsm8k` train 7,473) → **IID 균등 149문항/클라 × N=50**, 참여 5/50, R=200, 10 steps×b16, lr 1e-3, LoRA r16/α32(std50k5 FL 스펙 verbatim; 클라당 평균 참여 ~20회). val=200(공식 test에서 카브) · **심판 = 잔여 test 1,119 exact-match**(greedy, `#### 숫자` 추출; val-loss 병행). 위협 = 오염 클라 0–19 고정(**40%**, CNN R1 미러): clean / **noisy** = answer-swap@0.7(뒤섞인 풀이+오답 정답 = 현실적 mislabel) / **frzero**. 정책 2종 × 시점 2종: **P1** sign-게이트(cum>0) · **P5-soft** 확신-가중(`pweight`, Φ(t)-비례 soft 가중 — kept 의미론 없음) × **T1** online / **T2** 관찰자-최종-값 재학습(P1은 부호로 kept 결정·P5s는 가중 재학습; Tier A는 estimator 4점수원만 채점 — MC 점수원 비용 회피. 단 **T1 online arm은 flirds만**: 타 점수원 online은 Tier B 몫). 통제 = observer(=vanilla 비트동일)·oracle_excl(0–19 제외)·random_excl(랜덤 20 제외)·**t2_random**(flirds T2와 동일 kept-크기 무작위 = 순위정보 가치 분리). 스펙·사전등록 예측 = `runs/track_h/README.md` §1.6 + `p5/RUN_P5.md`.

**(b1) 무대 판정 — vanilla↔oracle_excl EM 갭**(selection 효과를 잴 공간이 있는가):

| threat | vanilla(observer) | oracle_excl | 갭 | 판정 |
|---|---|---|---|---|
| noisy(swap@.7) | .3342 | .3700 | **+3.6pt** | **성립** |
| frzero | .3601 | .3691 | +0.9pt | 소폭 성립 (⚠ 아래 EM 노이즈 바닥과 같은 자릿수) |
| clean | .3771 | – | – | 게이트 오발화 비용 측정용 |

> ⚠ **EM 측정 노이즈 바닥 ±0.5pt**: 같은 구성의 observer(=vanilla)를 두 번 돌린 값이 clean .3771↔.3727(5문항)·frzero .3601↔.3655(6문항)·noisy .3342↔.3351(1문항)로 갈린다(P5-soft leg가 observer를 재실행하며 생긴 dup rundir — caveat ④). **원인은 커널 비결정성이 아니라 H1 재현성-fix 경계**(07-23 `meta.json:git_sha` 확인): canonical observer 3개는 `fa5fc6e` = **H1 pre-fix**(LoRA-A init이 entropy-seeded = 프로세스마다 새 추첨), dup은 `262f67c` = **post-fix**(`seed_everything(0)` 고정, track_g.py:270) → 이 ±0.5pt는 **init 재추첨 2회의 실측 스프레드**다(§4.7 'run-인스턴스 재추첨 효과'·caveat 13①과 같은 현상). init을 고정한 뒤의 잔차는 훨씬 작다 — frzero에서 **kept 집합이 같은 두 post-fix 재학습이 1문항 차**(.3691↔.3700). ⟹ **noisy 무대(+3.6pt)는 재추첨 스프레드의 7배로 안전**하지만 **frzero(+0.9pt)는 2배**에 불과하고, 표 안의 0.5pt 미만 차이는 우열 판단 금지.

**(b2) GSM8K test EM 절대값 — 시점별, vanilla(바닥)~oracle_excl(천장) 사이 직접 비교**(seed0; §3.2.3과 같은 읽는 법 — 오염 열에서 vanilla보다 높으면 개입이 성능을 올린 것, clean 열은 vanilla가 천장):

**T1 online**(라운드마다 개입; Tier A는 flirds arm만 — 타 점수원 online은 Tier B)

| arm | clean | noisy(swap@.7) | frzero |
|---|---|---|---|
| vanilla (observer) | **.3771** | .3342 | .3601 |
| vanilla (observer 재실행 = **P5s 행의 기준선**) | .3727 | .3351 | .3655 |
| oracle_excl (천장) | – | .3700 | .3691 |
| random_excl (통제) | – | .3253 | .3575 |
| **flirds · P1 sign-게이트** | .3673 | .3530 | .3646 |
| flirds · P5-soft 가중 | .3735 | .3324 | **.3735** |

**T2 retrain**(관찰자 최종 φ로 kept/가중 결정 → init부터 재학습; 점수원 4종)
...
| vanilla (observer 재실행 = **P5s 행의 기준선**) | .3727 | .3351 | .3655 |
| oracle_excl (천장) | – | .3700 | .3691 |
| t2_random (동일예산 통제) | – | .3110 | .3566 |
| **flirds · P1 sign** | .3771 (=vanilla) | **.3584** | **.3691** |
| flirds1st · P1 sign | .3771 (=vanilla) | .3432 | .3691 |
| lossheur · P1 sign | .3771 (=vanilla) | .3548 | .3691 |
| fedif · P1 sign | .3771 (=vanilla) | .3432 | .3691 |
| flirds · P5-soft 가중 | .3735 | .3467 | .3691 |
| flirds1st · P5-soft 가중 | .3735 | .3423 | .3691 |
| lossheur · P5-soft 가중 | .3735 | .3503 | .3691 |
| fedif · P5-soft 가중 | .3727 (=재실행 vanilla) | **.3584** | .3700 |

**읽기(seed0 방향 판단):**
- **noisy T2(P1) = 첫 LLM-accuracy 점수원 우열**: **flirds .3584 > lossheur .3548 > flirds1st = fedif .3432**(kept 37/36/38/34) — 같은 정책에서 점수원만 바꾼 순위. 동일 예산 무작위 t2_random .3110은 vanilla(.3342)보다도 하락 — flirds와의 차 **+4.7pt = 순위정보의 가치**가 accuracy로 처음 실증. 노이즈 바닥(±0.5pt) 대조: **2차항 유무의 격차(flirds↔flirds1st 1.5pt)와 vs-random(+4.7pt)은 안전**, **flirds↔lossheur(0.36pt)는 미분리** — "flirds 1위"는 1-seed 방향 표기이고 확정은 Tier C. ⚠ **이 행은 H1 판본이 갈린다**(caveat ④): flirds만 pre-fix(init 재추첨), 경쟁 3종+t2_random은 post-fix로 서로 같은 init 공유 → flirds가 낀 비교는 재추첨 1회를 포함한다(1.5pt·4.7pt는 스프레드의 3~9배로 생존, 0.36pt는 이중 미분리). Tier C(L1) 재실행은 전부 post-fix라 해소.
- **P5-soft T2는 점수원 순위 역전**: 최고 = fedif .3584(P1의 flirds와 동값), flirds .3467은 lossheur .3503 아래 — **정책이 점수원 순위를 바꾼다**. 단 P5s 내부 상하위 간격은 좁다: fedif↔flirds1st 1.6pt는 노이즈 바닥 위지만 **flirds .3467↔flirds1st .3423(0.44pt)는 바닥 미만** = 미분리. **P5s 12런은 전부 post-fix 동일 판본**이라 이 계열 내부 비교는 판본 혼재가 없다(P1 계열과 다른 점). P1 vs P5s 공식 대조(HP-3/5/6)는 `make_analysis` 재생성 후.
- **frzero = H-9 완전 적중, 정책-무관**: P1-T2 4점수원 전부 kept=30(오염 20명 전원 배제 = oracle과 동일 집합) → **EM도 oracle 동값 .3691**. **P5-soft T2도 4점수원 전부 kept=30**(07-23 마지막 런 착지) — exact-0 φ는 soft 가중에서 weight 0으로 떨어져 hard 배제와 같은 집합이 된다. EM은 3종 .3691(kept-동일 dedupe로 재학습 1회 공유) + **fedif .3700**(dedupe 불발 = 독립 재학습) — 같은 kept·같은 판본의 독립 재학습 간 차가 **1문항**이라는 점이 위 노이즈 바닥의 하한 눈금이 된다. T1 게이트도 rounds-to-target(val 기준) flirds 122 ≈ oracle 123(vanilla 200); P5s online .3735는 oracle EM(.3691)마저 상회(재실행 vanilla .3655 대비 +0.8pt).
- **clean 오발화 비용**: T1 P1 게이트 −1.0pt(.3673; false-excl 140쌍) — CNN clean 소폭 하락(§3.2.3 판정 ④)의 LLM-accuracy 재현. **P1-T2는 4점수원 전부 kept=50 → vanilla와 동일**(최종-누적 부호는 clean서 전원 양수 = §5.2 전제의 accuracy 무대 재확인) — 온라인 중간 배제만 비용을 내고 사후 부호는 무해. **P5-soft의 clean 비용은 검출되지 않음**: 자기 계열 기준선(재실행 vanilla .3727) 대비 online .3735·T2 .3735/.3735/.3735/.3727 = **+0.1pt 이내**. 07-22 판(canonical observer .3771과 비교해 "−0.4pt 구조적 비용")은 **기준선 오배정이었고, 그 차는 vanilla 재실행 간 변동(5문항)** — clean t2 4런 착지로 정정.
- **시점·정책 비교**: P1은 T1 < T2(noisy .3530 vs .3584 · frzero .3646 vs .3691 — 관찰자-최종 부호가 온라인 부분 배제보다 정확; noisy T1 발화는 희소 P .78/R .05인데 T2 최종부호는 13/20 배제 = 온라인 경로의 0-교차가 늦음. 주의: 이 무대의 'noisy'=answer-swap=label-flip류라 silo5 alpaca-noisy[§5.2 판정 3]와 위협이 다름). online에선 P1 게이트 > P5-soft 가중(noisy .3530 vs .3324 ≈ vanilla = soft 무효과).
- **R-플라토 없음**(vanilla·oracle val-loss r100→200 지속 하락) → 사전등록 R-반감 룰 불발, Tier B/C도 R=200 유지.

**예측표 H-8·9·11 대조** (Tier A seed0 기준 — renorm 점수원 몫은 Tier B 대기; MISS 포함 그대로. H-10=grad-noise는 무대 미성립으로 본 절 미수록 — §6.3):

| # | 판정 | 근거 |
|---|---|---|
| H-8 (answer-swap: estimator 게이트 발화·회복) | 적중(방향; 크기는 점수원 의존) | 4점수원 전부 T2 배제 발생(kept 34~38)·EM 상승 — 단 크기가 갈림: flirds +2.4pt·lossheur +2.1pt vs 1차(flirds1st·fedif) +0.9pt. T1 온라인 발화는 희소(R .05, 그래도 +1.9pt). renorm의 CNN-lf 유사 예측은 Tier B서 판정 |
| H-9 (frzero: exact-0 완전 배제) | **완전 적중(정책-무관)** | P1-T2 4점수원 전부 kept=30(=oracle 집합) → EM oracle 동값(.3691); **P5-soft T2 4점수원도 동일**(weight 0 = 배제; EM .3691×3 + fedif .3700 = 1문항 차); renorm FR-붕괴의 LLM 재현 여부는 Tier B |
| H-11 (clean: LLM 무발화 유지) | **MISS(정직 보고)** | T1 온라인 게이트 false-excl 140쌍·−1.0pt = alpaca 무대의 무발화 전례가 GSM8K에선 깨짐. 단 **최종-누적** 부호는 전원 양수(T2 kept=50 = vanilla) — 예측 문구 중 '누적 φ 전원 양수'는 최종값 기준 적중이고, 온라인 중간 0-교차가 오발화의 원인 |

**(c) 출처·비용·caveat**
- **출처**: `runs/track_h/rundirs_llm/gsm50k5_{clean,noisy,frzero}_*`(**42 rundir** = Tier A 24 + P5-soft 18; gate·observer 런은 `phi_rounds.parquet` per-round φ 영속. gnoise 2 rundir·진단은 §6.3) · 정본 `runs/track_h/analysis/gsm50k5_tier_a.csv`(P5s 행은 make_analysis 재생성 대기 — 현재는 rundir `metrics.json` 직접 집계) · 스펙 `runs/track_h/README.md` §1.6 + `p5/RUN_P5.md` · 코드 스위치 = `T2_CSIGN`(P5-hard 스킵).
- **GPU-h**(rundir timing.json 합산; gnoise 8.4 제외): **Tier A 54.6**(clean 8.5 / noisy 24.1 / frzero 22.0) + **P5-soft 60.4**(20.5 / 20.3 / 19.6 — 마지막 frzero×fedif 2.65 포함) = **115.0**. frzero T2 런은 kept-동일 dedupe로 재학습 1회를 공유하며 timing이 여러 곳에 중복 기록(Tier A 실소요 ~15.8, 커밋 4c40e30 보고); noisy는 kept 상이(37/36/38/34)로 dedupe 불발 = 최장 셀. Tier B/C 비용 산정의 실측 기준 — P5s leg가 Tier A와 맞먹는 비용(정책 1개 추가 ≈ ×2)이라는 점이 Tier B 견적의 입력.
- **caveat**: ① **1-seed(seed0) 파일럿** — 수치는 방향 판단용, Tier C(3-seed) 전 소수점 셋째 자리 우열 확정 금지. ② baseline-set = estimator 4점수원·**T1 online arm은 flirds만**(Tier A 설계; renorm 4종·타 점수원 online은 Tier B). ③ 스펙-정합 **해소**(Yonghee 07-22 후속): '소규모 참여 지양'은 silo5류 소규모 cohort(클라 수 N=5급)를 지칭 — **R4(N=50·5/50)는 사용 확정, 스펙 유지**. ④ **observer 중복 rundir + H1 판본 경계 → EM 노이즈 바닥의 출처**: P5s의 t2 재실행이 observer를 해시-접미사 rundir로 별도 저장(원본 보존) — 위협 3셀 전부에 dup 존재(`*_observer_seed0_{bee42f0a,4a55c78a,1073ceb7}`), 값은 비트동일 아님(clean val .60219↔.60246·EM 5문항 / noisy .60882↔.60916·1문항 / frzero .61029↔.61030·6문항). **차이의 정체 = H1 재현성-fix 경계**(07-23 `meta.json:git_sha` 감사; fix 커밋 `8598cea`): canonical 계열은 pre-fix(`fa5fc6e`·`c953806d`·`088bb24f` = LoRA-A init 미고정), dup·P5s 계열은 post-fix(`262f67c` 이후 = 고정). **판본 분포**(42런): pre-fix 21 = observer 3·oracle_excl 2·random_excl 2·clean t2_sign 4·frzero t2_sign 4·frzero t2_random·**noisy t2_sign_flirds**·flirds_gate_v2 3 / post-fix 21 = noisy t2_sign 3종·noisy t2_random·flirds_pweight 3·P5s t2 12·dup observer 3(+gnoise observer 1은 §6.3). ⟹ **P5s arm의 `vanilla_target`이 dup을 가리키는 것은 단순 기록이 아니라 판본 정합**이며, **P5s 행은 dup을 기준선으로 읽어야 한다**((b1) 주 + 읽기 3번째 불릿). 표를 가로지르는 유일한 지점 = **(b2) noisy T2 P1 행**(flirds만 pre-fix — 읽기 1번째 불릿). `make_analysis`는 arm별 dup 중 1개 채택 — 분석 시 채택 rundir 확인. Tier C(L1)는 전부 post-fix 재실행이라 혼재 해소.
- **잔여**: (b2) 표는 **전 칸 착지**(⬚ 없음). 무GPU 후속 = `make_analysis` P1 vs P5s 표 + RUN_P5.md HP-3/5/6 대조(루트 REMAINING §1.1) · **차기 컨테이너 큐 L1·L2**(`runs/track_h/QUEUE_L1L2_2026-07-23.md`, 0c8b9aa): L1 = Tier C 3-seed 재실행(~120–138 GPU-h; T2 점수원별 프로세스 분할로 셀당 wall 13.7h→~8h), L2 = **R4 (b)-fidelity 셀**(§3.1 P12 — 이 무대에 처음으로 exact oracle을 붙임) · Tier B(전 8종) = 승인 게이트 · Tier C = 조건부(루트 REMAINING §1.5).
...
**(a) 세팅**: Llama-3.2-1B, **N=5 전원**, R=10, local 10 steps, batch 16, lr=1e-3, maxlen 768, train=200/val=20/test=40, warmup=2; 위협별 1명 오염(noisy=client0, free-rider=client1); **(b)=exact 2⁵**; 3 seeds. ⚠ 오염 셀 rundir는 **2026-07-20 β0.3 재실행판(ce0b454)이 canonical**(스위트에 ComFedSV 추가[Fed-LOO도 추가됐으나 논문 비게재 = caveat 15], timing.json 신설; 재실행 전 값은 git 이력).

**(b) 결과 — AUROC(corrupt=high-φ) + Spearman vs (b) + runtime, 3-seed mean±std**

| method | noisy AUROC ↑ | noisy Sp ↑ | frrand AUROC ↑ | frrand Sp ↑ | frzero AUROC ↑ | frzero Sp ↑ | runtime ↓ |
|---|---|---|---|---|---|---|---|
| Flirds | 1.000 | 1.000 | 1.000 | 1.000 | 1.000 | 1.000 | ~107s |
| Flirds-1st | 1.000 | 1.000 | 1.000 | 1.000 | 1.000 | 1.000 | ~35s |
| loss-heur | 1.000 | 1.000 | 1.000 | 1.000 | 1.000 | 1.000 | ~99s |
| FedIF | 1.000 | 0.933±.05 | 1.000 | 0.900±.08 | 1.000 | 0.900±.082 | ~37s |
| GTG | 1.000 | 1.000 | 1.000 | 1.000 | 1.000 | 1.000 | ~540s |
| FedSV | 1.000 | 1.000 | 1.000 | 0.967±.047 | 1.000 | 0.933±.047 | ~535s |
| ShapleyFL | 1.000 | 1.000 | 1.000 | 1.000 | 1.000 | 1.000 | ~530s |
| ComFedSV | 1.000 | 0.833±.170 | 1.000 | 0.867±.125 | 1.000 | 0.867±.125 | ~387s |
| (b)oracle | 1.000 | (truth) | 1.000 | (truth) | 1.000 | (truth) | ~530s |
| FLDetector | 0.750 | – | 1.000 | – | 0.750 | – | ~40–91s |
| STD-DAGMM | 0.250±.204 | – | 1.000 | – | 0.000±.000 | – | ~180–390s |
| FLTrust | 1.000 | – | 1.000 | – | 1.000 | – | ~37s |
| FedDQC | 0.917±.12 | – | 0.750 | – | 0.750 | – | ~22s |

> **읽기**: noisy·free-rider 위협에선 valuation 거의 전부 AUROC 1.0 + Spearman 0.87~1.0(near-additive) — same-game 방법(Flirds/Flirds-1st/loss-heur)은 전 칸 1.000, coalition 계열은 Sp에서만 소폭 이탈(FedSV frzero 0.933, ComFedSV 0.83~0.87). 전용 탐지기 중 FLTrust만 전 위협 1.0; STD-DAGMM(model-free)은 noisy 0.25·frzero 0.00으로 약함.
...
- 포함 13 = valuation 8(Flirds/Flirds-1st/loss-heur/FedIF/GTG/FedSV/ShapleyFL/ComFedSV) + (b)oracle + 탐지기 4(FLDetector/STD-DAGMM/FLTrust/FedDQC). 모든 method가 모든 위협서 동작("category-together"). ComFedSV는 β0.3 재실행 스위트에서 추가(재실행 전엔 partial-participation 전용 사유로 제외였음 — silo full에서도 실측되며 Sp 0.83~0.87 하위권). rundir엔 Fed-LOO 열도 있으나 논문 비게재로 표에서 삭제(caveat 15).

### 3.3.2 Robustness cross-device N=100 α-sweep + anchor (`phase2_matrix/1B_device100-a*`)
...
**(b1) noisy 위협, detection AUROC ↑** (열=α; 3-seed mean±std). ⚠ **α=0.1·α=0.01 열 = β0.3 재실행판 정본**(a0.1 = c05a951[noisy·frrand] + 49c402a[frzero], 07-22~23 · **a0.01 = 2d3f482**[noisy·frrand, 07-23] — H1 seeding 후 새 궤적 + 거리 metric 추가; **a0.01 frzero는 미재실행 = 구 실행**. 나머지 α 열도 구 실행 = 재실행 큐, §6.2 caveat 9. 재실행 전후 차는 **AUROC 기준 φ-method ≤.004·탐지기 ≤.052**[최대 = a0.01 noisy STD-DAGMM .652→.600], Spearman은 FedIF만 ≤.017 = 결론 불변):

| method | α=0.0 | α=0.01 | α=0.1 | α=0.5 (anchor) | α=5.0 |
|---|---|---|---|---|---|
| (b)oracle | – | – | – | 0.604±.041 | – |
| Flirds | 0.774±.058 | 0.572±.056 | 0.606±.056 | 0.604±.041 | 0.596±.039 |
| Flirds-1st | 0.772±.058 | 0.571±.056 | 0.607±.057 | 0.605±.042 | 0.597±.038 |
| loss-heur | 0.772±.058 | 0.571±.056 | 0.609±.055 | 0.605±.042 | 0.597±.038 |
| FedIF | 0.973±.017 | 0.570±.106 | 0.692±.126 | 0.830±.085 | 0.973±.022 |
| GTG | – | – | – | 0.734±.112 | – |
| FedSV | – | – | – | 0.708±.142 | – |
| ShapleyFL | – | – | – | 0.762±.095 | – |
| ComFedSV | 0.442±.115 | 0.438±.063 | 0.430±.027 | 0.371±.028 | 0.396±.002 |
| FLDetector | 0.535±.048 | 0.499±.078 | 0.528±.068 | 0.539±.055 | 0.532±.058 |
| STD-DAGMM | 0.856±.037 | 0.600±.153 | 0.615±.103 | 0.671±.142 | 0.760±.040 |
| FLTrust | 1.000 | 0.592±.107 | 0.726±.129 | 0.854±.090 | 0.994±.008 |
...
> **frrand α=0.1(β0.3 재실행판 c05a951; 표 밖 열)**: Flirds/Flirds-1st/loss-heur/FLTrust 1.000 · FedIF 0.989±.003 · FLDetector 0.571±.057 · FedDQC 0.532±.220 · STD-DAGMM 0.512±.316 · ComFedSV 0.431±.041 — 위 (b2) 패턴 그대로(rundir metrics.json 직접 집계).
> **frzero α=0.1(β0.3 재실행판 49c402a, 07-23; 표 밖 열)**: Flirds/Flirds-1st/loss-heur/FLTrust 1.000 · FedIF 0.991±.003 · **STD-DAGMM 0.945±.031**(frrand 0.512±.316 대비 안정 — zero-delta는 model-free 탐지기에도 쉬운 신호) · FedDQC 0.532±.220(frrand와 동값 = 데이터-품질 스코어가 delta 종류에 무감) · FLDetector 0.530±.067 · ComFedSV 0.442±.052.
> **frrand α=0.01(β0.3 재실행판 2d3f482, 07-23; 표 밖 열)**: Flirds/Flirds-1st/loss-heur/FLTrust 1.000 · FedIF 0.989±.004 · **STD-DAGMM 0.892±.078** · FLDetector 0.581±.036 · FedDQC 0.554±.105 · ComFedSV 0.461±.072 — a0.1과 같은 패턴이 α 한 단계 아래서도 유지(재실행 전 대비 φ-method 0.000·STD-DAGMM −0.033·ComFedSV +0.023).
...
| FedIF | 0.721±.027 | 0.827±.022 | 0.824±.017 | α별 0.61~0.83 (재실행 a0.1: noisy 0.702±.013·frrand 0.818±.009·frzero 0.818±.006 / 재실행 a0.01: noisy 0.616±.094·frrand 0.756±.030) |
...
**(c) 출처·baseline-set 노트**
- **출처**: `master_metrics.csv` (02_device100_sweep); **β0.3 재실행 5셀(a0.1 noisy·frrand·frzero + a0.01 noisy·frrand)은 rundir `metrics.json` 직접 집계**(c05a951 + 49c402a + 2d3f482; master_metrics는 gitignored 파생이라 재생성 필요할 수 있음) — 다섯 셀 모두 `timing.json` 신설(a0.1 **4.22 / 4.52 / 4.78 GPU-h**[client-training 6,811/7,277/7,802s + valuation 8,397/8,990/9,416s] · a0.01 **5.18 / 5.01 GPU-h**[7,569/7,928s + 11,061/10,121s] — §6.3. a0.01 셀이 a0.1보다 ~0.5–1.0 GPU-h 비싼 몫은 전부 valuation 위상 = 탐지기 수렴시간[a0.01 noisy: STD-DAGMM 2,992s·FedDQC 2,863s 3-seed 합 vs a0.1 1,996/1,414s]). anchor cell runtime: (b)perround ≈25,000s, GTG ≈16,700–19,800s(seed별 16,667/17,947/19,832; mean ≈18,100), FedSV ≈4,970s, ShapleyFL ≈24,900s, Flirds ≈157s, Flirds-1st ≈53s(재실행 5셀도 Flirds ~149–170s·Flirds-1st ~50–60s로 정합).
...
**(a) 세팅**: Llama-3.2-3B, N=5 full, R=10, batch 8; **seeds=[0]만**(3-seed ⬚=계획 P5; β0.3 재실행 잔여 18셀에 3B silo5 셀 포함 — 루트 REMAINING §1.2); (b)=exact 2⁵.

**(b) 결과 (1 seed)**

| method | noisy AUROC ↑ | noisy Sp ↑ | frrand AUROC ↑ | frzero AUROC ↑ | runtime(noisy) ↓ |
|---|---|---|---|---|---|
| Flirds | 1.000 | 1.000 | 1.000 | 1.000 | ~251s |
| Flirds-1st | 1.000 | 1.000 | 1.000 | 1.000 | ~82s |
| loss-heur | 1.000 | 1.000 | 1.000 | 1.000 | ~384s |
| FedIF | 1.000 | 0.600 | 1.000 | 1.000 | ~82s |
| (b)oracle | 1.000 | (truth) | 1.000 | 1.000 | ~1244s |
| FLDetector | 1.000 | – | 1.000 | 1.000 | ~133–382s |
| STD-DAGMM | 0.250 | – | 1.000 | 0.000 | ~206–745s |
| FLTrust | 1.000 | – | 1.000 | 1.000 | ~83–91s |
| FedDQC | 1.000 | – | 0.750 | 0.750 | ~46–50s |

> **3B(1-seed)**: noisy·free-rider(random/zero)에서 valuation 전부 AUROC 1.0 + Flirds/Flirds-1st/loss-heur Sp 1.000 = 1B silo5(§3.3.1)와 동일 구조가 스케일에서 유지. 예외 = STD-DAGMM(noisy 0.25·frzero 0.00)·FedDQC(frrand/frzero 0.75)·FedIF Sp 0.600.
...
| GTG | 0.333±.118 | 1.000±.000 | | **STD-DAGMM** | **1.000±.000** | – |
| FedSV / ShapleyFL | 0.333 / 0.250 | 0.933 / 0.967 | | FedDQC | 0.750±.000 | – |

> **읽기 — "φ=0 exact는 zero/random 한정"의 실측 확정**: Flirds는 Spearman +1.000(전 seed)으로 oracle을 완벽 추종하는데 AUROC가 낮다(0.25/0.25/0.50). 그런데 **(b)oracle 자신의 AUROC가 seed별로 정확히 같다** — 직전 글로벌 delta는 val-loss를 실제로 낮추는 방향이라 **val-loss 게임의 정직한 답이 "이 클라는 기여함"**((b) 저장-φ 전 클라 음수 −0.0017~−0.0052 = 기여 방향 전원 양수, free-rider가 중간 순위에 섞임; Flirds φ와 소수 6자리 일치 cos_d≈6e-9[seed0]). 즉 낮은 AUROC는 추정 실패가 아니라 **게임 자체의 성질**(루트 위계 2차-③ "기여도≠탐지"의 정확한 사례). 탐지 자체는 update-패턴 탐지기의 몫 — **STD-DAGMM 1.0**(복제 delta의 이상 패턴 탐지); 반면 FedIF·FLDetector·FLTrust는 **0.0**(free-rider를 최고 가치로 오정렬 = 방향 정렬 기반 방법들이 완전히 속음).

**(c) 출처·baseline-set 노트**: §3.3.1과 동일 규약(rundir엔 Fed-LOO 열도 있으나 논문 비게재 = caveat 15). 단일 rundir에 3-seed 동거(`freerider_delta_seed{0,1,2}` 키; `make_analysis.py` 미반영 → rundir 직접 집계). **출처**: `runs/phase2_matrix/rundirs_2026-07/1B_silo5_frdelta/{config.yaml,metrics.json,phi.parquet}` (커밋 5ed9b9e); 위협 구현 = `codes/flirds/data/corruptors.py` `free_rider(mode="delta")`.

### 3.3.5 CNN 탐지 AUROC (`track_c` C1 — §3.1.2와 같은 셀; C2는 arm-level 주석만)
...
| regime | Flirds | Flirds-1st | loss-heur | (b) |
|---|---|---|---|---|
| silo (N5·R10) | 10 HVP | 10 grad | 60 fwd | 320 fwd |
| anchor (N5·R30) | 30 HVP | 30 grad | 180 fwd | 960 fwd |
| device (N100·R30·K10) | 30 HVP | 30 grad | 330 fwd | **30,720 fwd** |
...
- **Robustness** (§3.3 표에 병기): N=5 silo5 — Flirds-1st ~35s / Flirds ~107s / loss-heur ~99s(post-fix) / (b)·coalition ~530s / 탐지기 22~390s. N=100 anchor — Flirds-1st ~53s / Flirds ~157s vs (b)perround **~25,000s** / GTG ~16.7–19.8k s(mean ~18.1k) / ShapleyFL ~24.9k s / FedSV ~4970s.
...
> - **grad-noise 개입**: Track H에서 Flirds GN acc .567~.607 vs **Flirds-1st/FedIF .244~.248 = vanilla 수준 실명**(§3.2.3) — 1차 정보만으론 noise 클라가 안 보임.
> - **물리 근사**: 2차 잔차가 1차의 ~1/3(2.7–3.4× 개선; §5.5).
> - **비용 대가**: Flirds-1st가 전 무대 최저가(§3.4.2) — 2차항의 가치는 위 세 무대(부분참여 fidelity·grad-noise 개입·물리 근사)에서만 비용을 정당화.
...
- 방법 스위트 = §3.1.1과 동형. truth=(b) only(ORACLE_A=0).
...
> → **개입 효과 크기는 A축(폭·참여)이 아니라 B축(오염)이 지배**. 폭·참여는 동작점(raw acc·속도)만 옮기고 parity↔이득 이분법과 갭(~0.09)은 안 바꾼다. track_c C2 오염 무대(rundir; 개입 표는 미사용 §6.2-14)와 정합.

**(c) 출처·baseline-set 노트 (4.3 공통)**
- **출처**: `runs/probe_signal/cnn_c1/pc1_*/metrics.json` + `runs/probe_signal/cnn_c2/pc2_*/metrics.json` + 재사용 = `runs/track_c/c1/cifar10_{iid,label-flip}_seed*` · `runs/track_c/c2/cifar10_iid_{clean,label-flip}_strmain_seed*`(C2 rundir와 값 일치 교차검증). **정식 재생성 = `runs/probe_signal/make_figures.py`**(rundir만으로 재현).
- C1 probe 포함 = §3.1.2와 동일 8종 + (b); truth = (b) only((a) retrain은 probe 스코프 밖 — ORACLE_A=0). C2 probe 포함 6 arm(C2의 flirds_repl/flirds_add는 dir1 size-skew 전용 → iid probe엔 없음). detector 별도 없음(개입 arm의 φ-as-detector AUROC만; truth = corrupt 마스크).

## 4.4 Removal-curve — 게임-무관 인과 검증 (`removal_dose` A2·A3)
...
- 흥미: **FedIF 역전** — LLM frzero 서 유일 낙오(§4.4.1)였으나 CNN mnist 에선 분리 최고(+0.0042). 척도의 스테이지-의존성 자체가 관찰 결과. (cifar10 label_flip 최고 분리는 비교군 제외 방법이 차지 — caveat 15.)
- 잔여 없음: label/quantity_skew 확장 옵션은 **추가하지 않기로 확정**(Yonghee 2026-07-22; README §Exp A3의 옵션 항목 폐기 — A3는 현행 18셀로 완결).

**(c) 출처·baseline-set 노트 (4.4 공통)**: `runs/removal_dose/rundirs/`(A2) · `runs/removal_dose/rundirs_cnn/`(A3). LLM removal 지표는 val_loss 뿐(생성 LM) → accuracy 축은 A3 CNN 에서 실측. distinct 곡선 카운트는 문서 편의상 11-method 표기 유지(현 비교군 기준으로도 카운트 재현 확인). C-# = 2026-07 내부 리뷰 항목 코드(원 문서 `review-claude`는 정리 커밋으로 삭제 — git 이력).

## 4.5 Dose-response (`removal_dose` B; 3-seed 확정) — φ 탐지 문턱 vs 오염강도

**(a) 세팅**: silo5 무대 × 위협 강도 ladder {noisy nr, free-rider 배율 dm} × 3-seed (B 세트 63셀 중 noisy·free-rider ladder만 수록 — 나머지 ladder는 위협-스코프 제외, §6.2 caveat 8).
...
- **1B_silo5 오염 셀 = β0.3 재실행 완주·착지**(ce0b454, 07-20) — §3.3.1이 그 정본. 부수 관찰: 재실행에서 seed-민감 지표(예 §3.1.5 frzero xseed ρ 1.000→0.933)가 이동 = β 효과라기보다 **run-인스턴스 재추첨 효과**(adapter init unseeded — §6.2 caveat 13 H1)와 구분 불가.
- **device100 5셀 완주**(a0.1 noisy·frrand c05a951 → a0.1 frzero 49c402a → **a0.01 noisy·frrand 2d3f482**) 후 **07-23 02:46 드레인 종료** — poison 3셀은 영구 제외, 잔여 10셀은 보류(다음 캠페인 L1·L2 뒤). a0.01 재실행 전후 차도 a0.1과 같은 크기(φ-method ≤.004·탐지기 ≤.052 = 결론 불변) → **"β0.5→0.3이 결과를 바꾸나"의 답은 5셀 모두 '아니오'**. 상태·부작용 상세 = §6.2 caveat 9.
...
> §3.2.3 경쟁 본판에서 파생된 **사전등록(preregistered) 정책 확증 런들** — 예측표를 먼저 등록하고 MISS를 그대로 보고. GPU-h 수치는 서버 로그 기준(track_h CNN rundir엔 timing.json 없음).
...
**(b) 결과 — P5-hard(cgate) · online / retrain** (절대 test acc, 3-seed mean; 형식 = §3.2.3 P1/P2 표와 동일)
...
**총평**: Flirds는 **1차(fidelity)에서 측정 가능한 전 무대 oracle-동률 최상위**이고, 비용 우위는 **라운드당 참여(cohort)가 큰 무대에서만** 성립하며, 실질 약점은 **IID-clean 무대(잴 신호 자체 부재 = 무대 결함)** 와 frdelta류 "기여도≠탐지" 축(게임 공통 한계)이다.
...
- 강건성·확장: A축 lever(rank·lr·steps) 전반 1.000 유지 = Taylor tradeoff 없음(§4.2) · N=10 exact 2¹⁰ 확장도 동률(§3.1.3) · Taylor 물리잔차 2차<1차 ~3×(§5.5).
...
- 약세: clean-IID 이득은 원리적 부재(효과 < 표본 SE); grad_noise 일부 칸 shapleyfl(0.645) 우위.
- **승(Track G)**: 온라인 부호-게이팅 — **frzero 자동배제 recovery 1.000**(LLM silo5·iid5 3-seed, 오배제 0 = φ=0 공리의 배포형 활용)·clean 게이트 무발화(max|Δ|=0.00056)·CNN grad-noise 회수 0.86–0.94(LLM 게이팅 = §3.2.2 · CNN 근거는 경쟁 무대 §3.2.3으로 일원화 — §6.2-14).
- **약세(Track G)**: noisy는 sign-게이트 작동영역 없음(누적 φ 양수, 예측 적중 — 회수는 탐지+selection 몫)·frrand는 부호 코인플립으로 seed-의존·**CNN clean에서 게이트 오발화**(경쟁 실측 §3.2.3: flirds .632 < vanilla .639 — LLM 무발화와 대비되는 CNN 레짐 특성).
- **승(Track H 점수원 경쟁)**: 같은 게이트 정책에 점수원 8종을 넣은 경쟁(§3.2.3)에서 **exact-0 계열(Flirds·Flirds-1st·loss-heur·FedIF)만 CNN 오염 무대 생존**(free-rider acc .61~.62 ≈ 천장 .620) vs coalition-renorm 4종(GTG/FedSV/ComFedSV/ShapleyFL)은 **free-rider서 파국**(acc .37~.40 < vanilla .59; FR은 못 잡고 clean만 오배제)·clean도 .60~.605 오발화 — zero-semantics 결함이 성능 감점으로 실측. grad-noise는 **1차 estimator(Flirds-1st/FedIF) 실명**(acc .244~.248 = vanilla vs Flirds .567~.607) = 2차항 존재 이유의 다운스트림 재현.
- **약세·트레이드오프(Track H)**: 개별 칸 최고는 점수원마다 갈림(clean·label-flip은 FedIF·loss-heur도 상위) — Flirds는 전 정책·전 시점 상위권이나 단독 1위 아님; **LLM silo5-noisy는 역전** — renorm 게이트가 (renorm-오차) 0-교차 덕에 발화해 val-loss 2.3308(<vanilla 2.3340), Flirds τ-게이트는 침묵(=vanilla, 절대-0 의미론의 위협-의존 트레이드오프)(§3.2.3).
- **승(R4 GSM8K 첫 LLM-accuracy 실측, seed0 ◐)**: answer-swap 무대(vanilla .3342↔oracle .3700)에서 T2 점수원 우열 **flirds .3584 > loss-heur .3548 > 1차 estimator(flirds1st·fedif) .3432**, 동일예산 무작위 .3110 대비 **+4.7pt = 순위정보의 가치**; frzero는 4점수원 전부 kept=30(=oracle 집합) → EM oracle 동값(H-9 적중). 단 clean에서 T1 온라인 게이트 −1.0pt 오발화(T2는 kept=50 무해) — 1-seed 파일럿, Tier B/C 대기(§3.2.4). ⚠ **EM 노이즈 바닥 ±0.5pt**(= H1 fix 전후 observer 2회 = init 재추첨 스프레드의 실측)이므로 안전한 주장은 vs-random(+4.7pt)·vs-1차(1.5pt)까지고, **flirds↔loss-heur(0.36pt)는 아직 미분리** — 게다가 noisy T2 행은 flirds만 pre-fix 판본이라 그 비교에 재추첨 1회가 섞여 있다(§3.2.4 caveat ④; Tier C 재실행서 해소).
...
- **패지만 게임 공통(Flirds 책임 아님)**: **frdelta는 (b)oracle과 seed별 완전 동일 0.33**(§3.3.4) — 루트 위계 "기여도≠탐지"의 실측 사례, 회수는 update-패턴 탐지기 몫(STD-DAGMM 1.0).
...
| 오염 무대 개입(성능) | **승** (GN·FR서 천장 근접, clean 소폭 오발화) | §3.2.3 절대 acc · §3.2.4 R4 T2 · §4.4.2 분리 인과 |
| 점수원 경쟁(같은 정책·Track H) | **계열-수준 승** (exact-0 계열 생존 acc≈천장·renorm free-rider 붕괴 .37~.40; 계열 내 1위는 정책·위협 의존 + LLM noisy 역전) · **R4 GSM8K seed0: T2 flirds 1위**(◐) | §3.2.3 · §3.2.4 |
| IID-clean 무대 | **무정보** (전원 동률·타깃 불안정) | §5.4 ρ −0.37 · §3.1.5 B축 |
| frdelta | 패지만 **게임 공통** | §3.3.4 ((b)oracle 동일 실패) |
...
| 관점 | 최고 세팅 | 수치 |
|---|---|---|
| **최대 절대 이득** | **CNN cifar10 dir1 grad-noise × sign-게이트(P1)** | 절대 acc **.5668**(online)/**.6065**(retrain) — vanilla .2436 → 천장 .6203 대비 **회수 0.86/0.96**(§3.2.3; estimator 중 Flirds만 회복, 1차 계열 .244~.248 실명) |
| **가장 깨끗한 완전 회수** | **LLM frzero × `flirds_gate_v2`** (silo5·iid5, 3-seed) | recovery **1.000 정확**(오배제 0쌍, oracle_excl과 최종손실 소수 4자리 동일; §3.2.2) — φ=0 null-player 공리가 그대로 배포 성능으로; parameter-free(τ=0, k 없음). retrain 대응물 `v3_sign`도 1.000 = **계산 시점 축(③)과 무관하게 성립** |
| **점수원 경쟁 (같은 정책)** | **Track H CNN dir1** | exact-0 계열이 free-rider acc .61~.62(≈천장) vs renorm 4종 .37~.40(<vanilla .59) = zero-semantics 실측 감점; grad-noise는 Flirds(2차)만 .567~.607 vs Flirds-1st/FedIF .244~.248(1차 실명). 역전 무대 = LLM noisy(renorm val-loss 2.3308 < vanilla 2.3340 vs 절대-0 침묵) (§3.2.3). **LLM accuracy 첫 실측(R4 seed0 ◐)**: answer-swap서 T2 flirds .3584 1위·동일예산 무작위 대비 +4.7pt (§3.2.4) |
...
| clean 무해성 (do-no-harm) | LLM alpaca 무대 | 게이트 무발화, 게이트·V3 arm max Δ최종손실 0.00056 (§3.2.2) + track_d MMLU/ROUGE parity (§3.2.1). **예외 = CNN clean 게이트 오발화**(경쟁 실측: Flirds .632·loss-heur .626 < vanilla .639 vs FedIF/Flirds-1st .638~.639 통과, renorm .60~.605 §3.2.3; P5 정책이 대부분 회수 §4.8.1) **+ R4 GSM8K clean T1 온라인 게이트 −1.0pt**(.3673, false-excl 140쌍; T2 사후-부호는 kept=50 무해 — seed0 §3.2.4) |

**서사 한 줄**: 신호가 존재하는 곳(비IID·오염·부분참여)에서는 exact oracle 동률의 fidelity를 5~160× 싸게 달성하고 그 순위는 removal로 인과 검증된다; 한계는 IID-clean 무대(잴 신호 자체가 없음 — 전 semivalue 공통)와 delta-재활용 free-rider처럼 게임의 정직한 답이 탐지와 어긋나는 축(기여도≠탐지)이다.

**보조 증거(링크)**: 사후 removal-재학습 — worst-first 제거가 val-loss↓(§4.4.1)·cifar10 acc 분리 +0.039~0.045(§4.4.2) = 순위→성능 인과의 게임-무관 확인.

## 5.2 φ 부호 감사 (Track G Stage 0) — 게이팅 전제 확정

> Track G(φ 부호-게이팅 실효성; 스펙 `runs/track_g/README.md`) 두 단계 중 **Stage 0**. Phase B 게이팅 본실험 = §3.2.2(LLM; CNN 몫은 07-22 미사용 결정 = §6.2-14). **Stage 0** = 재실행 0, 기존 **309개 rundir 전수**의 φ 부호 감사(`phi_sign_audit.py` → `audit/SIGN_AUDIT.md` + `sign_table.csv` 73,288행; 부호 규약 = contribution orientation, 도움=양수)로 예측표 확정·수정(커밋 6623fdf). 판정 2·3의 성능 귀결은 Track H 경쟁이 실측(§3.2.3).
...
2. **frzero exact-0**: (b)oracle·Flirds·Flirds-1st·loss-heur·FedIF가 free-rider(zero)에 **bit-exact 0.0**(전건 100%) — strict `cum>0` 규칙이 정확 포착. coalition-renorm 계열(ComFedSV·FedSV·GTG)은 exact-0 아님(작은 음수).
3. **[예측 수정 ①] noisy엔 sign-게이트 작동영역 없음**: silo5 noisy dose ladder(nr 0→1)에서 Flirds φ 전 구간 양수(+0.00242→+0.00181), 0-교차 선형외삽 **nr≈3.44 = 도달불가**((b) 3.44·Flirds-1st 3.42·loss-heur 3.41 동일 구조) → noisy 회수는 sign-게이트가 아니라 z-게이트/V2w 몫. GTG(~0.76)·FedSV(~0.65)만 nr∈(0,1]서 교차 = **coalition-renorm 값오차의 부산물**(진짜 게임값의 0-교차 아님).
...
**(a) 세팅**: Llama-3.2-1B, **N=5 full**, K=3, 오염 주입(noisy=client0 answer-swap / free-rider=client1 zero-update), per_domain train=12000·val=200·test=2000, local 10 steps. **full** = R=50, lr∈{1e-3, 3e-3} × 3 seed, oracle_b off. 메트릭: AUROC(noisy/free-rider) + selection(K=3 keep). (top-k 재학습 arm은 미사용 — §6.2-14.)
...
5. **track_c C2 pool 주의(기록)**: C2 관련 그룹 통계는 partition·강도·dataset을 threat 내에서 pool한 값이라 std가 크고 절대 수준이 높아 보인다(쉬운 조합 혼합) — C2 개입 표 자체는 미사용(caveat 14). 셀별 30칸 = `RESULTS.txt`; C2 arm 탐지 AUROC 주석 = §3.3.5.
...
- **R4 gnoise γ축 negative result** (`rundirs_llm/gsm50k5_gnoise_*` 2 rundir + `rundirs_llm_gn20/` 1 + 진단 `runs/track_h/gnoise_diag/`; 커밋 77ba500, §3.2.4 스코프에서 제외 — Yonghee 07-22): 등방 LoRA-delta 가우시안 노이즈는 γ={1.0(상대), 5, 20(GN_ABS)} 전부 GSM8K 무대 미성립 — γ5 vanilla EM .3753 > oracle .3735(부호 반대)·γ20 .3718(목표 밴드 밖). 진단(forward-only 섭동 γ1~1000×3방식×2스냅샷) = gradient-방향 섭동만 2,400~38,000배 민감·등방 응답 ≈0(γ=1000도 ‖Ξ‖/‖W₀‖ 1.3% = INT8 수준); Fang(USENIX Sec'20) 가우시안=음성 대조군·LoRASC(EMNLP'24) 같은 축을 성능향상 레시피로 사용과 정합; γ\*=5 선정근거(train_loss 판독)는 주입 시점상 무효. **H-10은 CNN(§3.2.3 GN)으로만 서술(LLM leg 공백)**; 위협 교체(LIE/sign-flip, I-28) 미결. 구정의 rundir 폐기 560a2fd·`llm_competition.csv` 구정의 행 잔존(재생성 시 소멸).
...
- ~~baseline 수치 ↔ 원 논문 대조~~ · ~~deferred-rigor 인벤토리~~: 2026-07-23 삭제(구식, 4399be3) — git 이력에서 조회
...
- **Flirds 원리 분석(승패 메커니즘 → 원리 10건 → 세팅 제안; 독립·비판적 재분석)**: [[flirds-principle-analysis]] (2026-07-22, ae1c063; 수치 검증 산출물 = `flirds-principle-analysis/{verify_numbers.py,verification_report.txt}`)
- 잔여 작업 정본: 루트 `REMAINING.md`(재현성 재실행 P0 스코프 = §1.4 · 차기 캠페인 = §1.6) · 차기 컨테이너 큐: `runs/track_h/QUEUE_L1L2_2026-07-23.md` · CNN fidelity leg 사전등록: `runs/track_c/c2fid/README.md`
...
| `removal_dose` | rundirs 75 (A2 12 + B 63) + rundirs_cnn 18 + rundirs_trackd 6 (A1 3 + D 3) | §4.4(A2·A3) · §4.5(B) · §4.6(A1·D) | 전부 3-seed; A2·B의 poison 셀/ladder는 위협-스코프 제외로 미수록(§6.2-8) |
| `track_g` | rundirs 218 (LLM; std50k5 8런 포함) + rundirs_cnn **67**(구 36 + **skew 그리드 착지분 31**) + rundirs_cnn_v3 12 + audit 파생(309 rundir 스캔, 73,288행) | §3.2.2(LLM 게이팅) · §5.2(감사) · 신규 31 = **P14(미수록·인용 금지)** | `rundirs_llm/` 폴더는 부재(경로 정정); **CNN 그리드+V2w·V3 = 미사용**(07-22, §6.2-14 — rundir는 §3.2.3 R1이 재사용); std50k5 8런 = 미사용(07-22, §3.2 서두 ③); skew 31런은 gskew 90 중 중간 스냅샷(48734fc — 완주 후 재생성본이 canon) |
| `track_h` | rundirs_cnn **204**(경쟁 96 + P5 108; strmain 51런 제출·미착지) + rundirs_llm **56**(silo5 12 + **gsm50k5 44**) + rundirs_llm_gn20 1 + gnoise_diag(파생) + rundirs_cnn_scale **21**(12+앵커 9) + rundirs_cnn_dyn 9 | §3.2.3(경쟁 Tier1·2) · **§3.2.4(R4 Tier A+P5s)** · §4.8(P5·Scale·Dyn) | R4 = **Tier A seed0 + P5-soft 12/12 완주**(4fb02fb, 07-23 — (b2) 표 전 칸 착지) — §3.2.4; **gnoise γ축 종결 = §6.3**(negative result); R2·Tier 3 std50k5 = 미사용 예정(07-22); 차기 큐 = `QUEUE_L1L2_2026-07-23.md`(L1 Tier C · L2 = P12) |
...
| **계획·미실행** | – | P2–P6(수치 ⬚) · **P12–P14 신규(⬚/◐)** | P1 ◐((b) 2¹⁰ seed0=§3.1.3; (a)·seeds1-2 = 미진행 확정 07-22)·P7–P9 완료(§4.4–4.6)·P10 ◐(§3.2.3)·**P11 ◐(Tier A+gnoise 종결+P5s 완주=§3.2.4; Tier B/C ⬚)**; **P12**(R4 (b)-fidelity, 큐 등재)·**P13**(CNN c2fid 144셀, 파일럿 제출)·**P14**(CNN downstream 확장, 31/195 착지) |
