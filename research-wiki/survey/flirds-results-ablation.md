...
> **세팅**: cifar10/iid **N=10 full · R=10 · 오염 4/10** · φ 순위대로 worst-first(나쁜 클라부터) vs best-first 제거 후 재학습 → acc 분리. 게임-무관 인과 검증. *↔ LLM 짝*
> **지표** = **곡선 평균 분리** `mean(worst-first acc − best-first acc)` (10점 곡선 전 구간 평균). 높을수록 순위가 인과적, **음수면 순위가 반-인과**.
...
> **읽기**: ① **위협마다 갈리는 방법이 다르다** — grad-noise에선 **Flirds-1st만 −0.0189로 부호가 뒤집히고**(순위가 반-인과), frzero에선 **renorm 계열이 무너진다**(ShapleyFL **−0.1626** · ComFedSV +0.0385 · FedSV +0.1050). lf@0.70은 전 방법 +0.14대로 포화라 변별이 없다. ② **Flirds는 세 위협 전부에서 (b)oracle과 사실상 동값**(Δ≤0.0013) — 게임-무관 심판에서도 오라클-동행. ③ 오염평균 1위 FedIF +0.2026 · 2위 loss-heur +0.2014 · Flirds +0.2001은 3자 차이가 0.25pt 이내로, **평균만 보면 변별이 안 되고 위협별로 봐야 한다**. **출처**: `runs/removal_dose/rundirs_cnn/cifar10_iid-*`.
> 레거시 시나리오 {label_flip 사다리, feature_noise}는 §0.1 오염축 밖이라 표에서 제외했다(rundir는 존속; 종전 값 = 각각 곡선 평균 분리 +0.0445 · +0.0385).
