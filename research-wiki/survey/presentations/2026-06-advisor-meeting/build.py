...
────────────────────────────────────────────────────────────────────
2026-06-12 갱신: silo5 재실행본·device100 α-sweep 12셀·3B 4-threat 결과 반영
(소스: runs/phase2_matrix/rundirs/*/metrics.json, 커밋 8d364cc — 20/25 셀).

⚠ TODO-UPDATE — 잔여 5셀 확정 시 갱신할 지점 (이 파일에서 "TODO-UPDATE" 검색):
  [1] S3 표 N=100 행 + S6 anchor 줄 : α=0.5 anchor 3셀(oracle 동반) 완료 시 잔여 문구 교체
  [2] S5/S6                        : device100 poison 2셀(dev_a0.5/a0.0_poison) 완료 시 결과 추가
  [3] S3/S6                        : 7B tier 완료 시 ⓒ→ⓑ 갱신
...
<div class="conv">
  ...
  <span class="ph">기울임 회색</span> = 잔여 셀 확정 대기(device100 poison 2셀 · α=0.5 anchor 3셀 · 7B) — 확정 시 갱신.
</div>
"""))
...
<tr><td>기존 방법 대비 무엇이 나은가</td><td>같은 frozen 궤적 위 9-method 비교 (+FedIF)</td><td><span class="lbl">ⓑ</span> phase1 + silo5 재실행 3-seed (FedIF 포함)</td></tr>
<tr><td>위협을 식별하는가</td><td>2 regime × 4 threat + matched detector 4종</td><td><span class="lbl">ⓑ</span> silo5·3B·device100 sweep · <span class="ph">poison 2셀 잔여</span></td></tr>
<tr><td>N=100에서 성립하는가</td><td>per-round exact 분해 + α-sweep</td><td>sweep 12셀 <span class="lbl">ⓑ</span> 3-seed · <span class="ph">α=0.5 anchor 3셀 잔여</span></td></tr>
<tr><td>실용 가치가 있는가</td><td>selection run (top-k 재학습)</td><td><span class="lbl">ⓑ</span> 3-seed, 양 lr</td></tr>
<tr><td>scale에서 유지되는가</td><td>1B → 3B → 7B ladder</td><td>1B <span class="lbl">ⓑ</span> · 3B 4-threat <span class="lbl">ⓑ</span>(1-seed) · 7B <span class="lbl">ⓒ</span></td></tr>
...
<ul>
  <li class="tight"><span class="lbl">ⓑ</span> phase1(1B N=5 silo5 3-seed, lr 1e-3·3e-3): noisy/free-rider에서 <b>전 방법 Spearman +1.000 동률</b>(N=5 near-additive) → 주장은 "더 정확"이 아니라 <b>"같은 랭킹을 더 싸게"</b>. silo5 재실행본(3-seed, 8d364cc)도 동률 재확인.</li>
  <li class="tight">런타임 <span class="lbl">ⓑ</span>: Flirds-1st 35s · FedIF 35s · Flirds 107s · loss-heur 164s · GTG/FedSV/Banzhaf/ShapleyFL/(b) oracle ~530s · Ripple ~4515s(별도 세션; eigsh flaky로 자동 비교에서 분리).</li>
  <li class="tight">free-rider φ exact-0: Flirds·oracle·Banzhaf·loss-heur. GTG·FedSV는 within-subset renorm으로 ≠0. FedIF(정규화 1차) <span class="lbl">ⓑ</span>: noisy/FR AUROC 1.0, Spearman 0.90–0.93(silo5 재실행 3-seed).</li>
</ul>
...
<div class="bt">결과 — silo5 4-threat 3-seed 재실행본 <span style="font-weight:400">ⓑ</span> <span class="muted" style="font-weight:400">(FedIF 포함 · run-dir 영속화, 8d364cc)</span></div>
<ul>
  <li class="tight">noisy / free-rider: valuation 전 방법 AUROC 1.0; Spearman 동률(예외: FedIF 0.90–0.93 · FedSV FR-random 0.933±0.047).</li>
  <li class="tight"><b>poison(ASR=1.00 전 seed, working-backdoor config) = 동률의 첫 붕괴</b> — <b>raw 1차(Flirds-1st)만 AUROC 0.000 완전 회피</b>. Flirds-2차 0.917±0.118(seed별 0.75/1.0/1.0; 단 이전 동일 config run은 0.417±0.425 — run간 비결정성, 원인 규명 중). 정규화 1차(FedIF·FLTrust)와 exact 계열(oracle·loss-heur·Banzhaf·ShapleyFL)은 AUROC 1.0; Spearman은 GTG +0.867 · FedSV +0.367로 열화.</li>
  <li class="tight">detector: poison — FLDetector·FLTrust·FedDQC 1.0, STD-DAGMM 0.75±0.204 · noisy — FedDQC 0.917±0.118, FLDetector 0.75(off-threat), STD-DAGMM 0.417±0.312 · FR-zero — STD-DAGMM 0.250±0.204 <b>실패</b>(matched threat 실패, 진단 필요), FedDQC 0.75 · FR-random — STD-DAGMM·FLDetector 1.0.</li>
</ul>
<div class="box">현재 데이터 기준 정리: Flirds는 noisy + free-rider를 잡는다. clean-preserving backdoor는 raw 1차가 일관 회피하고, 2차는 1B서 0.917±0.118(run 분산)·3B(1-seed)서는 0.000 — 정규화 1차(FedIF·FLTrust)·loss-heur·FLDetector·FedDQC가 보완(전부 1.0). <span class="muted">(headline 채택은 논의 항목 ①)</span></div>
"""))
...
<ul>
  ...
  <li class="tight">α-sweep 12셀 <span class="lbl">ⓑ</span>(α∈{0, 0.01, 0.1, 5.0} × noisy/FR-zero/FR-random, 3-seed): free-rider — Flirds·Flirds-1st·loss-heur·FLTrust 전 α <b>AUROC 1.000</b>(FedIF 0.98–0.99) · noisy — cheap valuation 0.57–0.77로 약함, matched FedDQC 0.96–1.0이 보완 · 1차=2차=loss-heur 랭킹 동일(proxy-truth Sp 1.000) · ComFedSV 0.39–0.45 실패, STD-DAGMM 0.51–0.96 불안정.</li>
  <li class="tight">anchor: Flirds vs per-round oracle Spearman +1.000 <span class="lbl">ⓑ</span>(1B, 1-seed smoke) · free-rider φ=0 exact 유지 · <span class="ph">α=0.5 anchor 3셀(oracle 동반) + device100 poison 2셀 잔여.</span></li>
</ul>
<div class="bt">scale ladder</div>
<ul>
  <li class="tight">1B <span class="lbl">ⓑ</span> 완료 · 3B 4-threat <span class="lbl">ⓑ</span>(1-seed, (b)oracle 동반): noisy/FR valuation 전 방법 AUROC 1.0(detector 예외: STD-DAGMM noisy 0.25·FR-zero 0.00·FR-random 1.0, FedDQC FR 0.75), Flirds vs (b) Sp +1.000(FedIF 0.600) — <b>poison(ASR=1.0)은 Flirds 1차·2차 모두 AUROC 0.000 회피</b>(loss-heur·FedIF·FLTrust·FLDetector·FedDQC는 1.0) · 7B <span class="lbl">ⓒ</span>(경로 구현됨, 미실행). 전부 fp32.</li>
</ul>
...
<div>
  ...
  <ul>
    <li class="tight"><b>Taylor 절단</b> — clean-preserving backdoor의 γ-update가 clean val-loss를 내림 → tangent는 most-helpful로 오판, exact secant는 잡음. 2차 회복은 1B서 부분적(run 분산)·3B(1-seed)서는 부재. 전개반경 진단 코드 부재.</li>
...
<div>
  ...
  <ul>
    ...
    <li class="tight"><b>N=100 noisy 약함</b> — cheap valuation 0.57–0.77(전 α, 3-seed); matched FedDQC 0.96–1.0이 보완.</li>
    <li class="tight"><b>1-seed 항목</b> — 3B(dual-oracle +0.900 · 4-threat tier); α=0.5 anchor 미실행; FedDQC per-domain IRA 분석.</li>
...
<ul>
  <li class="tight">완료: silo5 재실행 · device100 sweep 12셀 · 3B 4-threat (20/25셀, 8d364cc). 잔여: device100 poison 2셀 + α=0.5 anchor 3셀 + 7B. poison은 working-backdoor config 별도 invocation.</li>
</ul>
<div class="bt">무비용 분석 (기실행 데이터 재분석)</div>
<ul>
  <li class="tight"><b>poison Flirds-2차 거동 규명</b> — 1B run간 분산(0.417↔0.917) + 3B(1-seed) 완전 회피. per-round φ 분해 재분석(비용 ~0). 2차항 novelty의 사활처 — <b>최우선</b>.</li>
