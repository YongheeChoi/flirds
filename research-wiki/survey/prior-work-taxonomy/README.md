...
> **자매 서베이**(taxonomy 밖, 이 지형을 우리 실험에 적용): [[flirds-experiment-axis-map]](실험 축-지도 + 축별 결과 페이지) · [[baseline-selection-audit-2026-07-02]](**baseline 선정 정당성 감사** — 이 taxonomy를 근거로 실험별 baseline을 A/B/C 판정; aggregation-first 분류가 C-제외 논거) · [[baseline-original-paper-verification]](baseline 수치 ↔ 원 논문 대조).
