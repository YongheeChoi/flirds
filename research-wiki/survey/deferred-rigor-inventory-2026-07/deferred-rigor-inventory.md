...
| I-26 | Track D real run 미완 + 결정 3건 대기 | 1-seed 파일럿→3-seed 2-레짐→3B/7B 미완(루트 CLAUDE.md); 결정 3건(bridge arm=I-20/FlowerTune 채점=I-24/ShapleyFL β=I-50) 대기 | Yonghee 검토 대기(TRACK_D_REVIEW; 루트 CLAUDE.md next) | 2차-①(성능)·②(수렴 rounds-to-target) 축 결과 자체가 미산출 | 1B ≈40–60 GPU-h(루트 CLAUDE.md); 신호크기 probe 판정 후 착수가 합리적(I-03/I-24 연동) | P3-3 |
...
| I-50 | ShapleyFL β0.3 재실행 잔여 | 우리 β=0.5 vs 논문 β=0.3 발견(log:526) → 재실행 캠페인 진행 중; 7B track_d·phase2_matrix 반영 대기(ov:789 Caveat 9) | 원문 대조에서 발견된 수치 오류 교정(진행 중) | β 미통일 상태의 7B·robustness 표 수치는 곧 바뀔 값 | 캠페인 완료 대기(재집계는 rundir-only) | P5-7 |
...
- **패키지 3 — 진행 중 완주 (신규 비용 0)**: probe 스위트 seeds1–2(≈40 GPU-h 기계획; I-10), probe C(I-07), CNN width×참여 grid 제출(I-10), β0.3 재집계(I-50).
- **패키지 4 — 중기**: §2.4 clean×non-IID 매트릭스 본실행(I-03), E-sweep 최소판(I-04; 항목 1 세션과 병합), Track D real run(I-26), 3B seed 완성(I-14).
