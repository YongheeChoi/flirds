...
SILO_M = ["(b)oracle","Flirds","Flirds1st","FedIF","GTG","FedSV","ShapleyFL","Banzhaf","loss-heur","FLDetector","STD-DAGMM","FLTrust","FedDQC"]
SILO_AUROC = {
 "(b)oracle":(1.000,1.000,1.000,1.000),"Flirds":(1.000,1.000,1.000,0.917),"Flirds1st":(1.000,1.000,1.000,0.000),
...
SILO_RT = {"(b)oracle":532,"Flirds":106,"Flirds1st":35,"FedIF":35,"GTG":538,"FedSV":533,"ShapleyFL":530,
...
M3B = ["(b)oracle","Flirds","Flirds1st","FedIF","loss-heur","FLDetector","STD-DAGMM","FLTrust","FedDQC"]
M3B_AUROC = {"(b)oracle":(1.000,1.000,1.000,1.000),"Flirds":(1.000,1.000,1.000,0.000),"Flirds1st":(1.000,1.000,1.000,0.000),
...
M3B_RT = {"(b)oracle":1240,"Flirds":251,"Flirds1st":82,"FedIF":82,"loss-heur":384,"FLDetector":250,"STD-DAGMM":530,"FLTrust":85,"FedDQC":49}
...
notes(s, 0.55, 4.5, 12.2, [
 ...
 ("비교 대상 11종 + 탐지기 4종 / 듀얼 오라클: (a) 재학습-검증손실, (b) in-run 분해(exact 2ᴺ).", False, SUB),
], size=12.5, gap=7)
...
# ── S5 Track C 명세 ─────────────────────────────────────────────────
s = slide()
header(s, "Track C · 이미지 (CNN)", "실험 명세 — C1 fidelity / C2 개입")
rows = [["항목","C1 — fidelity","C2 — 개입(일반 성능·탐지)"],
 ...
 ["오라클","(b) exact 2¹⁰ + (a) 2¹⁰ 재학습","— (개입 결과로 평가)"],
...
footnote(s, "3 seed(0,1,2), 150셀 실패 0. (a)-오라클 efficiency-gap ≤1e-15 (전 셀). 환경: conda lora4cl, RTX 3090.")

# ── S6 C1 fidelity vs (b) ───────────────────────────────────────────
s = slide()
header(s, "Track C · 1차 fidelity", "C1 — (b) in-run 오라클 대비 (순위 Spearman, 3-seed 평균)")
...
footnote(s, "iid·feature_noise는 (b)오라클 자체 신호가 약함(클라 동질). 거리 지표는 S4 정의. 출처: runs/track_c/c1/*/metrics.json 재집계.")

# ── S7 C1 fidelity vs (a) ───────────────────────────────────────────
s = slide()
header(s, "Track C · 1차 fidelity", "C1 — (a) 2¹⁰-재학습 오라클 대비, 그리고 (b)와의 괴리")
rows = [["시나리오","Flirds (b)","Flirds (a)","Flirds1st (a)","GTG (a)","loss-heur (a)"]]
for ds in ("cifar10","mnist"):
    for sc in SCEN_ORDER:
...
notes(s, 8.45, 1.62, 4.3, [
 ("핵심 — 게임 정의 차이", True),
 "Flirds는 (b) in-run 게임을 거의 완벽 재현하나,",
 "label_skew에서 (b)와 (a)가 갈린다:",
 ("  CIFAR·label_skew : (b)+0.98 → (a)−0.18", False, ACCENT),
 ("  MNIST·label_skew : (b)+0.71 → (a)−0.29", False, ACCENT),
 "",
 "이는 Flirds 추정오차가 아니라 모든 in-run",
 "방법이 공유하는 한계. 희귀라벨 클라는 매 라운드",
 "한계기여는 낮아도 최종 재학습 모델엔 필수.",
 "(iid 음수는 (a)신호 자체가 미약 → 노이즈)",
], size=11, gap=4)
footnote(s, "(a) 열은 rho_a_mean(3-seed). 동일 시나리오에서 Track D anchor5(IID·clean LLM)는 (a)와 +1.000 일치 → 괴리는 이질성 구동(S14).")

# ── S8 C2 개입 ──────────────────────────────────────────────────────
s = slide()
header(s, "Track C · 2차-① 성능·③ 탐지", "C2 — 개입 결과 (CIFAR-10, dir1, 3-seed 평균)")
rows = [["arm"]]
cells = list(C2.keys())
...
# ── S9 Phase 2 명세 ─────────────────────────────────────────────────
s = slide()
header(s, "Phase 2 · LLM 위협매트릭스", "실험 명세")
rows = [["항목","silo5 (cross-silo)","device100 (cross-device)"],
 ...
 ["오라클","(b) exact 2⁵","(b) per-round exact (α=0.5 anchor만)"],
...
footnote(s, "LoRA 어댑터, attn=eager(전진모드 AD 위해). silo5/3B 3seed (3B는 1seed). device100 α별 1–3seed. off-anchor truth=Flirds proxy(α=0.5서 (b)와 +1.000 검증).")

# ── S10 silo5 결과 ──────────────────────────────────────────────────
s = slide()
header(s, "Phase 2 · silo5 N=5 (1B)", "탐지 AUROC · fidelity Spearman vs (b) · 런타임")
...
# ── S11 device100 결과 ──────────────────────────────────────────────
s = slide()
header(s, "Phase 2 · device100 N=100 (1B)", "α-sweep — 탐지 AUROC")
...
notes(s, 0.55, 5.1, 12.2, [
 ...
 ("        anchor α=0.5서 GTG +0.78·FedSV +0.75·ShapleyFL +0.58·FedIF +0.72 (vs (b) per-round exact).", False, INK),
 ("poison(install config): Flirds AUROC 1.0(both α), 1차도 α=0서 1.0 — silo5와 달리 cross-device에선 1차가 회피되지 않음.", True, ACCENT),
], size=11, gap=5)
footnote(s, "device100 φ는 한 번이라도 선택된 클라만 기록(K=10/R=30 → ~96–98/100). 출처: RESULTS.md.")

# ── S12 3B 스케일 ───────────────────────────────────────────────────
s = slide()
header(s, "Phase 2 · 스케일 (3B, silo5 N=5)", "Llama-3.2-3B — 탐지 AUROC · ρ vs (b) · 런타임 (seed 0)")
rows = [["방법","noisy","fr-rand","fr-zero","poison","ρ noisy","ρ poison","런타임"]]
for me in M3B:
...
notes(s, 0.55, 5.42, 12.2, [
 ("noisy·free-rider: Flirds ρ=+1.000 유지(1B→3B), 런타임 251s vs (b) 1240s (~5× 저렴).", True),
...
footnote(s, "3B는 seed 1개 → 잠정. 7B·N=10 (a)오라클은 설계상 deferred(ⓒ). 출처: RESULTS.md.")

# ── S13 Track D 명세 ────────────────────────────────────────────────
s = slide()
header(s, "Track D · LLM 표준무대 (IID·clean)", "실험 명세 — 오염축 제거, OpenFedLLM 표준 셋업")
rows = [["항목","std20 (주 무대)","anchor5 (듀얼 오라클)"],
...
footnote(s, "무대=OpenFedLLM run_sft.sh verbatim(deviation: SGD mom=0/lr1e-3 상수/r16/fp32). clean·IID → 개입은 do-no-harm parity가 기대값. 3 seed.")

# ── S14 Track D fidelity ────────────────────────────────────────────
s = slide()
header(s, "Track D · 1차 fidelity (IID·clean)", "std20 vs (b) · anchor5 vs (a) (3-seed 평균)")
def drow(F, me):
...
rows = [["std20 (N=20) vs (b)"]+hdr[1:]] + [drow(D_STD_F, m) for m in order if m in D_STD_F["spearman"]]
...
rows2 = [["anchor5 (N=5) vs (a)"]+hdr[1:]] + [drow(D_ANC_F, m) for m in order2 if m in D_ANC_F["spearman"]]
table(s, 7.05, 1.55, 6.25, rows2, [1.5,1.1,1,1,1,1], ["l"]+["c"]*5, size=9.6, row_h=0.355, zebra=True)
notes(s, 0.55, 5.5, 12.2, [
 ("Flirds: (b) ρ=+1.000(cosine_d 3.5e-7) · (a) ρ=+1.000 — IID·clean에선 (a)/(b) 게임이 일치 → S7의 괴리는 이질성 구동임을 확인.", True, ACCENT),
 ("2차항 효과: cosine_d Flirds 3.5e-7 vs Flirds-1st 4.4e-5 (~125× 타이트). ShapleyFL·FedIF·ComFedSV는 스케일서 fidelity 약함.", False, INK),
], size=11.5, gap=6)
footnote(s, "anchor5는 (a) 2⁵ 재학습 오라클 기준. 출처: runs/track_d/rundirs/*/metrics.json 재집계.")

# ── S15 Track D 성능·수렴 ───────────────────────────────────────────
s = slide()
header(s, "Track D · 2차-① 성능 ② 수렴", "개입 arm — MMLU · ROUGE-L · 수렴 (std20, 3-seed 평균)")
...
footnote(s, "rounds-to-target=검증손실 목표 도달 라운드(전 arm ~199/200). anchor5(N=5)도 동일 parity. 출처: 재집계.")

# ── S16 종합 1차 ────────────────────────────────────────────────────
s = slide()
header(s, "종합 · 1차 fidelity", "기여도 측정 정확성 — 트랙 횡단 요약")
rows = [["측면","결과","근거"],
 ["순위 vs (b)","Flirds ρ≈+1.000 (전 트랙). Banzhaf·loss-heur도 우수; MC-SV는 noisy","silo5·std20 +1.000 / 이미지 +0.71–1.00"],
 ["값-거리 vs (b)","2차항이 1차 대비 값오차 ~2× 축소; loss-heur은 순위만 비기고 크기는 빗나감","euclid_d: 이미지 ~0.5× · std20 cosine 125×"],
 ["순위 vs (a) 재학습","IID·clean은 +1.000 일치; label_skew(이질성)에서 (b)와 괴리(−)","anchor5 +1.000 / 이미지 label_skew −0.18~−0.29"],
 ["비용","Flirds 1 HVP/round; (b) exact의 5–15× 저렴, 동일 순위","silo5 106s vs 530s · 3B 251s vs 1240s"]]
table(s, 0.55, 1.55, 12.23, rows, [1.7,4.7,3.5], ["l","l","l"], size=11.2, row_h=0.62, accent_first=True)
notes(s, 0.55, 5.35, 12.2, [
 ("한 줄 요약", True),
 "Flirds는 (b) in-run 게임을 순위·값 모두에서 충실히, exact 대비 저비용으로 재현한다. 2차항의 고유 기여는",
 "순위가 아니라 값-크기 정밀도. (a) 재학습 게임과의 일치는 데이터 이질성에 의존 — 동질이면 일치, label_skew면 갈림.",
], size=12, gap=6)
footnote(s, "(a) 괴리는 Flirds 특정 오차가 아니라 모든 in-run 방법 공유. 상세 S6·S7·S14.")
...
rows = [["항목","상태","내용"],
 ["(a)/(b) 게임 괴리","ⓑ 실측","label_skew(이질성)서 in-run φ가 재학습 가치와 역상관. 모든 in-run 방법 공유. 프레이밍 필요."],
...
notes(s, 0.55, 5.55, 12.2, [
 ("다음 단계 (제안)", True),
 "  1차: (a)/(b) 괴리 규명(φ 직접 분석, 희귀라벨 가설) — 핵심질문 위계상 우선.",
