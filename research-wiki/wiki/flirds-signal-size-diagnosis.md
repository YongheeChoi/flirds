...
재계산 가능. 3B_std20 metrics는 진단 당시 working tree의 β=0.3 재실행본을 읽었으나
방법 공통 수치의 차이는 1e-3 이하(ShapleyFL·재실행 환경 차이만)로 결론 불변.
**β=0.3 재실행 분리 보존(2026-07-02 Yonghee 지시)**: 재실행 출력은 canonical rundir을
덮어쓰지 않고 `runs/*/rundirs_beta03/`(track_c는 `c{1,2}_beta03/`)에 저장 — 3B_std20
3-seed 분리 완료+원본 복원, 진행 중인 3B_anchor5는 `runs/rerun_beta03/preserve_beta03.sh`
가드가 완료 시 동일 처리, 캠페인 큐의 #PAUSED 셀들엔 RUNDIR_ROOT 리다이렉트 삽입.
