...
- **Implemented 2026-06-08** (`baselines/fltrust.py`, task 7e step 2): root = the server val set, **g0 = −∇_val(w_r)** (one gradient step; cosine is scale-free). This makes the trust signal `cos(Δw_i, −∇_val)` the **NORMALIZED Flirds first-order term** — "cosine ≈ Flirds-1st" is *exact*, so FLTrust is the AUXILIARY (not independent) free-rider baseline. **Design choice**: the DETECTOR uses the SIGNED cosine (corrupt high), NOT FLTrust's ReLU(cosine) — ReLU clips benign (<0) and free-rider (~0) both to 0, erasing the free-rider sign; the ReLU + magnitude-norm are FLTrust's robust-AGGREGATION gates and (cosine being scale-free) do not change the per-client ranking. Unit AUROC free-rider & poison = 1.0; **real 1B N=100 free-rider AUROC = 1.0** (the free-riders are the top-5 suspicious), Spearman vs Flirds-1st = +0.60 (both detect perfectly; normalization reorders the benign tail). Confirms the gradient is the decisive free-rider signal (vs model-free STD-DAGMM's 0.63). See [[flirds-implementation-plan]] §3.9.
