...
- **✅ REAL GRID COMPLETE 2026-06-15 — 25/25 cells, 0 failures** (raw [[raw/conversations/flirds/2026-06-15-phase2-grid-completion-analysis-tooling]]). The 06-09 LOCKED grid ran to completion exactly as designed (cost-tiered stage-gate; `runs/phase2_matrix/run_driver.sh` 4-GPU slot scheduler over `master_queue.txt`; `DRIVER DONE` 06-15 ~03:37). **5 categories = the master_queue stage taxonomy**: `01_silo5` (1B N=5, 4 threat × 3 seed, all methods + (b) exact 2⁵) · `02_device100_sweep` (α∈{0,0.01,0.1,5.0} × 3 threat = 12, cheap methods, Flirds proxy-truth) · `03_device100_poison` (α∈{0.5,0.0} = 2, D2b install config `LR=2e-3 R=60 EPOCHS=5 frac0.8`) · `04_device100_anchor` (α=0.5 + (b)-perround + coalition, 3 threat = 3) · `05_scale_3b` (3B silo5 seed0, (b) on, coalition off, 4). **anchor = the most expensive cells** (per cell ≈ 63h = 3 seed × [FL ~38min + valuation ~20.4h: (b)-perround + ShapleyFL + GTG + FedSV sequential, log-silent]) → finished last on GPU 1·2·3 after the other 22; queue comment "expensive, last". frzero (final cell) had a missing driver `done[ok]` line (raced the queue-drain) but its rundir (4 files + 3-seed metrics) + log `MATRIX DONE` confirm completion; grid-wide fail/CHECK/error/traceback = 0. **Every cell persisted to `runs/phase2_matrix/rundirs/<cell>/` (config.yaml + meta.json[git_sha/dirty/env_hash/versions] + phi.parquet + metrics.json) and committed** (`8d364cc` 20 → `b9113c4` 22 → `a755149` 25; author Yonghee, push pending). **rundirs/ = TRACKED** (small, immutable, the actual result → no method re-run ever needed for re-analysis); tier logs + RESULTS.md + analysis/ = IGNORED (derived).
- **Results-aggregation tooling NEW 2026-06-15 = `runs/phase2_matrix/make_analysis.py`** (~590 lines, re-runnable; committed `3a5c2a1`): wipes + rebuilds `analysis/` from rundirs only. `classify(cfg)` → the 5 categories above; Spearman reference = `(b)perround`/`(b)oracle` at anchor, `Flirds(proxy)` off-anchor (mirrors `phase2_matrix.py report()`'s truth selection); all stored φ vectors are higher=more-suspicious (sign-aligned at persist time → one AUROC axis, no per-method flip). Output: per-category `csv/` (metrics_long, auroc_table, spearman_vs_*, runtime_table, phi_long) + `charts/` (AUROC×threat, Spearman heatmap, runtime log-bar, φ z-heatmap, vs-α panels, φ-separation box+dots, poison per-cell, 1B-vs-3B scale pair, method×cell AUROC grid, runtime-vs-Spearman frontier) + `00_overview/` (master_metrics/master_phi/runs_inventory + heatmap_all + frontier); every figure carries a config-provenance footnote; `analysis/README.md` (Korean) lists taxonomy + pending cells + orientation/coverage notes + the regen command. Re-analysis of the whole grid = re-run this script over the committed rundirs (no GPU, no method re-run). The result numbers live in RESULTS.md + analysis/ only (per Yonghee "결과는 파일로, 채팅에 풀지 말 것").
- **NEXT after the grid = results analysis + writeup** following the core-question hierarchy ([[flirds#핵심 질문 위계 (locked 2026-06-12, Yonghee 명시)]]): 1st = fidelity (Spearman/Kendall/distance vs (a)/(b) oracle), 2nd = efficacy (general performance → convergence → detection, in that order). Then presentation deck + Track C/D real runs (D = implemented + smoke-green, real run not yet ⟪이후 완료: Track C 150셀 `edd5608` 06-15 · Track D 18 rundirs {1B,3B,7B}×{std20,anchor5}×3-seed 커밋 06-18→06-25⟫). 7B (Tier 3) + N=10 (a)/(b) oracle remain intentionally deferred (not in this queue).
...
### §3.11 Track C/D — 표준-세팅 비교 실험 (2026-06-12 설계 확정; 구현 세션 ①② 06-12/13 완료 — 아래 기록; real run도 이후 완료 C=06-15·D=06-18~25)
...
- **D LLM 표준-세팅** ⟪**SUPERSEDED 2026-06-13** — 아래 "구현 세션 ②" 재정의가 현행; 원설계(오염축 포함)는 기록용⟫ (전부 API-free): **D-메인** Alpaca-GPT4 20k **IID**, N=5 full(+N=20 2/round 옵션), answer_swap 50%(FedDQC convention) → **Spearman vs (b)**(1차 fidelity) + **φ-bottom 필터링 후 재학습→MMLU**(2차 성능; FedAvg-mixed 하한/clean-oracle 상한/random-q% 대조) + AUROC(탐지; 후순위) / **D-옵1** FedDQC Table-1 미러(FiQA·AQUA-RAT, N=5, 8k, 50% swap; GPT-judge 컬럼은 로컬 메트릭 대체) / **D-옵2** FedHDS 미러(Dolly-15k category-Dirichlet α∈{0.5,5} 200클라→held-out task Rouge-L; published 4-seed 앵커). 모델: 1B/3B=Llama-3.2-Instruct, **7B=meta-llama/Llama-2-7b-hf**(task8 원결정=FL-LLM 문헌 표준 모델 일치; HF 토큰 접근 확인).
...
- **개입 루프 코어 DONE (세션 계속)**:
  - **결정(Yonghee)**: 훅 시임 = **공유 코어**(`fl/server._fedavg_core`에 `select_fn`/`weights_fn` 옵션 추가, 기본 None=비트 동일 — CNN C2·LLM D 공용); EMA는 06-12 결정("min-max+EMA 필수, FedIF/ShapleyFL 그대로") 재확인 — min-max=음수 φ의 비음수 가중화 필수조건, EMA=round 노이즈 평활; 계수는 **knob `beta` 노출, 기본 0.5(당시 ShapleyFL 동일로 파악 — ⚠ 이후 정정: ShapleyFL 논문값 β=0.3, `runs/rerun_beta03` 통일 재실행)**.
...
**구현 세션 ② 진행 기록 (2026-06-13, Track D 세션 — D 전면 재정의·재구현; raw [[raw/conversations/flirds/2026-06-13-track-d-redesign-iid-clean]]):**

- **⚠ Track D 재정의(Yonghee)**: 위 원 D 설계의 **오염축(answer_swap 50%/AUROC/필터링 arm)·비IID 요소 전면 제거** — "일반적인 학습 상황(IID)에서 SV 계산, benchmark accuracy, 수렴 속도 측면에서 우리 방법론이 다른 방법론에 비해 얼마나 잘 하는지"; val/test도 IID. D-옵1(FedDQC 미러: 오염이 정당화였음)·D-옵2(FedHDS 미러: 비IID+OOD-eval이라 충돌) **제외**(빌더 코드 존치; FedHDS 무대는 후속 "비IID·clean 분리축" 실험 후보로 파킹). **같은 날 핵심 질문 위계 명시**(1차=fidelity, 2차=성능→수렴→탐지; [[flirds#핵심 질문 위계 (locked 2026-06-12, Yonghee 명시)]] + 루트 CLAUDE.md 새김 + 역전 표현 교정). **NEW 후속 설계 항목: 오염 탐지축↔비IID축 분리 실험**(현 LLM main은 둘 결합; CNN C2엔 clean×non-IID 셀 존재, LLM이 빈칸; 결합 설계가 있어 분리는 쉬움) — 별도 세션.
- **확정 설계(추천 전부 채택, "추천대로" 지시)**: 무대 = **OpenFedLLM run_sft.sh verbatim 미러**(로컬 클론 확인: alpaca-gpt4 **20k**=그들 dataset_sample 기본값, N=20·2/round·R=200·10steps×batch16·**seq512**·alpaca 템플릿·Llama-2-7b-hf=우리 7B와 동일). deviation caveat 3(SGD mom=0 lr1e-3 상수 vs AdamW 5e-5 cosine[FedIT-SGD 전례 인용]·LoRA r16/α32 vs r32/α64·fp32 vs 8bit). **레짐 2**: **std20**(문헌 표준형; (b)=per-round exact 2²/round) + **anchor5**(N=5 full R=30; exact (b) 2⁵+**(a)-retrain oracle**[val-loss·fp32·32 retrains; (a)+(b) 듀얼 GT=문헌 공백]+Banzhaf). **축①** 11-method fidelity(Spearman·Kendall·**Pearson**(값-수준)·GTG 거리 3종·wall-clock; Ripple 제외 관행; IID=near-tie 분해능 시험대 프레이밍) / **축②** 개입 arm 6종(base·vanilla·flirds_w 곱셈β0.5·flirds_sel[std20만]·shapleyfl_w β0.5·fedif_w β0.7)→MMLU full-test 0-shot+같은분포 Alpaca-test(1k) ROUGE-L; clean-IID 기대=do-no-harm parity 명시; removal curve 제외 / **축③** round별 val-loss 곡선(post-hoc)+rounds-to-target+wall-clock. 3 seeds.
- **직접 비교 가능성(Yonghee 요청 확인)**: caveat-free 직접 비교 셋업 **없음** — OpenFedLLM은 무대 동일하나 general 지표=GPT-judge(API 불가; 클론 evaluation/ 확인), FlowerTune(arXiv:2506.02961)은 alpaca-gpt4+MMLU 0-shot이나 설정 상이(4/20클라·R15·DoRA r32·AdamW)+생성-추출 채점 시사(Llama-3.2-1B 1.03%)→참조점 수준, fidelity축은 LLM-scale 선행 0(=novelty; 숫자-옆-숫자 무대는 C1 담당). 옵션 2 미구현·승인 대기: **bridge arm**(vanilla-AdamW 문헌 레시피 1회→optimizer 갭 수치화)·**FlowerTune-채점 모드**(생성-추출 MMLU).
- **구현(미커밋)**: `fl/llm_server.py` `select_fn/weights_fn` seam 통과(기본 None=비트동일) · `data/llm.py` `build_alpaca_iid` n_test 추가(같은분포 test, domain="alpaca") · `experiments/track_d.py` 전면 재작성. (a)=러너 내 `make_a_utility`(val-loss-only; ROUGE 생성 비용 회피). **훅 위생 발견**: SFTTrainer가 retrain/라운드마다 train모드+임베딩 훅 재등록→functorch HVP 차단(LLM 3-musts의 런타임 재발형)→`_guard`(온라인 점수기 매 호출 eval+훅 클리어)+(a) 직후 동일 처리(anchor5 스모크에서 재현→수정 확인).
- **검증 green**: gpt2 std20(11-method+arm5+persist; Flirds vs (b) +1.000) · gpt2 anchor5((a) 32-retrain 경로 green; Banzhaf 등장·flirds_sel full-참여 자동제외) · **1B anchor5 스모크**(tiny R=2; 결과 아래) · CNN guard green(+import-graph 증명). ComFedSV 저R 행렬완성 퇴화(NaN)는 task7d 기지 특성(R≥30 정상). 비용 추정(1B): 3-seed 2-레짐 ≈ 40–60 GPU-h; real run 전 1-seed 파일럿 권장. **검토용 전체 정리 = 루트 `TRACK_D_REVIEW_2026-06-13.md`**(Yonghee 검토 후 삭제 예정; bridge arm/FlowerTune-채점 결정 대기).
...
> Superseded: Phase 0 started 2026-06-02 and all of the below were satisfied. Current entry point = the Status snapshot's "Next concrete action" (당시 06-04 기준: launch the FULL #7 scale run; then ③ SV-baselines port — 이후 스냅샷이 계속 갱신되므로 스냅샷 최신 내용을 따를 것).
