...
| 4 | retrain oracle 주 metric ROUGE → **val-loss** | 06-07 | `2026-06-07-phase2-task6-a-retrain-oracle.md` |
...
| 9 | N=10 retrain oracle **연기** (2–5일) | 06-07 | `2026-06-07-phase2-task6-...md` |
...
### 4. retrain oracle 주 metric: ROUGE → val-loss (06-07) — **방법론적으로 가장 중요**
- **원**: retrain oracle을 ROUGE-L(CNN test-acc 유추)로 프레이밍.
- **변경/왜**: Yonghee 교정 — 우리 method(estimator/in-run oracle)가 Shapley를 *옳게 계산*하는지 검증하려면 retrain oracle가 **같은 게임(val-loss)** 풀어야. ROUGE는 다른 게임(불일치가 metric 탓일 수 있음). estimator는 val-loss의 Taylor → apples-to-apples 필수. ROUGE는 미분불가→estimator-ROUGE 불가능, in-run ROUGE도 infeasible. 실측이 정당화: retrain val-loss=in-run oracle=estimator +1.000 vs retrain ROUGE 발산(1B +0.4, 3B −0.9, answer_swap의 도메인-포맷 학습에 속음). [`2026-06-07-phase2-task6-...md`].
