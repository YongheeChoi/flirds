...
5. **속도/정확도 프론티어 지배** [ⓑ]: N=5 near-additive → Shapley linearity로 in-run utility 쓰는 모든 방법이 같은 ranking(+1.000) → Flirds가 **5–15× 더 싸게** 같은 답 (Flirds-1st 35s vs GTG/FedSV/oracle ~530s). 단 이건 N=5 near-additive의 결과 — 큰 N·강한 상호작용서 분리력은 real grid로 확인 필요(ⓒ).
