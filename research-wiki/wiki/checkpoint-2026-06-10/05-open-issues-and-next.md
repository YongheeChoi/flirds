...
- **여전히 열린 것**: ① 이 framing을 논문 headline으로 확정 = **Yonghee 결정** ② **real config(full val, in-run oracle 동반)에서 재확인** 필요(현재는 tiny val=20 smoke) ③ in-run oracle이 잡는다는 건 smoke 논증 — real서 in-run oracle/loss-heur AUROC 직접 측정 필요.
...
- **real grid 전체**: silo5 N5(4 threat × 3 seed, all method + in-run oracle Spearman) → device100 α-sweep{0,0.01,0.1,0.5,5.0}(in-run oracle@α0.5만) → 3B → 7B. ⓒ
...
코드와 phase1(silo5 N5, 3-seed)은 단단하다. 방법은 retrain oracle = in-run oracle=estimator +1.000으로 검증됐다. **남은 건 real grid를 돌려 (1) 큰 N 분리력, (2) detector 경쟁, (3) poison-vs-Flirds framing을 real config에서 확정하는 것** — 전부 ⓒ(미실행), 설계는 LOCKED.
