...
## [2026-06-19] conv | Pearson(값-수준 fidelity) 추가 + Track C/D 파생 emitter + merge_oracle_a 경로 수정
- raw: raw/conversations/flirds/2026-06-19-pearson-value-level-fidelity.md
- distilled into: [[flirds-implementation-plan]] §3.11 (fidelity 지표에 Pearson) + 루트 CLAUDE.md + runs/README.md
- note: Yonghee 질문(순위 Spearman 외 **값 자체의 선형 일치** Pearson — 타 연구 관행)에서 출발. fidelity vs oracle/truth 비교 전체에 Pearson 추가 — `metrics.pearson`(affine-invariant; N=5 near-additive 순위 포화 아래 값 격차 노출) + 러너 3종(track_c1 (a)/(b)·track_d (b)·phase2_matrix truth, 앞으로의 런 네이티브 저장) + make_analysis가 phase2 기존 25셀 phi.parquet 백필(+GTG 거리 3종). **스코프 = 기여도-vs-oracle만**(spearman_vs_rate·track_c3 안정성·detection AUROC 제외; Yonghee "다른 곳 굳이 안 해도"). Track C/D 기존 셀 = rundir 불변 + 파생 emitter: merge_oracle_a 확장((a)+(b)×Spearman+Pearson→runs/track_c/fidelity.csv) / make_fidelity.py 신규(→runs/track_d/fidelity.csv); 둘 다 gitignore. **부수 발견·수정**: merge_oracle_a가 reorg 이전 경로(runs/track_c1)+옛 `_aonly` 이름이라 깨져 있던 것 → repo-루트 상대+새 명명으로 standalone 교정. 검증: py_compile 7파일 OK, make_analysis 25셀 clean(34CSV/31charts), 두 emitter clean. 메트릭 효용 = silo5 poison Flirds1st Spearman 0.0→Pearson −0.95 등 순위 포화 아래 값-구조 노출(수치는 analysis/·fidelity.csv에만). 미추적 3B rundir(Track D 본실행)는 커밋 제외.

## [2026-07-02] conv | 신호 크기 진단(Phase 1) + rank/참여 probe 계획
- raw: raw/conversations/flirds/2026-07-02-signal-size-diagnosis-probe-plan.md
- distilled into: [[flirds-signal-size-diagnosis]] (신규 분석 문서; Phase 1 완결 + Phase 2 계획 §2)
- note: Yonghee 가설("val-loss 변화량이 너무 작아 fidelity·intervention 정밀도 저하") 진단 — 재실행 0, 기존 rundir만. **판정: fp32 병목 아님(신호가 ulp의 10²⁺배) / 절대 학습은 절반만 실재(val-loss·ROUGE 실재, MMLU 0/음수 = 포맷 학습) / 주 병목 = IID-clean 무대에 클라 간 진짜 신호가 구조적으로 없음**((b) oracle 자기 순위 cross-seed ρ≈0, CNN 대조군 label_flip 0.968 vs iid −0.042; 게임 가산적[갭 ≤0.9%]이라 semivalue 전부 같은 순위로 붕괴). intervention은 paired val-loss 축에서만 SNR 2.4–4.5(효과 0.07–0.3%; 7B 최대), MMLU/ROUGE는 효과<표본 SE라 원리적 검출 불가. 가산성 갭은 loss-heur=singleton U({i}) 트릭으로 phi.parquet만에서 계산. **Phase 2 probe(승인 대기)**: LLM 1B anchor rank{32,64}+std N=50/5 rank{16,32,64} 3-seed ≈270 GPU-h(파일럿 seed0 게이트), CNN 폭w{0.5,1,2,4}×참여 C1 72셀+C2 36셀(yonsei sbatch 작성/제출 분리). env-gated 기본값=현행. 운영: GPU0–2에 β0.3 캠페인의 3B_anchor5 재실행(ORACLE_A=0) 진행 중 — canonical 덮어쓰기는 의도된 설계로 확인(정정: 3B anchor rundir은 원래 (a)oracle 미포함, 1B anchor만 (a) 있음).

## [2026-07-02] note | probe 파일럿 가동 + yonsei 인계 (β0.3 캠페인과 비간섭 확인)
- **이번 probe는 기존 결과를 덮어쓰지 않음(Yonghee 질문에 확답)**: LLM은 `runs/probe_signal/rundirs/` 신규 셀명(`1B_anchor5_r{32,64}_*`, `1B_std50k5_r*_*`), CNN은 `runs/probe_signal/cnn_c{1,2}/` `pc*` 셀명; 기준점(rank16 anchor, CNN w=1)은 재실행 없이 기존 rundir 재사용(그리드에서 제외). β0.3 재실행 캠페인(runs/rerun_beta03, v2 큐)은 **의도대로 canonical rundir을 덮어씀**(단일 provenance; β0.5 원본은 git 히스토리) — 세션 중 오해로 분리 조치를 넣었다가 Yonghee 정정으로 전부 원복(3B_std20 canonical=β0.3본, 가드·리다이렉트 제거). 파일럿(seed0, full 11종) 가동: GPU3 anchor r32 진행 중, 캠페인 v2 잔여(3B_anchor5 3-seed 후 전부 #PAUSED) 종료 시 GPU0-2가 B셀(std50k5)로 자동 전환. CNN 66+30셀은 `runs/probe_signal/PROMPT_yonsei_cnn.md`를 yonsei 새 세션에 붙여넣어 제출(선행: main push). 커밋 "signal-size probe" 계열.

## [2026-07-03] note | 기여도-가중 training 조사 + 신규 방법 제안 3건
- filed into: research-wiki/survey/contribution-weighted-training.md (survey 스레드 페이지)
- method: 12-agent 병렬 조사(위키 리더 4 + 웹 테마 조사 5 + 초안 메커니즘 novelty 공격 3, 원 논문 PDF 대조 검증)
- note: 4 naive arm(min-max→EMA→정적 매핑)이 문헌의 집계-가중 셀을 정확히 대표함을 확인(FedIF Eq7-9 = 우리 공통 기계). 공백 5개 확정 — G1 2차(HVP) 정보의 training 소비 0건 / G2 개입강도의 실측-적응(폐루프 제어) 0건(FedLAW·FedHAW·ByGARS는 train-loss·brute-force) / G3 val-loss Taylor 폐형-유도 가중 없음(핵심 항등식: ∂L_val/∂w_i = a_i+c_i = Flirds가 이미 지불한 양) / G4 검증-앵커 성분수술 빈칸(수술은 전부 peer/buffer 참조) / G5 LLM/PEFT 스케일 0건(적극 반박 실패; near-miss FedPSF-LLM·WinFLoRA 구분 인용 필요). 제안 3건: TRAC(신뢰영역 지수-tilt 가중, w∝p·exp(−λ(a+c)), λ=TR-적응, dead-zone→FedAvg 붕괴 = 구성적 do-no-harm, 비용 0 — 주력 권고) / QP-Agg(k-HVP Gram B_ij로 강하-최적 QP + Shapley interaction 진단; 상호작용-실질 무대 전용) / VDP(val-상승 성분 rank-1 투영 제거, a_i≤0 ∀i면 FedAvg 항등, 비용 0). novelty 공격 3/3 partial-overlap — 필수 인용: DoGE·FedAdp·ByGARS·FedHAW(TRAC), FedMGDA+·SmartFL·GREATS(QP-Agg), A-GEM/Fed-A-GEM·ATTITTUD·Zeno++(VDP). 부수 교정 발견: arm-4 'Ripple convention' 어트리뷰션 미확인(Ripple 위키에 가중규칙 없음, SPACE가 동형 혼합 사용 — 원문 대조 필요), sources/game-of-gradients-sfedavg의 SV-가중 집계 서술은 원 논문과 불일치(실제 선택-only — lint 후보). 구현·실험 미착수(Yonghee 검토 대기).

## [2026-07-04] conv | 논문화 전 검증·감사 5건 (수학 엄밀성·Ripple·정밀도·deferred·cost)
- raw: raw/conversations/flirds/2026-07-04-verification-survey-5items.md
- filed into: research-wiki/survey/{irds-fl-math-rigor,ripple-audit,precision-policy,deferred-rigor-inventory,cost-comparison-methodology}-2026-07/ + 통합 2026-07-verification-overview.md
- method: 정찰 16-agent 병렬 → 문서화 2 워크플로(작성→적대적 검증→수정; 항목1은 명제별 반박 패널 18) → 소형 실측 2(원격 CPU-only). 세션 중 Fable5 한도→Opus4.8 전환 후 반박·fix·개정 재실행 완주.
- note: **감사·수학 검증·소형 재현 측정** 세션(그리드 재실행 아님). 코드·rundir·git-modified 위키 미수정, commit/push 없음(Yonghee 검토 대기). **핵심 확정**: ① estimator = (b) in-run oracle **정확히 같은 고정가중 게임**(S-비의존 p_k^r), S-재정규화는 (a)에서만 실현 — 명제 P1–P8 논문급 정리(가정·완전증명·코드대응·IRDS대응·성립경계). ② gpt2 스모크가 **P1·P2 대수 정확성 1e-12 확증**(closed=flirds_values 5.83e-12, 2차-Shapley=닫힌형 7.61e-12, per-round=2^N 3.93e-7); P3 물리 잔차는 gpt2 노이즈 바닥이라 1B 대기(measure_taylor_residual.py+RUN_1B.md 준비). ③ **Ripple 사전 CPU-spin 가설 실측 반박**(eigsh 12/12 정상 수렴, |λ|max≈1.02, maxiter 미도달; tol=0은 ×2 비효율이나 폭주 아님) — 62×/49×는 소형모델·학습포함 누적시간을 느린 baseline 대비로 잰 것(절대 오버헤드 2.05×); from-logs 재구성 불가. ④ 정밀도: matmul 진짜 fp32·cuDNN conv TF32 기본on(CNN 노출), (a) oracle 러너 기본 bf16 발견, protocol §1 문서-코드 불일치; 옵션①/② 대칭 서술; ×3.1은 미검증 placeholder. ⑤ 선행 7편 원문 조사: **valuation-only wall-clock 표준 관행 사실상 부재** → Flirds 보고가 더 엄격. ⑥ deferred 총 인벤토리(A–E, 위계 우선순위): 최상위=(a)/(b) N=5·near-additive·등n 축퇴 갇힘(현행 +1.000/+0.900 무정보)·Fed-LOO 수치 부재·free-rider/PGD 미실행. **반박 패널 최중요 지적**: P6 (a)/(b) 판별력은 축퇴 레짐 밖에서만 유효 검증됨. Yonghee 결정 대기 다수(overview 취합). 원격 작업물=신규 flirds_verify_scratch/(원본 무수정). 프롬프트 파일 삭제 완료.

## [2026-07-14] lint | survey↔file-canon 정합성 감사
...
## [2026-07-23] note | overview 후속 — 뭉친 baseline 개별화 + R4 frrand/strmain-dose ⬚ 축 등재

- touched: [[survey/flirds-paper-results-overview]], REMAINING.md
- note: Yonghee 지적 2건 처리. **①뭉친 baseline 값 개별화**(cnn_competition.csv 재소싱): §5.3 CNN online/retrain `gtg/fedsv/comfedsv/shapleyfl`·`flirds1st/fedif` 뭉치 → 방법별 개별 행(online frzero gtg .3915·fedsv .3966·comfedsv .3918·shapleyfl .4020; retrain 등 전부 기입) + vanilla/oracle/random_excl 앵커 명시. 같은 방식으로 §5.2 anchor5 same-game 3종(각 0.933±.047, (b)-완전일치라 provably 동일)·부록 D stability(track_c RESULTS.txt 10칸 재집계 — (b) 0.518·Flirds 0.547 검증 일치, loss-heur 0.474±.448·Flirds-1st 0.510±.461·GTG 0.311·FedSV 0.289·ComFedSV 0.198·ShapleyFL 0.124·FedIF 0.243) 개별화. 서술 클레임도 방법명 명시형으로. **②frrand 축**: CNN competition에 frrand 실재(단 **flirds 단독 leg**·online-only, 다른 7방법·retrain 미실행) → §5.3 CNN online 표에 열 추가(flirds .5895+앵커, 나머지 "–"+각주). **R4(LLM) frrand·strmain류 per-client dose는 미실행 = 설계 공백**(비용/모달리티 제약 아님; 필연 배제는 grad-noise뿐) → §5.1(C) 위협표·§5.2 F-4 각주·§5.3 R4 개입표에 **⬚ 축으로 등재**, 설계 노트를 "Yonghee 판단 대기"→"결정: 추가"로 갱신. **REMAINING.md**: 제외목록의 "R4 frrand 재제안 금지" **번복**(취소선+§1.6a 이관) + **§1.6a 신설**(L9 R4 frrand ~50–70 GPU-h·L10 R4 strmain-dose ~30–45; L6 graded-noisy 승격·caveat=near-additive 포화). figure 재생성 불요(표-only). runs/ 읽기전용·paper/ 무수정·로컬 커밋만.
