...
- note: Yonghee 가설("val-loss 변화량이 너무 작아 fidelity·intervention 정밀도 저하") 진단 — 재실행 0, 기존 rundir만. **판정: fp32 병목 아님(신호가 ulp의 10²⁺배) / 절대 학습은 절반만 실재(val-loss·ROUGE 실재, MMLU 0/음수 = 포맷 학습) / 주 병목 = IID-clean 무대에 클라 간 진짜 신호가 구조적으로 없음**((b) oracle 자기 순위 cross-seed ρ≈0, CNN 대조군 label_flip 0.968 vs iid −0.042; 게임 가산적[갭 ≤0.9%]이라 semivalue 전부 같은 순위로 붕괴). intervention은 paired val-loss 축에서만 SNR 2.4–4.5(효과 0.07–0.3%; 7B 최대), MMLU/ROUGE는 효과<표본 SE라 원리적 검출 불가. 가산성 갭은 loss-heur=singleton U({i}) 트릭으로 phi.parquet만에서 계산. **Phase 2 probe(승인 대기)**: LLM 1B anchor rank{32,64}+std N=50/5 rank{16,32,64} 3-seed ≈270 GPU-h(파일럿 seed0 게이트), CNN 폭w{0.5,1,2,4}×참여 C1 72셀+C2 36셀(yonsei sbatch 작성/제출 분리). env-gated 기본값=현행. 운영: GPU0–2에 Yonghee 3B_anchor5 재실행(ORACLE_A=0) 중 — 완료 시 (a)oracle 포함 기존 rundir 덮어씀(커밋본 안전) 인지 필요.

## [2026-07-02] note | β0.3 재실행 분리 보존 + probe 파일럿 가동 + yonsei 인계
- Yonghee 지시: 재실행이 기존 셀을 덮어쓰지 말 것(비교용 원본 유지) → **β0.3 재실행 출력 분리**: `runs/track_d/rundirs_beta03/`에 3B_std20 3-seed 이동+원본 git 복원(원본 Flirds1st ρ 1.000 vs β0.3 0.997 병존); 진행 중 3B_anchor5 3-seed는 `runs/rerun_beta03/preserve_beta03.sh` 가드가 종료 시 동일 처리; v2 큐 #PAUSED 31라인 전부에 RUNDIR_ROOT 리다이렉트 삽입(재개 안전). 맥락: β0.3 캠페인 v1(06-26~30)의 c1/c2 덮어쓰기 127셀은 06-30 22:00에 이미 원본 복원됨(β0.3본 미보존), v2는 3B track_d만 진행 후 전부 PAUSED — 캠페인 종료 시 GPU0-2가 probe 파일럿 B셀로 자동 전환. probe 파일럿(seed0, full 11종) 가동: GPU3 anchor r32 진행 중. CNN 66+30셀은 `runs/probe_signal/PROMPT_yonsei_cnn.md` 프롬프트로 yonsei 새 세션 인계 대기. 전부 커밋(시그니처 "signal-size probe").
