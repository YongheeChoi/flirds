...
| H-14 | **R4 P1w 크기-가중의 추가 가치**(flirds-only; T1 `flirds_gatew_v2`·T2 `t2_signw_flirds`; {clean,noisy nr0.7,frzero}×3seed; 사전등록 2026-07-23, T3/L7) | **frzero**: kept=P1과 동일(exact-0→w=0)이라 EM ≈ P1(oracle-동값). **noisy**: **T2는 P1 ≥ P1w**(CNN P2-retrain 열세·P5s-T2 −1.2pt 전례), **T1은 P1w ≥ P1 가능**(CNN P2-online +0.7pt 전례). **clean**: T1 오발화 비용 P1과 유사~소폭 악화, T2는 가중 차등만(±노이즈 바닥). 판정 = 성능만(§3), MISS 그대로 보고. **수록 규칙(사전 고정)**: 전 범위(W-A·W-B·L7) 승→본문 승격 / 동률→"부호가 가치의 대부분" ablation 1문장 / 열세·타 소스 역전(dir1 FedIF>flirds 이미 관측)→미수록(P1만; rundir 존속) | overview §3.2.3 P1/P2(dir1)·H-5·CNN W-A(P1w online +0.7pt / retrain −1.5pt)·T3/T4 |
...
| H-16 | **fmnist 다운스트림 competition**(8점수원 = flirds[`_g` 재사용] + 비-flirds 7 + obs; `sbatch_cnn_fmnist_comp.sh`; {fmnist iid/dir1}×{clean,fr,frrand,gn,lf@0.7,strmain}×3seed = 288 rundir; 사전등록 2026-07-25, Yonghee "fmnist 전부") | cifar10/dir1 §3.2.3 경쟁 구조의 fmnist 재현: ① **오염 셀** same-game(flirds)·estimator 계열 게이트 발화로 recovery 상위, exact-0 계열(free-rider)서 완전 배제 · renorm(GTG/FedSV/ComFedSV/ShapleyFL) 부분회복+clean 오배제 동반 ② **clean 셀** flirds 무발화(do-no-harm; dir1 clean 전례 유지) vs renorm 오발화 가능 ③ **fmnist는 cifar10 대비 저해상**(태스크 쉬움 → 오염 dAcc 절대폭 작음·순위 축퇴 가능 = MISS 아니라 무대 효과) ④ 크기-비대칭 없는 iid서 소스 간 격차 최소·dir1서 확대. **판정 = 성능만**(§3); recovery 분모(vanilla/oracle_excl)는 track_g fmnist `_g`서 조인. **스택 = 3090/lora4cl(torch2.11)** = `_g`·seed0/1 c2fid와 동일(셀 내부 무혼합) | overview §3.2.3(dir1 기준선)·H-7·H-9·c2fid fmnist fidelity(§5.2) |
