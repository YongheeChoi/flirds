...
**Slurm = CNN 전량 (4계정 × 8슬롯, 전부 3090) — wall-time 균등 배분**

| 실행처 | 담당 | c1축 array | GPU-h | wall |
|---|---|---|---|---|
| **YH** | **코드 C-b·C-a** + G3(80)·G8(25)·G6(15) + c1 cifar10 s0 | `0-7` | **193** | **24h** |
| **HJ** | **G10** mnist downstream(135) + c1 cifar10 s1 일부 | `16-21` | **190** | **24h** |
| **JW** | c1 cifar10 s2·s1잔여 + mnist s0 | `32-39,22-23,8-15` | **182** | **23h** |
| **JB** | c1 mnist s1·s2 | `24-31,40-47` | **182** | **23h** |

> **균등화**: 종전 15/17/30/32h → **전부 23–24h**. c1축 48셀(492 GPU-h)이 2계정에만 몰려 있던 게 원인이라 4계정으로 쪼갰다. 단가 차이(cifar10 9.1h vs mnist 11.4h)를 반영해 셀 수가 아니라 **GPU-h 로 맞췄다**.
> **본문 우선**: cifar10(본문 G2) seed0=YH · seed1=HJ+JW · seed2=JW 로 3계정이 나눠 갖고, mnist(부록 G9)는 JB 가 몰아 받는다 → 본문 표가 먼저 닫힌다.
...
1. **HJ**: **L11 배열 전량 `scancel`**(실행 중 포함) — HJ 는 LLM 을 전혀 맡지 않는다. 착지한 rundir 은 지우지 않는다(집계는 canonical 이 dup-win).
2. **YH**: **G3 즉시 착수**(게이트 없음) + 병행해 **C-b·C-a 코드**.
3. **B200**: 세션 A = `CID=1`·`CID=2`, 세션 B = `CID=3`·`CID=4`. **c4 가 online flirds1st 9셀 전량**을 맡는다.
4. **C-a 착지 후**: HJ → `sbatch_cnn_mnist_comp.sh`(G10), YH → `sbatch_fid_mnist.sh`(G8).
5. **C-b 착지 후**: `sbatch_c1_axis.sh` 를 4계정이 각자 몫으로 — YH `0-7` · HJ `16-21` · JW `32-39,22-23,8-15` · JB `24-31,40-47`.
6. **JB(L9)** 는 이미 전량 중단 확정 — 잔재만 확인.
