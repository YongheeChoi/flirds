...
**0″. 오염-집합 규약이 무대별로 다르다 — 확정 사항 + 논문에 1문장 필요.**

| 무대 | label_flip 오염-집합 | update-level(fr·gn) | 근거 |
|---|---|---|---|
| **N=100 주무대**(`track_c2`) | **독립 Bernoulli(ρ=0.4)** → 개수 변동(**실현 39/48/47**) | 고정 `⌊ρN⌉`=40 | FedCorr 공식 구현 재현(`track_c2.py:248-259` 감사 · 논문 `paper-ko.md:863-869`) |
| **N=10 (a) 무대**(`track_c1`, 신규) | **고정 `⌊ρN⌉`=4** | **고정 4** (동일) | `track_c1.py:188-192` — ① 도즈를 0.70 으로 고정해 **FedCorr 를 재현하는 게 아니다**(τ~U(0.5,1) 미사용) ② N=10 Bernoulli 는 **3/7/6** 으로 흔들려 label_flip 열이 다른 도즈가 되고 **4위협이 한 fidelity 표를 공유할 수 없다** |

**Yonghee 판정(07-25) = YH 버전 그대로 통일.** ⟹ 코드 변경 없음 · **JW 의 실행 중 8셀 전부 유효
(재실행 0)** · JW 의 중복 구현 `989f5ca` 철회(`f056b16`).

- 기대 corrupt 집합(전 위협 공통 · **seed-only** = dataset/partition/dose 무관):
  **seed0 `[1,4,6,7]` · seed1 `[0,4,6,7]` · seed2 `[3,4,6,9]`** — 스모크 대조 기준.
  (`default_rng(1000+seed)` 의 첫 소비 = `track_c2` 와 같은 스트림/오프셋.)
- **N=10 Bernoulli 를 안 쓰는 판단의 정량 근거**: 재현 결과 3/7/6(평균 5.33 vs 명목 4, sd 1.55).
  같은 코드로 N=100 을 돌리면 논문 기재값 **39/48/47 을 정확히 재생산**하므로 이 수치는 신뢰 가능.
  N=100 은 39–48(상대변동 ~12%)이라 무해하지만 N=10 은 3–7(~39%)로 열마다 도즈가 달라진다.
- **⚠ 남은 작업 = 논문 1문장**(§9): 부록 B 의 오염-집합 문단에 "(a) 무대(N=10)는 도즈 고정으로
  FedCorr 재현 대상이 아니고, N=10 에서 Bernoulli 는 3/7/6 으로 흔들려 4위협이 한 fidelity 표를
  공유할 수 없으므로 **전 위협 고정 `⌊ρN⌉`=4**" 를 명시. 이게 없으면 §B 서술이 이 무대에 대해
  거짓이 된다 — **코드가 아니라 서술이 유일한 미결 항목이다.**
