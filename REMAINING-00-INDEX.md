...
> **[07-27] 착지 4건 + 실행 취소 2건 — 잔여가 ~117 → ~24런으로 줄었다.**
...
> - ⛔ **G5(4) · G12(23) 실행 취소** (2026-07-27 Yonghee) — 해당 표는 **논문 미수록**(T12 LLM 2차항 레그 · A10 LLM lever 레그).
> - **남은 무게중심**: ① **G1(LLM 주무대 (b) 오라클)** = **유일한 본문 공백** ② G6 6~9 · G9 7(제출됨) · G4. 잔여 총 **~24런**.
...
- **미수록 확인**: `std50k5` 의 오염 규약(noisy `{0-4}`·fr `{5-9}` = **각 10%**)은 수록 대상이
  ...
  있다. 따라서 위 3종이 전부다.
**0‴. 해결 = 논문에서 FedCorr 주장을 걷어낸다** (Yonghee 07-25). **코드 변경 0 · 재실행 0.**

"c1축을 Bernoulli 로 맞출까"를 검토하다 **YH 근거 ①이 주무대에서도 성립하지 않음**을 확인했다 —
주무대 rundir 도 `label-flip_fr0.70` 로 **도즈를 0.70 에 고정**하고(`C2_FLIP_RATE`) 그러면서
Bernoulli 마스크를 유지한다. 논문은 그 둘을 분리해 썼다(`:864` 집합=FedCorr / `:867` 강도=우리 것).
즉 "도즈를 고정했으니 FedCorr 재현 대상이 아니다"는 두 무대를 구별하지 못한다.

더 결정적으로, **FedCorr 와 맞은 건 4개 축 중 1개뿐**이었다:

| 축 | 우리 | FedCorr |
|---|---|---|
| 오염-집합 draw | Bernoulli(ρ) ✔ **(주무대만)** | Bernoulli(ρ) |
| 강도 | **0.70 고정** ✗ | `τ~U(0.5,1)` draw |
| relabel 분포 | **참 라벨 제외 K−1 균일** ✗ | 전 K 균일((K−1)/K 희석) |
| 적용 무대 | 3무대 중 1 ✗ | — |

⟹ "공식 구현 그대로"는 과대주장이었다. **주장을 걷어내고 우리 프로토콜을 그대로 서술**하면
무대 간 차이가 방어 대상이 아니라 단순 보고 항목이 된다. 코드는 손대지 않는다.

**논문 부록 B.2 문단 재작성 완료**(`paper-ko.md` — FedCorr 언급 2줄이 그 문단에만 있었다):
① **강도는 어느 무대에서도 분포에서 뽑지 않고 상수 고정**(전 무대 공통 원칙; 값은 무대별) ·
② **오염 집합은 무대별 명목 비율·실현 방식이 다르다 → 실현 수 병기 표**. 전수 확인 결과:

| 무대 | 실현 방식 | 실현 오염 수 |
|---|---|---|
| `CNN-Main` N=100 | lf = Bernoulli(0.4) / fr·gn = `⌊ρN⌉` 추출 | 39/48/47 · 40 |
| `CNN-Small` N=10 | 전 위협 `⌊ρN⌉` 추출 | 4 (40%) |
| `LLM-Main` N=50 | 고정 인덱스 `{0..19}` | 20 (40%) |
| **`LLM-Device` N=100** | 고정 인덱스 `{10,30,50,70,90}` | **5 (5%)** |
| **`Silo` N=5** | swap=`{0}` · fr=`{1}` | **1 (20%)** |
| `LLM-Scale` · `Anchor` | clean 전용 | — |

> ρ 는 **무대별로 40%/40%/40%/5%/20%** 로 다르다(초안에 "전 무대 ρ=0.4" 라 쓴 것을 정정).
> 실제로 uniform 한 것은 **강도를 상수로 고정한다는 원칙**뿐이며, 값도 무대별로 다르다
> (`LLM-Main` swap 0.7 vs `Silo` nr 1.0).

**⚠ `paper-ko.md` 는 커밋하지 않았다** — 다른 세션의 미커밋 변경 916줄이 함께 들어가기 때문이다.
문단은 디스크에 있으니 그쪽 작업과 충돌하지 않는지 확인 후 커밋할 것.
...
| 파일 | 담당 |
|---|---|
| `REMAINING-b200.md` | G1 · L1 clean 보강 · ~~G5 · G12~~(⛔ 07-27 취소) |
