...
## 0″. ★ 콜드 스타트 — **새 세션은 이 절만 읽고 재개하면 된다** (07-27 22:20 기준)
...
| 항목 | 값 |
|---|---|
| 구성 | **컨테이너 3대 `yonghee-1..3` × B200 1장** (단일 3-GPU 서버 아님) — 07-27 22:00 교체로 `yong-1..4`(4대)에서 **3대로 줄었다** |
...
| 제어 | `bash runs/track_h/b200_fleet.sh {status,up,tail,drain,seal,kill}` — SSH 동시 제어 |
| 런처 | `runs/track_h/run_b200_lane.sh` (`LANE=1..4`, `GPUS="0"`), 큐 = `queue_b200_lane{1..4}.txt` |

**★ 레인↔노드 매핑** (`b200_fleet.sh` 의 `NODE_ARR` 기본값 = 레인 1,2,3,4 순):

| 레인 | 노드 | 큐 내용 |
|---|---|---|
| 1 | **없음**(`_none_`) | 비어 있음 — B 두 줄은 레인3 으로 옮겼다(아래) |
| 2 | `yonghee-1` | `G1_L2_frzero_s2` |
| 3 | `yonghee-3` | R-clean ×3 → `sflb03_seed{1,2}` |
| 4 | `yonghee-2` | `G1_L2_frzero_s0` |

> **레인↔큐 매핑은 절대 바꾸지 않는다**(큐 내용이 레인에 묶여 있다). 노드가 바뀌면 **레인→노드만**
> 다시 붙인다 — `b200_fleet.sh` 상단 `NODE_ARR` 기본값 또는 `FLEET_NODES="n1 n2 n3 n4"` 로.
> 노드가 4대로 늘면 레인1 자리에 실제 노드를 넣으면 된다(단 레인1 큐는 지금 비어 있다).
...
```bash
bash runs/track_h/b200_fleet.sh nodes       # 노드 이름·도달성·GPU (이름이 바뀌었으면 여기서 DOWN 으로 뜬다)
bash runs/track_h/b200_fleet.sh status      # 레인별 driver/GPU/현재셀/큐진행
bash runs/track_h/b200_fleet.sh seal        # 완주 셀을 레포 큐에 '#' 처리(삭제 아님)
bash runs/track_h/b200_fleet.sh up 2 4 3    # 우선순위 순 기동 (레인1 은 노드 없음)
```

**컨테이너 이름이 또 바뀌면** `nodes` 가 전부 `DOWN` 으로 뜬다 → `/etc/hosts` 에서 새 이름을 확인하고
`b200_fleet.sh` 상단 `NODE_ARR` 기본값을 갈아끼운다(또는 `FLEET_NODES=` 로 덮어쓴다). **레인↔큐 매핑은
건드리지 않는다.** `seal` 은 **전 레인 로그를 뒤진다** — 부하균형으로 셀이 다른 레인에서 돌았어도 정상 처리된다.
GPU 가 모자라면 **레인4·2(G1 frzero) 를 먼저** 띄운다 — 유일한 필수분이다.

### 5) 잔여 작업 — **07-27 22:11 전 레인 재기동 완료** (3-GPU)

> **07-27 22:00 컨테이너 교체.** 구 컨테이너는 22:00 경까지 살아 있다가 죽었고, 그때 돌던
> **4셀이 전량 소실**됐다(`frzero_s0` 3.2h · `frzero_s2` 3.0h · `R-clean_s0` · `sflb03_seed1`).
> `phase2_matrix` 는 cell-end 1회 persist 라 **부분 rundir 오염은 없다** — 그대로 재실행분이다.
> `seal` 이 완주분 3줄(`L1_clean_obs_s2` · `L1_clean_online_s2` · `sflb03_seed0`)만 주석 처리했다.

| 우선 | 셀 | 레인/노드 | 단가 | 상태 | 없으면 |
|---|---|---|---|---|---|
| **1** | `G1_L2_frzero_s0` | 4 / `yonghee-2` | 9.6h | ▶ **22:11~** → 07-28 **~07:47** | **G1 frzero 가 1-seed → §5.2 fidelity·§5.4 탐지 표에서 frzero 행을 못 쓴다** |
| **1** | `G1_L2_frzero_s2` | 2 / `yonghee-1` | 9.6h | ▶ **22:11~** → 07-28 **~07:47** | 〃 |
| 2 | `1B_silo5_clean_removal_seed{0,1,2}` (R-clean) | 3 / `yonghee-3` | 2.5h | ▶ seed0 **22:11~** → ~05:45 | 부록 T13 removal 의 clean 대조 부재 = 위협-특이성 미증명 |
| 3 | `1B_anchor5_sflb03_seed{1,2}` (B) | 3 (R-clean 뒤) | 2.66h | 대기 → ~11:05 | 본문 [F4] ShapleyFL 행이 "β=.5 잠정" 으로 남음 |
...
B `sflb03_seed0`(단 **카나리아 실패** — §0′.10).
**`frzero_s0` 은 07-27 04:06 에 `std_dagmm` cholesky non-PSD 로 30h 돌고 전손 → 이건 재실행분이다.**

**B 는 Yonghee 결정 ⓒ 로 진행한다** (07-27 22:20): 카나리아 실패로 저장된 `(a)` 재사용이 불가하니
`ORACLE_A=0` 그대로 두고 **신규 런 자체의 `(b)` 로 채점**한다. seed1·2 만 추가(seed0 은 착지분 재사용).
⚠ [F4] 의 다른 방법 행은 구 rundir(`39a0a97`) 궤적이라 **ShapleyFL 행만 오라클 기준이 다르다** —
표에 각주로 명시할 것.

**레인1 큐의 sflb03 두 줄은 주석 처리**하고 **레인3 큐 끝에 append** 했다(노드가 3대뿐이라 레인1 에
GPU 가 없다). 드라이버는 15초마다 큐를 재읽고 `consumed` 인덱스로 추적하므로 **끝 append 는 가동 중에도
안전**하다. 되돌리려면 레인3 꼬리 2줄을 주석하고 레인1 2줄의 주석을 푼다(**둘 다 활성 금지**).

**마감 역산**(07-28 24:00): 지금 배치면 전량 07-28 **~11:05** 착지 = 마진 **12.9h**.
1장만 남으면 G1 두 셀 직렬 19.2h → **07-28 04:48** 까지 시작해야 한다.
...
### 0′.4 진행 현황 **스냅샷 (2026-07-27 15:00)** — 실측 단가 기록용

> ⚠ **최신 현황은 §0″-5 를 본다.** 이 절은 그 시점의 스냅샷이며, 남겨 두는 이유는 아래
> **셀별 실측 단가**(9방법 30.65–30.88h vs MIN 9.37–9.88h)가 이후 모든 계획의 근거이기 때문이다.
...
> ### ⛔ 07-27 21:56 — **카나리아 실패. B 는 설계대로는 성립하지 않는다.**
>
> `sflb03_seed0` 착지 후 §0′.10 이 지정한 `(b)oracle` 대조를 수행한 결과:
>
> | | 신규 `rundirs_beta03/1B_anchor5_sflb03_seed0` vs 기존 `rundirs/1B_anchor5_seed0` |
> |---|---|
> | `(b)oracle` max\|Δ\| | **8.65e-04** (값 스케일 1.57e-02 → **상대 5.51 %**) |
> | Spearman | **+0.90** — N=5 인데 순위가 뒤바뀐 쌍이 있다 |
> | `git_sha` | `39a0a97`(06-15) → `30edbad`(현재), 약 6주치 커밋 차 |
>
> **궤적이 재현되지 않는다** → 저장된 `(a)oracle` 과 조인할 수 없다. `ORACLE_A=0` 으로 나온
> ShapleyFL(β0.3) φ 는 **자기 런의 (b) 하고만** 정합한다.
> **함의**: 06-15 rundir 들은 현 HEAD 에서 재현되지 않는다. 저장 궤적 재사용을 전제한 다른 계획도
> 같은 검사를 먼저 해야 한다.
>
> **선택지** — ⓐ `ORACLE_A=1` 재실행(**약 11.2 h/셀 × 3**; 실측 = 학습 2,309 s + valuation 7,276 s
> + (a) 재학습 32회 약 30,817 s) ⓑ B 포기([F4] 에 "β=.5 잠정" 각주 유지) ⓒ 신규 런의 자체 (b) 로 채점.
>
> ### ✅ 07-27 22:20 — **Yonghee 결정 = ⓒ**
>
> `ORACLE_A=0` 그대로 `sflb03_seed{1,2}` 만 추가하고(**셀당 2.66 h 실측**, seed0 착지분 재사용)
> **신규 런 자체의 `(b)` 로 채점**한다. 레인3 큐 끝에 append 됨(R-clean 3셀 뒤 → 07-28 ~11:05).
> ⚠ **[F4] 각주 필수**: 이 표의 다른 방법 행은 구 rundir(`39a0a97`) 궤적이므로 **ShapleyFL 행만
> 오라클 기준(궤적)이 다르다**. ⓐ 를 안 고른 대가이며, 표에 명시하지 않으면 오독된다.
