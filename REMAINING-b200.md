# REMAINING (B200) — HVP 전용: 주무대 (b) 오라클 + clean 열 보강 + probe
...
> **B200 이 이번 계획의 임계경로다** — G1(59 wall-h)이 전체 종료 시각을 정한다.
...
**B200 = 세션 2개 × GPU 2장, 컨테이너는 GPU마다 별개** → **컨테이너당 큐 1개**.

```
CID=1 bash <repo>/runs/track_h/run_b200_batch.sh    # 세션 A — G1 noisy ×3
CID=2 …                                             # 세션 A — G1 clean ×3
CID=3 …                                             # 세션 B — G1 frzero ×3
CID=4 …                                             # 세션 B — L1 clean 보강 → G5 → G12
```

| 세션 | CID | 담당 | GPU-h | wall |
|---|---|---|---|---|
| **A** | 1·2 | G1 noisy ×3 / G1 clean ×3 | 118 | 59h |
| **B** | 3·4 | G1 frzero ×3 / L1c(19)+G5(20)+G12 꼬리 | 59 + 39+ | 59h |

- **두 세션은 서로 의존하지 않는다.** 한쪽이 늦게 잡혀도 다른 쪽 산출물은 그대로 쓴다.
...
- **⚠ 영속 단위**: `phase2_matrix`(G1) = **cell-end 1회** → 셀당 ~19.6h 를 통째로 잃을 수 있다(직전 컨테이너 실사례). `track_g`·`track_d`(c4) = arm/셀 단위라 컷에 강하다.
  → **컨테이너 컷이 예상되면 G1 신규 투입부터 끊고 드레인.** `$BATCH/runlogs/seal_watchdog.sh` 재가동 권장(07-24 사례: 중도컷 0).

## 2. G1 — R4-L2 주무대 (b) 오라클 (9셀 · 176 GPU-h) ★임계경로
...
## 3. L1 clean seed1·2 — §5.3 clean 열의 3-seed 정본 (4셀 · 19 GPU-h)

> 디스크 확인(07-25): **noisy·frzero 는 이미 3-seed 완결**(observer·oracle_excl·random_excl·flirds online + `t2_sign` ×4). **clean 만 seed0** 이다 → 이 4셀이 §5.3 clean 열(무해성 parity·오발화 대조)을 3-seed 로 닫는다.

- 셀 = `clean_obs` seed1·2 (5.5h) + `clean_online` seed1·2 (3.8h).
- clean 은 kept=전원이라 T2 가 `equals_vanilla` 로 스킵되고, online 에선 `oracle_excl`/`random_excl` 가 자동으로 빠진다(제외 대상 부재; `track_g` l.618) → 저비용.
- **noisy·frzero 는 절대 재실행하지 않는다** — 완결·커밋본이다.

## 4. G5 · G12 — probe seed 보강 (c4 꼬리)

| 항목 | 무엇 | 셀 | GPU-h | 위치 |
|---|---|---|---|---|
| **G5** | 2차항(HVP) LLM 레그 — **본문 §5.6①** | 4 (`1B_std50k5_r{32,64}_seed{1,2}`) | ~20 | 필수 |
| **G12** | A축 lever probe — **부록·최저** | 19 | 50–90 | **꼬리·droppable** |

- 둘 다 **R4 무대가 아니다** → `ROUNDS` 를 주지 않는다(track_d 가 자기 레짐 값을 쓴다).
- G12 핵심 질문 = "lr 로 커진 φ가 cross-seed 실재 신호인가"(예측 ρ≈0) → 큐에 **`lr{2,3}e-3` 을 먼저** 배치했다. 마감에 걸리면 꼬리부터 그냥 안 돌아도 된다.

## 5. (옵션) LLM P1w T1 — 여유 시 최우선 추가

- 9런 · **~35 GPU-h**. CNN P1w 는 G3·G10 rundir 에 동반 산출되므로, LLM 레그만 얹으면 **"CNN·LLM 전 범위 3-seed 승격" 판정이 성립**한다. 지금 그게 유일한 공백.
- 관찰자 불요 → canonical `rundirs_llm` 에 직접 착지(arm 명이 신규라 충돌 0). **큐 c4 하단에 주석으로 대기** — G12 와 맞교환하면 된다.
...
| P | 무엇 | 근거 |
|---|---|---|
| **P0** | G1 seed0 (3위협 · 세션 A·B 동시) | 비어 있는 축(LLM fidelity·탐지·canonical timing)을 **여는** 유일한 셀 |
| **P1** | G1 seed1·2 | 3-seed 규칙 |
| **P1** | L1 clean seed1·2 (c4) | §5.3 clean 열의 유일 소스 |
| **P2** | G5 | 본문 ablation seed 미달 |
| **P3** | G12 → (또는 P1w 와 맞교환) | 부록·최저 |
