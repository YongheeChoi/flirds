...
>
> **⚠ 미결 = 논문 부록 B `paper-ko.md:863-869` 에 2문장.** 현 서술(CNN label-flip Bernoulli +
> update-level 고정 `⌊ρN⌉`)은 **그 자체로는 맞지만 두 무대가 빠져 있다**:
> ① **LLM 전 무대** = 결정적 `0..19` = 정확히 40%(클라 샤드 시드-셔플이라 WLOG)
> ② **CNN N=10 (a) 무대** = 전 위협 고정 `⌊ρN⌉`=4 + 위 두 근거.
> **코드가 아니라 서술이 유일한 남은 항목이다.**
>
...
| **NEW** | **논문 부록 B(`paper-ko.md:863-869`)에 2문장** — LLM 결정적 `0..19` · CNN N=10 고정 4 | — | ⬚ | §2 — 코드 아님, 서술 미결 |
