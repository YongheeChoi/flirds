...
> # ✅ **YH 레그 전량 완주 — 2026-07-27 10:22 · 큐 비었음 · 실패 0**
>
> | 잡 | 셀 | 실측 GPU-h | 커밋 |
> |---|---|---|---|
> | G3 cifar10/iid 점수원 경쟁 | **96/96** | ~89 | `4395028` |
> | c1축 cifar10 `0-7,16` | **9/9** | **130.9** | `be53009`·`1ea3856`·`61bd93a`·`c539f4d` |
> | G8 mnist fidelity+탐지 | **24/24** | **9.32** | `b160c85`·`bac5cff` |
> | G6 removal 오염축 | **9/9** | **1.4** | `93dbedd` |
> | G14 mnist 기준 arm | **18/18** | **5.09** | `3111ca1` |
> | **계** | **156/156** | **~236** | 미커밋 산출물 0 |
>
> **마감(07-28 24:00) 대비 ~38h 조기 종료.** 이후 할 일 = §9 인수인계 3건(집계기 상수 교체·표 각주 2건).
...
| **P2** | **c1축 `c1axis` `0-7,16`** | 9셀 · **130.9 실측** | ✅ **9/9 `c539f4d`** | **본문 G2 의 P0 seed** — C-b 를 쓴 사람이 직접 |
...
**✅ 실제 종료 — 07-27 10:22:55 (예측 10:34 대비 11분 조기)**
...
| ✅ **07-27 10:22:55** | **idx16 착지 = YH 전량 완주**(`c539f4d`) — 큐 비었음 |

**c1축 비용 구조 최종**(YH 9 + JB seed2 8 = 17셀 교차 확인) — 갈림의 원인은 Ripple 하나:

| 위협군 | wall | 구성 |
|---|---|---|
| {clean, label-flip} | 14.9~17.4h | (a) 12.9~14.7h + **Ripple 1.9~2.8h** (methods 11종) |
| {free-rider, grad-noise} | 12.9~14.9h | (a) 단독 (methods 10종 · ripple phase 없음) |

(a) 2¹⁰ 재학습은 위협 무관 12.8~14.7h 로 17셀 전부 동일 대역이다.
**최장 셀 15.97h** 였으므로 종전 9.1h 추정을 근거로 `--time` 을 깎았다면 4셀 전손이었다.
...
- 임계경로는 예상대로 **9번째 셀 하나**였다(9셀/8슬롯 → 2차 웨이브 불가피 · QOS 8장 상한).
  G8·G6·G14 51셀은 전부 그 그늘에서 소화돼 makespan 에 0 영향 — G14 를 꼬리에 붙인 판단이 맞았다.
- **마감 07-28 24:00 대비 ~38h 조기 종료.**
...
## 9. ✅ 완료 · 인수인계 (YH 레그 종료 — 2026-07-27 10:22)

**YH 가 더 돌릴 실험은 없다.** 156/156 · 실패 0 · 미커밋 산출물 0.
아래는 **다른 세션·계정이 이어받을 3건**이며 전부 GPU 0 이다.

0. **rundir + 집계는 전부 커밋됨. push 만 남았다** — `push 는 Yonghee 직접`.
   미push 커밋: `61bd93a` `bac5cff` `93dbedd` `3111ca1` `65f78bd` `c539f4d`.
   집계 재생성은 rundir-only 라 언제든 `python runs/track_h/make_analysis.py` 로 재현된다.

   ⚠ **c1축 집계는 아직 돌리지 말 것** — 4계정 공유 48셀 그리드인데 현재 **17/48**
   (YH 9 + JB seed2 8)이다. 지금 표를 뽑으면 부분 seed 가 된다. JW 잔여 착지 후 일괄.

1. **표 각주 2건 — 없으면 리뷰어에게 오류로 읽힌다**
   - **FedSV ≡ ComFedSV(등n 무대 한정)**: cifar10/iid(§3)에 이어 **mnist/iid 에서도 독립
     재현**됐고(P1 둘 다 +0.424) dir1 에서는 갈린다(−0.408 vs −0.476). iid 표에서 두 열을
     독립 baseline 으로 보고하지 말 것. 근거·증명 = §3 박스.
   - **free-rider recovery 분모**: `oracle_excl − vanilla` 가 +0.006~0.007 로 좁다
     (grad-noise 는 +0.053~0.075). 그 열은 잡음에 민감하니 폭을 병기하거나 해석에 단서를 달 것.
1. rundir 커밋(push는 Yonghee) → `make_analysis.py` 재생성 → `flirds-results-{downstream,fidelity,detection}` → paper.
   ...
   - **YH 9셀이 이 페어링의 첫 실물**이다 — `oracle_a` 키가 같은 rundir 의 `metrics.json`
     안에 있음을 `cifar10_iid_clean_seed1` 에서 확인했다(별도 `c1_oracle/*_aonly_*/` 아님).
     위 각주 l.174 가 후자를 가정하고 있으니 **페어링 경로는 전자로 확정**하면 된다.
