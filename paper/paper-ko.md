...
> 명시; 수치는 rundir/분석 스크립트 재생성 값만 기입). † = CNN dir1 기존 실측 — 캠페인
...
FL"이라는 같은 구도를 공유한다. exact retrain oracle (a)가 필요한 비교는 $2^N$ 재학습이
가능한 작은-$N$ sub 무대가 담당한다(§5.2).

| 무대 | 트랙 / 데이터 | $N$ · 참여 | $R$ | 위협 축 | oracle | 쓰임 |
|---|---|---|---|---|---|---|
| **주** `gsm50k5` | LLM 1B(LoRA) · GSM8K | 50 · 5/50 | 200 | clean / noisy(answer-swap@0.7) / free-rider(zero) — 오염 40% | (b) exact per-round $2^5$ | §5.2–5.5 |
| **주** `c2` 캠페인 | CNN · CIFAR-10×{iid,dir1,shard,qskew} + FMNIST×{iid,dir1} | 100 · 10/100 | 120 | clean / free-rider(zero·rand) / grad-noise / label-flip@{.15,.35,.70}·strmain — 오염 40% | (b) exact per-round $2^{10}$ | §5.2–5.4 |
| sub `gsm5` | LLM 1B · GSM8K(주무대 정합) | 5 · 전원 | 30 | clean / noisy@0.7 — 오염 40% | **(a) retrain $2^5$** + (b) $2^5$ | §5.2 |
| sub `silo5` | LLM 1B · 5-도메인 비IID | 5 · 전원 | 10 | clean / noisy / free-rider | (b) exact $2^5$ (+ (a) ⬚) | §5.2·§5.6 |
| sub `anchor5` | LLM 1B · alpaca IID clean | 5 · 전원 | 30 | – | **(a) retrain $2^5$** + (b) $2^5$ | §5.2 |
| sub `c1` | CNN · MNIST+CIFAR-10 × 5시나리오 | 10 · 전원 | 10 | iid / label·quantity-skew / label-flip / feature-noise | **(a) retrain $2^{10}$** + (b) $2^{10}$ | §5.2·§5.6·부록 C·D |

성능 심판은 gsm50k5 = GSM8K 공식 test의 잔여 1,119문항 exact-match(EM; greedy 디코딩),
...
- **같은-게임 계열(3)** — 식 (6)의 고정-가중 라운드 게임을 겨냥한다: **Flirds**(2차 폐형),
  **Flirds (first-order)**(1차 항만), **loss-heur**(라운드 게임의 singleton utility
  $u_r(\{k\})$를 forward 평가로 직접 계산해 합산하는 가산 근사 — Shapley 값 대신 singleton
  값). 이들과 (b)의 차이는 순수한 근사 오차다.
- **cross-game 계열(5)** — GTG-Shapley, FedSV, ComFedSV, ShapleyFL, FedIF: §2의 계보
  ...
  (b)와의 불일치에는 근사 오차와 "다른 게임" 성분이 섞여 있으므로 본문 fidelity 표에서는
  같은-게임 계열과 분리하고, 전표를 부록 C에 둔다. 단 **(a) retrain 대비 비교는 전 방법을
  같은 표에** 둔다 — (a)는 어느 방법의 목표값도 아닌 중립 참값이기 때문이다.
...
방법과 (b) oracle을 같은 로그 위에서 채점한다**(§4.1의 프로토콜). 방법 간 차이는 전부 방법
자신의 몫이며 궤적 분산이 아니다. (a) retrain oracle만 정의상 부분집합별 재학습을 따로
...
정책은 P1(누적 기여도 $\le 0$ 배제, 문턱 $\tau{=}0$의 parameter-free) 하나로 고정하고
무대·셀별 튜닝을 하지 않는다. ③ 예측이 어긋난 칸·오발화도 그대로 보고한다.

### 5.2 Fidelity — exact in-run Shapley 재현 (1차)

**주무대, 같은-게임 계열 vs (b).** 표 [F1]은 CNN 주무대 144셀(파티션 6 × 위협 8 ×
3-seed)을 위협별로 풀링한 Spearman, 표 [F2]는 LLM 주무대의 위협별 Spearman·Pearson이다.

표 [F1] — CNN 주무대(`c2`): 같은-게임 3종 vs (b), Spearman ↑ (위협별; 파티션 × seed 풀) ⬚
<!-- 채움: runs/track_c/c2fid/analysis/{fidelity,cellmean}.csv (make_analysis.py 재생성) -->

| method | clean | free-rider | frrand | grad-noise | lf@0.15 | lf@0.35 | lf@0.70 | strmain |
|---|---|---|---|---|---|---|---|---|
| Flirds | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ |
| Flirds (1st) | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ |
| loss-heur | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ |

표 [F2] — LLM 주무대(`gsm50k5`): 같은-게임 3종 vs (b) exact per-round $2^5$ ⬚(◐)
<!-- 채움: runs/phase2_matrix/rundirs/1B_gsm50k5_{noisy_nr0.7,clean}_s0/metrics.json (L2) -->

| method | noisy Spearman | noisy Pearson | clean Spearman | clean Pearson |
|---|---|---|---|---|
| Flirds | ⬚ | ⬚ | ⬚ | ⬚ |
| Flirds (1st) | ⬚ | ⬚ | ⬚ | ⬚ |
| loss-heur | ⬚ | ⬚ | ⬚ | ⬚ |

(free-rider(zero) 셀의 fidelity는 별도 채점하지 않는다 — (b)와 같은-게임 계열 모두 해당
클라이언트에 대수적으로 정확히 0을 주어(명제 2) 일치가 자명하며, 이 성질은 탐지(§5.4)와
개입(§5.3)에서 실효성으로 검증된다.)

읽기 각주 두 개. ① **clean 칸은 신호-부재 레짐이다**: 클라이언트 간 실제 차이가 없어 (b)
자신의 순위조차 seed를 넘어 재현되지 않는다(부록 D). 이 칸의 낮은 $\rho$는 방법의 실패가
아니라 잴 것이 없음을 뜻하며, 오발화 대조용으로만 둔다. ② strmain(오염 클라이언트별 강도가
연속 분포) 칸에서는 φ가 오염 강도 자체를 얼마나 해상하는지도 채점한다(φ vs 실현 오염율의
Spearman): ⬚ <!-- F-4 사전등록: Flirds ≈ (b) 자기천장 > 1st — c2fid make_analysis 자동 대조 -->.

**sub — exact retrain Shapley (a) 특성화.** (a)는 $2^N$번의 전체 재학습을 요구하므로
주무대($N{=}50/100$)에서는 계산이 원리적으로 불가능하다. 그래서 부득이하게 작은-$N$ 별도
무대에서 특성화하되, 무대마다 다른 축을 담당하도록 세팅을 의도적으로 다양화했다:
**gsm5** = 주무대와 데이터·위협·오염 비율(2/5 = 40%)·검증 규칙·하이퍼를 전부 공유하고
$N{=}5$ 전원 참여·$R{=}30$만 축소한 "라운드-cohort 축소판"(주무대의 라운드당 참여자 수가 곧
5다) — 주무대 정합 주 표. **silo5** = 도메인 이질성(비IID) 신호 무대. **anchor5** = IID-clean
참조. **CNN c1** = (a)·(b)를 동시에 $2^{10}$ 전수로 계산하는 시나리오 격자.

표 [F3] — `gsm5`: 전 방법 vs (a) retrain $2^5$ (주 표) ⬚
<!-- 채움: L8 (paper/workplan/T5-retrain-a-suite.md) rundir → 분석 CSV; 3-seed -->

| method | clean Sp vs (a) | noisy Sp vs (a) | (참고) Sp vs (b) |
|---|---|---|---|
| Flirds | ⬚ | ⬚ | ⬚ |
| Flirds (1st) | ⬚ | ⬚ | ⬚ |
| loss-heur | ⬚ | ⬚ | ⬚ |
...
silo5의 (a)-leg(clean·noisy·free-rider × 3-seed)는 ⬚
<!-- 채움: L8 silo5 *_aonly_* rundir + 기존 canonical phi.parquet 조인(merge 패턴) -->
— silo5는 $N{=}5$ 무대 가운데 유일하게 (b) 타깃 순위가 seed를 넘어 재현되는 무대라(부록 D:
clean +0.87, 오염 +0.93) "(a)가 그 실재 신호를 같게 매기는가"를 처음 재는 칸이다.

표 [F4] — `anchor5`(IID-clean 참조): 전 방법 vs (a) retrain $2^5$ (1B, 3-seed)
<!-- 출처: runs/track_d/rundirs/1B_anchor5_seed{0,1,2}/phi.parquet — truth=(a)로 피벗 재계산 -->

| method | Spearman vs (a) ↑ | Pearson vs (a) ↑ | (참고) Spearman vs (b) |
|---|---|---|---|
| Flirds | 0.933±.047 | 0.933±.055 | 1.000 |
| Flirds (1st) | 0.933±.047 | 0.929±.060 | 1.000 |
| loss-heur | 0.933±.047 | 0.931±.057 | 1.000 |
...
읽기: 같은-게임 3종과 GTG가 0.933으로 동률인 것은 천장 효과다 — 이들은 (b)와 사실상 완전
일치(vs (b) ≈ 1.000)하므로, vs (a) 점수가 두 참값 사이의 일치도 **(a)↔(b) = 0.933±.047**
그 자체로 수렴한다. 즉 이 무대에서 같은-게임 계열의 (a) 재현은 게임 간 간극이 상한이다.
주의할 점은 anchor5가 IID-clean이라 (b) 타깃 자체가 seed-불안정하다는 것(부록 D: cross-seed
$\rho$ −0.37)이며, 이것이 gsm5·silo5를 주 표로 두는 이유다.

표 [F5] — CNN `c1`: 같은-게임 3종 vs (a) retrain $2^{10}$, Spearman ↑ (시나리오별, 3-seed 평균)
<!-- 출처: runs/track_c/fidelity.csv spearman_a group-mean. 전 방법(8종)·Pearson = 부록 C -->

| dataset / scenario | Flirds | Flirds (1st) | loss-heur |
|---|---|---|---|
...
label-flip·quantity-skew에서 같은-게임 계열 vs (a) +0.50~+0.97이고, 전 방법(8종) 가운데
...
T1 online(P1; 점수원 = Flirds):

| arm | clean | noisy | free-rider |
|---|---|---|---|
| vanilla(관찰자) | ⬚ | ⬚ | ⬚ |
| oracle-제외 (천장) | – | ⬚ | ⬚ |
| random-제외 (통제) | – | ⬚ | ⬚ |
...
**CNN(dir1)†, 절대 test acc.** 같은 정책·같은 무대·같은 seed에서 **점수원만 8종으로 교체**한
경쟁이다. 표 [I2] (오염-평균 = free-rider·grad-noise·label-flip 3위협 평균).
...
| loss-heur | .6264 | .6114 | .5981 | .5670 | .5922 |
...
| loss-heur | .6293 | .6125 | .4518 | .6205 | .5616 |
...
계열(Flirds·Flirds-1st·loss-heur·FedIF)은 .61~.62로 천장(.620)에 근접하는 반면, 재정규화
...
복리 악화가 없어 .51~.52로 완화). ② **grad-noise는 2차항 판별 칸이다**: estimator 계열 중
...
재학습(T2) 우위: 전 estimator가 online .57대 → retrain .62대(천장 근접)로 상승한다.
경계-강도 무대(strmain, 클라별 rate ~ U(.5,1))에서도 같은 서열이 재현된다(P1 retrain 회복률
Flirds +0.98·renorm 0.60~0.74; online renorm 파국 최심 FedSV −0.32)†. ④ **clean 오발화의
정직 보고**: online에서 Flirds −0.7pt·loss-heur −1.3pt(누적 부호의 0-교차 노이즈),
Flirds-1st·FedIF는 무발화. 종합하면 정확한 클레임은 "Flirds 단독 1위"가 아니라 — **전
정책·전 시점 상위권 + grad-noise를 잡는 유일한 estimator**이며, 개별 칸 최고는
FedIF·loss-heur도 차지한다. LLM 주무대에서 같은 구조가 재현되는지는 표 [I1] ⬚.

**P1w(크기-가중 변형).** [자리] P1과 같은 배제에 더해, 남는 클라이언트를 양수 누적 기여도
크기에 비례해 가중($w_k \propto n_k \max(\hat\phi_k, 0)$, 합-1 재정규화)하는 변형의 전 무대
검증이 진행 중이다 ⬚.
<!-- 수록 규칙(사전 고정 — workplan 00-INDEX §1): CNN·LLM 전 범위에서 P1 상회 시 본문 승격 /
     동률 시 "부호가 가치의 대부분" 1문장 / 열세·타 점수원 역전 시 미수록(본 절은 P1만). -->

### 5.4 탐지 (2차 ③)

탐지를 위계의 마지막에 두는 이유는 기여도와 탐지가 직결이 아니기 때문이다: 검증 손실을
실제로 낮추는 오염(예: 직전 라운드의 글로벌 업데이트를 재제출하는 free-rider 변형)에 대해
val-loss 게임의 정직한 답은 "기여함"이고, 이는 추정 실패가 아니라 (b) oracle 자신의 답이기도
하다(§6). 따라서 주장 형식은 절대 AUROC가 아니라 **oracle-동행**이다 — 추정기가 (b)와 같은
답을 주는가(사전 등록 기준: $|\mathrm{AUROC}(\text{Flirds}) - \mathrm{AUROC}((b))| \le
...
<!-- 채움: c2fid analysis auroc 열. F-2 사전등록: free-rider 셀 exact-0 계열 AUROC 1.0 vs
     renorm 유령값 저하; frrand 셀 Flirds 계열 ≥ GTG/FedSV -->

### 5.5 비용

**연산수 모델(하드웨어-독립).** 방법별 라운드-누적 지배 연산수는 해석적으로 닫힌다:
Flirds = 라운드당 **HVP 1회**(cohort 크기와 무관), Flirds-1st = 라운드당 val-gradient 1회,
loss-heur = 라운드당 $1{+}|P_r|$ forward, (b) exact = 라운드당 $2^{|P_r|}$ forward. per-op
실측(fp32·B200: forward 1.60s, HVP 10.36s — HVP/forward ≈ 6.5)과의 곱이 wall-clock을
재현한다:

| 무대 | Flirds | Flirds-1st | loss-heur | (b) exact |
|---|---|---|---|---|
| silo5 ($N{=}5$·$R{=}10$) | 10 HVP → 예측 104s / 실측 ~107s | 10 grad (~35s) | 60 fwd → 96s / 96.6–100.1s | 320 fwd → 512s / ~530s |
| anchor5 ($N{=}5$ 전원·$R{=}30$) | 30 HVP → 실측 707±16s | 30 grad (231±5s) | 180 fwd (657±19s) | 960 fwd (3,528±83s) |
...
비를 지배한다 — silo5 행은 microbench 곱이 실측을 그대로 재현하는 검증 라인이다.)

**주무대 실측** ⬚ <!-- 채움: gsm50k5 L1·L2 timing.json + c2fid runtime_s 열 -->. 구조는 위
모델이 이미 말해 준다. 주무대 LLM은 라운드당 cohort 5라 (b)가 라운드당 $2^5{=}32$ forward로
아직 감당 가능하고(같은 구조인 anchor5의 실측 비 ≈ 5×), cohort 10이면 실측 159×
(cross-device), $N{=}10$ 전원 참여의 $2^{10}$ 전 라운드 열거는 32.7h vs Flirds 733s =
160×로 벌어진다(부록 E). 반대로 라운드당 참여가 아주 작은 무대(예: 2명 → $2^2{=}4$ forward
< 1 HVP ≈ 6.5 forward)에서는 (b) 직접 열거가 Flirds(2차)보다 싸다 — **Flirds의 비용 우위는
"참여가 많아 전수 열거가 지수적으로 비싼" 무대의 것**이고, 그렇지 않은 무대에서도
Flirds-1st(라운드당 gradient 1회)는 항상 최저가다. exact retrain (a)는 같은 무대에서 (b)의
~9배다(anchor5: 30,817±244s vs 3,528±83s — $2^5$개 부분집합 × $R{=}30$ 재학습). CNN에서는
valuation이 학습 자체보다 두 자릿수 싸다(c1: Flirds 0.6~1.4s vs FL 학습 80~94s).

### 5.6 Ablation

**① 2차항(클라이언트-상호작용 항)의 기여.** §4.2의 Flirds (first-order)와의 대조다.
fidelity 축: CNN c1 참여 sweep에서 클라이언트당 참여 횟수가 적으면 1차 근사가 붕괴하고
...
분수"가 아니라 **클라이언트당 참여 횟수**다. 다운스트림 축: grad-noise 무대에서 2차항
유무가 회복과 실명을 가른다(§5.3: Flirds .5668/.6065 vs 1차 계열 .2479/.2436 ≈ vanilla)†.
오염-강도 해상도(strmain, F-4) ⬚.
...
Flirds vs (b)는 **전 칸 1.000**(lr·steps로 per-round 이동이 커져도 라운드당 1 HVP가 (b)를
...
확인한다. LLM silo5(3위협 × 3-seed): worst-first 제거가 val-loss를 내리고(+0.0067~+0.0076)
best-first는 올린다(−0.0084~−0.0015); Flirds·Flirds-1st·loss-heur·(b)의 removal 곡선은 9/9
셀에서 엄밀히 일치하고(같은 순위 → 같은 곡선), FedIF만 free-rider(zero)에서 질적으로
얕다(worst-first +0.0038). CNN(c1 무대, 6셀 × 3-seed)은 **정확도 축**으로 같은 구조를
보인다: cifar10 label-flip·feature-noise에서 Flirds 순위의 worst−best 정확도 분리
+0.039~+0.045로 (b)와 동급(+0.038~+0.045)이고, 순위가 낮은 방법(ShapleyFL $\rho$
+0.07~+0.26)은 분리 ≈ 0 — 순위 품질이 그대로 인과 분리로 이어진다. iid 통제군은 기대대로
분리 ≈ 0이다(cifar10은 소폭 음수 −0.0033이나 (b)도 −0.0027 — 데이터량 손실이 지배하는 무대
특성이지 방법 실패가 아니다).
...
<!-- 스텁 — 실험 완주 후 작성. 확정 재료 목록(T1 스펙):
 ...
 - IID-clean 신호-부재 무대의 해석(부록 D): 방법이 아니라 무대의 결함 — retrain-(a) 특성화의 iid 칸 낮은 ρ 해석 포함.
-->
...
**A.11 대수 검증 (GPT-2 스모크).** GPT-2($N{=}5$, $R{=}3$, fp32)에서: 닫힌형 =
...
|---|---|---|---|---|---|---|
| `gsm50k5` | Llama-3.2-1B-Instruct + LoRA r16/α32 | GSM8K train 7,473 → 클라당 149문항 IID 균등 | 50 · 5/50 | 200 · 10 steps × batch 16 | SGD mom 0, lr 1e-3 | maxlen 512; val 200 = 공식 test에서 카브, 심판 = 잔여 1,119 EM |
| `gsm5` | 동일 | 동일 규칙(클라당 149문항) | 5 · 전원 | 30 · 동일 | 동일 | 오염 2/5(=40%); dual oracle (a)+(b) |
| `silo5` | Llama-3.2-1B + LoRA r16/α32 | 5-도메인 비IID(클라 = 도메인); train 200 / val 20 / test 40 | 5 · 전원 | 10 · 10 steps × batch 16 | SGD mom 0, lr 1e-3 | maxlen 768, warmup 2 |
| `anchor5` | Llama-3.2-1B-Instruct + LoRA r16/α32 | alpaca-gpt4 20k IID; val 200 / test 1000 | 5 · 전원 | 30 · 10 steps × batch 16 | SGD mom 0, lr 1e-3 | seq 512 |
| `c2` 캠페인 | CIFAR-10 = FedSVCNN / FMNIST = LeNet5 | 파티션 4종(B.3) | 100 · 10/100 | 120 · 5 epochs × batch 64 | SGD mom 0, lr 0.01 | 분배 seed 고정 |
| `c1` | MNIST = LeNet5 / CIFAR-10 = FedSVCNN | 시나리오 5종(B.3); val 2000 / test 8000 | 10 · 전원 | 10 · 5 epochs × batch 64 | SGD mom 0, lr 0.01 | dual oracle (a)+(b) $2^{10}$ |

부록 E 보조 무대: **완전참여** = `c2`의 dir1 무대에서 참여만 100/100; **$N{=}10$ LLM** =
anchor 무대의 $N{=}10$ 확장($R{=}30$, (b) = $2^{10}$ 전수); **cross-device** = 1B,
...
- **noisy / answer-swap@rate**(LLM): 클라이언트 데이터의 rate 비율에서 응답(풀이 + 최종
  답)을 같은 클라이언트의 다른 문항 것으로 순열 교체 — 형식은 완전한 정상 데이터이나
  문항-응답 대응이 깨진 현실적 mislabel. `gsm50k5`·`gsm5`는 rate 0.7; `silo5`의
  noisy($nr$)도 같은 순열 교체($nr$ = 클라 내 교체 비율).
...
1,119문항이 성능 심판 — 학습 데이터와의 분리는 공식 split 그대로다. `c2` 파티션: `iid` /
`dir1` = Dirichlet($\alpha{=}1.0$) 라벨+크기 skew / `shard` = 클라당 2-shard 라벨
skew(McMahan) / `qskew` = 크기 skew 전용(라벨 IID; GTG 비율). `c1` 시나리오 5종은
...
(b)-쪽 전제). 실행 환경(GPU·라이브러리 버전)은 셀별 rundir meta에 기록된다.
...
**B.6 LLM 위협 축에 grad-noise가 없는 이유.** 업데이트에 주입하는 등방(isotropic) 노이즈는
...
**C.1 주무대 전 방법(cross-game 포함) vs (b).** ⬚
<!-- 채움: c2fid analysis(8방법 × 위협 8종) + gsm50k5 L2(8방법 × noisy·clean) — §5.2 표
     F1·F2의 전 방법 확장판. cross-game 계열의 불일치는 근사 오차 + "다른 게임" 성분 합산임을
     본문 §5.1 구분과 함께 다시 명시. -->

| method | `c2` clean | fr | frrand | gn | lf@.15 | lf@.35 | lf@.70 | strmain | `gsm50k5` noisy | clean |
|---|---|---|---|---|---|---|---|---|---|---|
| GTG | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ |
| FedSV | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ |
| ComFedSV | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ | ⬚ |
...
**C.2 CNN `c1` 시나리오별 vs (b), 전 방법 — Spearman ↑** (3-seed 평균)
...
풀 평균(10시나리오 × 3-seed): Flirds **0.919±.134** > loss-heur 0.860±.154 > Flirds-1st
...
**C.3 CNN `c1` 시나리오별 vs (a) retrain, 전 방법 — Spearman ↑** (§5.2 표 F5의 확장)
...
각주: cifar10/quantity_skew·label_skew처럼 재정규화 계열이 유리한 칸은 (a) 게임 자체가
부분집합 크기로 재정규화되는 게임이라는 구조와 정합한다(§4.1의 게임 구분이 심판 선택에도
작용하는 사례). Pearson 표는 같은 파일에서 재생성.

**C.4 CNN `c1` Kendall·거리 풀** (10시나리오 × 3-seed; vs (b))
...
**C.5 부분참여 probe(`std50k5`: $N{=}50$, 5/50, $R{=}200$, IID-clean) — vs (b) Spearman ↑**
...
**D.1 방법 순위의 seed 간 안정성(CNN `c1`).** 같은 방법의 φ 순위를 seed 간
상관($\rho_{xseed}$, 3-seed 쌍별 평균; 10시나리오 풀)으로 잰다:

| method | $\rho_{xseed}$ ↑ |
|---|---|
| (b) oracle (자체) | 0.518±.453 |
| **Flirds** | **0.547±.394** |
...
(b) oracle 자체의 안정성이 0.518이다 — CNN 무대는 seed마다 클라이언트 기여가 실제로
달라진다. Flirds(0.547)는 **oracle의 내재 안정성을 그대로 추종**하고, Monte-Carlo
재구성 계열은 추가 분산으로 0.12~0.31까지 떨어진다.

**D.2 (b) 타깃 자기-안정성(수록 무대).** fidelity의 매칭 대상인 (b) 자신이 seed를 넘어
재현되는가((b) φ 순위의 seed 간 쌍별 Spearman 평균):

| 무대 | cross-seed $\rho$ ↑ |
|---|---|
| `anchor5` (IID-clean) | **−0.367** |
| `silo5` clean (비IID) | **+0.867** |
| `silo5` noisy | +0.933 |
| `silo5` free-rider(zero) | +0.933 |
| `c1` (10시나리오 풀) | +0.518±.453 |
| `gsm50k5` | ⬚ <!-- 채움: L1·L2 rundir (b) φ 피벗 --> |
| `c2` 캠페인 | ⬚ <!-- 채움: c2fid rundir (b) φ 피벗 --> |

판정: IID-clean 무대의 (b) 타깃은 seed-불안정하다(−0.37) — 그 위의 per-seed fidelity
1.000은 "불안정한 참값을 그때그때 정확히 좇는 것"이다. 반면 비IID·오염 무대에서는 타깃이
안정하고(+0.87~+0.93) 그곳의 fidelity가 실재 신호에 대한 재현이다. 본 논문의 fidelity 표는
이 구분과 함께 읽어야 하며(§5.2 각주 ①), 이것이 retrain-(a) 특성화의 주 표를
gsm5·silo5(신호-실재 무대)에 두고 anchor5를 참조로만 쓰는 이유다.

### 부록 E. 비용·규모 보조 실험

**E.1 완전참여 100/100(CNN, dir1 무대).** 라운드당 100/100 참여 — coalition 계열은
라운드당 $2^{100}$ 평가라 개입 arm 자체가 존재할 수 없고(전수·MC 어느 쪽도), Flirds는
라운드당 HVP 1회 그대로다(비용 주장 "cohort 크기에 상수"의 극한 무대). P1 게이팅의 절대
test acc(3-seed):
...
| Flirds · P1 | .6440±.005 | .5862±.010 | .6223±.007 | .6102±.001 | .6062 |

완전참여는 vanilla 자체를 강하게 만들지만(grad-noise .5497 vs 10/100 무대의 .2436 — 오염
업데이트가 100클라 평균에 희석), 그럼에도 P1이 오염 3셀 전부에서 vanilla를 상회한다(회수율
평균 +0.56). random-제외는 전 위협에서 vanilla보다 해롭다(회수율 −0.43~−0.47) — 게이트
이득이 "그냥 동수를 뺀 효과"가 아님의 통제 실증.

**E.2 $N{=}10$ LLM, (b) exact $2^{10}$ (◐ 1-seed).** 전원 참여 $N{=}10$·$R{=}30$에서 (b)를
라운드당 $2^{10}{=}1{,}024$ coalition 완전 열거로 계산: 같은-게임 3종 모두 Spearman
1.000(값 수준 Pearson 잔차만 분리 — Flirds $1{-}r \approx 9{\times}10^{-7}$로 최소).
비용: **(b) 117,649s(32.7h) vs Flirds 733s = 1/160**(Flirds-1st 240s, loss-heur 1,240s).
$N{=}5$($2^5$)에서 $N{=}10$($2^{10}$)으로 갈 때 oracle 비용이 실증적으로 폭발하는 동안
Flirds는 라운드당 HVP 1회로 고정임을 보이는 규모 축 실측이다.

**E.3 cross-device anchor($N{=}100$, 10/100, $R{=}30$).** (b) per-round $2^{10}$ ≈
