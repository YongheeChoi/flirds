...
그리고 이 질문은 연합학습(FL) 환경에서 가장 큰 실용성을 발휘한다.
연합 학습에서는 데이터가 클라이언트 단위로 분리·비공개된 채 하나의 모델을 공동 학습하므로,
보상 분배·데이터 마켓·품질 관리가 전부 클라이언트별 기여도 수치를 요구한다.
FL의 기여도 평가는 그 세팅의 조건 아래에서 이루어진다: 서버는 원데이터가 아니라 각
